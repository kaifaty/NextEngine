#!/usr/bin/env python3
"""Freeze R3A V3 source, split and native-48 kHz NDAC before target audio."""
...
import numpy as np
import physical_sound_contact_field_r3a_v3_common as common
...
def parse_headers(data: bytes) -> tuple[int, dict[str, str]]:
    ...
    if not blocks:
        raise common.R3AV3Error("HTTPS response has no HTTP header block")
    lines = blocks[-1].decode("latin1").split("\r\n")
    parts = lines[0].split()
    if len(parts) < 2 or not parts[1].isdigit():
        raise common.R3AV3Error("HTTPS response status is malformed")
    headers = {
...
def curl_request(url: str, arguments: list[str]) -> tuple[bytes, int, dict[str, str]]:
    with tempfile.TemporaryDirectory(prefix="nextengine-r3a-v3-fetch-") as directory:
...
def require_archive_headers(status: int, headers: dict[str, str]) -> None:
    if (
        ...
        raise common.R3AV3Error("REALIMPACT archive HTTP identity changed")
...
def fetch_entry(spec: dict[str, Any]) -> bytes:
    ...
    if len(body) != length:
        raise common.R3AV3Error(f"short ZIP entry range for {spec['name']}")
    local = body[:header_bytes]
    if len(local) < 30 or local[:4] != b"PK\x03\x04":
        raise common.R3AV3Error(f"bad ZIP local header for {spec['name']}")
    ...
    if (
        ...
        raise common.R3AV3Error(f"ZIP metadata changed for {spec['name']}")
    try:
        raw = zlib.decompress(body[header_bytes:], wbits=-15)
    except zlib.error as error:
        raise common.R3AV3Error(f"decompress {spec['name']}: {error}") from error
    if (
        len(raw) != spec["uncompressed_bytes"]
        or f"{zlib.crc32(raw) & 0xFFFFFFFF:08x}" != spec["crc32"]
        or common.sha256_bytes(raw) != spec["sha256"]
    ):
        raise common.R3AV3Error(f"decoded integrity changed for {spec['name']}")
    return raw
...
def npy(raw: bytes, label: str) -> np.ndarray:
    try:
        ...
        raise common.R3AV3Error(f"parse {label}: {error}") from error
    ...
    for line in raw.decode("utf-8").splitlines():
        if line.startswith("v "):
            ...
            if len(coordinates) != 3:
                raise common.R3AV3Error("REALIMPACT mesh has a malformed vertex")
            vertices.append(coordinates)
    values = np.asarray(vertices, dtype=np.float64)
    if values.shape != (48_272, 3) or not np.all(np.isfinite(values)):
        raise common.R3AV3Error("REALIMPACT Plastic Bin mesh identity changed")
    return {
...
def derive_contacts(arrays: dict[str, np.ndarray]) -> list[dict[str, Any]]:
    ...
    for name, shape in expected_shapes.items():
        if arrays[name].shape != shape:
            raise common.R3AV3Error(f"{name} shape changed: {arrays[name].shape}")
    if not np.all(np.isfinite(arrays["vertex_xyz"])) or not np.all(
        np.isfinite(arrays["listener_xyz"])
    ):
        raise common.R3AV3Error("REALIMPACT coordinates contain non-finite values")
    ...
    if len(source_order) != common.IMPACT_COUNT:
        raise common.R3AV3Error("Plastic Bin no longer exposes exactly five impacts")
    ...
    for impact_index, (vertex_id, role) in enumerate(
        ...
        if impact_rows.size != common.LISTENERS_PER_IMPACT or selected.size != 1:
            raise common.R3AV3Error("canonical listener is not unique for every impact")
        row = int(selected[0])
        ...
        if not np.all(arrays["vertex_xyz"][impact_rows] == position):
            raise common.R3AV3Error("impact position changes across listeners")
        if canonical_listener is None:
            canonical_listener = listener
        elif not np.array_equal(listener, canonical_listener):
            raise common.R3AV3Error("canonical listener changes across impacts")
        contacts.append(
            {
    ...
    if [item["row_index"] for item in contacts] != [7, 607, 1207, 1807, 2407]:
        raise common.R3AV3Error("Plastic Bin canonical source order changed")
    return contacts
...
def repository_identity(repository: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise common.R3AV3Error("FlowDec repository revision or cleanliness changed")
    return {
...
def environment_identity() -> dict[str, Any]:
    if sys.version_info[:3] != (3, 11, 15) or platform.system() != "Linux":
        raise common.R3AV3Error("NDAC-75 oracle Python/platform identity changed")
    distributions = {
    ...
    if selected != common.DAC_REQUIRED_PACKAGES:
        raise common.R3AV3Error(f"NDAC-75 oracle package identity changed: {selected}")
    record_sha256 = {}
    for name in common.DAC_REQUIRED_PACKAGES:
        record = distributions[name].read_text("RECORD")
        if record is None:
            raise common.R3AV3Error(f"NDAC-75 dependency has no RECORD: {name}")
...
def run(
    ...
    try:
        ...
        import physical_sound_contact_field_r3a_v3_codec_oracle as oracle
        ...
        report = {
            ...
            "status": "Validated",
            "decision": "R3AV3LearnedCodecOracleReady",
            "profile": common.PROFILE_ID,
...
def main() -> None:
    ...
    print(f"R3A V3 source/dependency preflight: {output}")
