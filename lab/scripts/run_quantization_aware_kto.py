#!/usr/bin/env python3
"""Run the single bounded TRAIN-4 R115 quantization-aware KTO solve."""
...
from next_lab.quantization_aware_kto_execution import (
    CACHE_FILE_NAME,
    execute_quantization_aware_kto,
)
...
def main() -> None:
    ...
    if repository["dirty"]:
        raise ValueError("R115 execution requires a clean repository")
    ...
    report, cache_bytes = execute_quantization_aware_kto(
    ...
    staging = Path(tempfile.mkdtemp(prefix="nextengine-r115-kto-", dir=output.parent))
    try:
        (staging / "quantization-aware-kto-execution.json").write_bytes(
        ...
        json.dumps(
            {
                ...
                "qp_status": report["solver_result"]["qp"]["status"],
                "exact_emission_audits": report["solver_result"][
                    "exact_emission_audits"
                ],
                "accepted_line_search_fraction": report["solver_result"][
                    "accepted_line_search_fraction"
                ],
...
def _run_validations(
    ...
    for row in commands:
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"R115 validation {row['id']} failed:\n{detail}")
...
def _external_directory(candidate: Path) -> Path:
    ...
    if output == repository or repository in output.parents:
        raise ValueError("R115 execution output must stay outside repository")
