#!/usr/bin/env python3
"""Run the frozen query-seeing R3A V3 native-48 kHz NDAC oracle."""
...
def configure_torch(torch: Any) -> None:
    ...
    if torch.get_num_interop_threads() != 1:
        try:
            ...
            raise common.R3AV3Error("cannot freeze torch interop threads") from error
...
def load_model(weights: Path) -> tuple[Any, Any]:
    ...
    if identity != expected:
        raise common.R3AV3Error(f"loaded NDAC-75 model identity changed: {identity}")
    return torch, model
...
def reconstruct_codes(
    ...
    if value.ndim != 1 or value.dtype != np.float32 or not np.all(np.isfinite(value)):
        raise common.R3AV3Error("NDAC input must be one finite float32 waveform")
...
def warm_model(torch: Any, model: Any) -> None:
    ...
    if reconstructed.shape != warmup.shape or codes.shape[:2] != (
        ...
        raise common.R3AV3Error("NDAC-75 synthetic warm-up failed")
...
def synthetic_control(weights: Path) -> dict[str, Any]:
    ...
    if not np.array_equal(first, second) or not np.array_equal(
        ...
        raise common.R3AV3Error("NDAC-75 is not exact after frozen synthetic warm-up")
    return {
        "device": "cpu",
...
def evaluate(
    ...
    if waveforms.shape != (4, common.SAMPLE_COUNT) or positions.shape != (4, 3):
        raise common.R3AV3Error("oracle requires three fit and one development contact")
    ...
    if not np.isfinite(fit_peak) or fit_peak <= 0.0:
        raise common.R3AV3Error("fit-only normalization peak is invalid")
...
def run(
    ...
    manifest_path = common.external_file(root, manifest_argument, "R3A V3 manifest")
    manifest_bytes, manifest = common.load_json(manifest_path, "R3A V3 manifest")
    ...
    common.validate_dac_weights(weights)
    from physical_sound_contact_field_r3a_v3_preflight import environment_identity

    if environment_identity() != manifest["dac_dependency"]["environment"]:
        raise common.R3AV3Error("NDAC-75 runtime environment changed after preflight")
    ...
    if contacts_directory.is_relative_to(root) or not contacts_directory.is_dir():
        raise common.R3AV3Error("contacts must be an external extraction directory")
    report_bytes, extraction_report = common.load_json(
        contacts_directory / "report.json", "R3A V3 extraction report"
    )
    metadata_bytes, metadata = common.load_json(
        contacts_directory / "contacts-metadata.json", "R3A V3 contacts metadata"
    )
    if (
        ...
        raise common.R3AV3Error("R3A V3 contact extraction lineage changed")
...
def main() -> None:
    ...
    _, report = common.load_json(output / "report.json", "R3A V3 oracle report")
    print(f"R3A V3 native-48 kHz NDAC oracle: {output}")
