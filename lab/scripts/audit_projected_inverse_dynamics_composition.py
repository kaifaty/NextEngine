#!/usr/bin/env python3
"""Run report-only R135 projected inverse-dynamics composition conformance."""
...
def main() -> None:
    ...
    from next_lab.projected_inverse_dynamics_composition_conformance import (
        build_projected_inverse_dynamics_composition_conformance,
    )
    ...
    if _repository_state(repository_root) != repository:
        raise ValueError("R135 repository identity changed during validation")

    report = build_projected_inverse_dynamics_composition_conformance(
    ...
    staging = Path(tempfile.mkdtemp(prefix="nextengine-r135-", dir=output.parent))
    try:
        report_path = staging / "projected-inverse-dynamics-composition.json"
        ...
        json.dumps(
            {
                ...
                "synthetic_system_compositions": report[
                    "synthetic_system_compositions"
                ],
                "synthetic_singular_value_decompositions": report[
                    "synthetic_singular_value_decompositions"
                ],
                "real_inverse_dynamics_system_assemblies": report[
...
def _run_validations(
    ...
    for row in commands:
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"R135 validation {row['id']} failed:\n{detail}")
...
def _external_directory(candidate: Path) -> Path:
    ...
    if output == repository or repository in output.parents:
        raise ValueError("R135 output must stay outside repository")
