#!/usr/bin/env python3
"""Consume the single bounded TRAIN-4 R130 projected fixed-PD execution."""
...
def main() -> None:
    ...
    from next_lab.tangent_projected_fixed_pd_execution import (
        execute_and_build_tangent_projected_fixed_pd_report,
    )
    ...
    if _repository_state(repository_root) != repository:
        raise ValueError("R130 repository identity changed during validation")
    ...
    if _repository_state(repository_root) != repository:
        raise ValueError("R130 repository changed during descriptor export")
    report, cache_bytes = execute_and_build_tangent_projected_fixed_pd_report(
    ...
    staging = Path(tempfile.mkdtemp(prefix="nextengine-r130-", dir=output.parent))
    try:
        if cache_bytes is not None:
            cache_path = staging / "solver-private-r130-tangent-projected-fixed-pd.npz"
            cache_path.write_bytes(cache_bytes)
            if _sha256(cache_path) != report["solver_private_cache"]["file_sha256"]:
                raise ValueError("R130 solver-private cache identity differs")
        report_path = staging / "tangent-projected-fixed-pd-execution.json"
        ...
        json.dumps(
            {
                ...
                "invalid_reason": report["solver_result"]["invalid_reason"],
                ...
                "force_gauge_interval_classifications": report[
...
def _run_validations(
    ...
    for row in commands:
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"R130 validation {row['id']} failed:\n{detail}")
...
def _external_directory(candidate: Path) -> Path:
    ...
    if output == repository or repository in output.parents:
        raise ValueError("R130 output must stay outside repository")
