#!/usr/bin/env python3
"""Freeze R3A V3B source, split and native-48 kHz NDAC before target audio."""
...
import numpy as np
import physical_sound_contact_field_r3a_v3b_common as common


def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--dac-repository", required=True, type=Path)
    parser.add_argument("--dac-release-archive", required=True, type=Path)
...
def parse_headers(data: bytes) -> tuple[int, dict[str, str]]:
    ...
    if not blocks:
        raise common.R3AV3BError("HTTPS response has no HTTP header block")
    lines = blocks[-1].decode("latin1").split("\r\n")
    parts = lines[0].split()
    if len(parts) < 2 or not parts[1].isdigit():
        raise common.R3AV3BError("HTTPS response status is malformed")
    headers = {
...
def curl_request(url: str, arguments: list[str]) -> tuple[bytes, int, dict[str, str]]:
    with tempfile.TemporaryDirectory(prefix="nextengine-r3a-v3b-fetch-") as directory:
...
def require_archive_headers(status: int, headers: dict[str, str]) -> None:
    if (
        ...
        raise common.R3AV3BError("REALIMPACT archive HTTP identity changed")
...
def fetch_entry(spec: dict[str, Any]) -> bytes:
    ...
    if len(body) != length:
        raise common.R3AV3BError(f"short ZIP entry range for {spec['name']}")
    local = body[:header_bytes]
    if len(local) < 30 or local[:4] != b"PK\x03\x04":
        raise common.R3AV3BError(f"bad ZIP local header for {spec['name']}")
    ...
    if (
        ...
        raise common.R3AV3BError(f"ZIP metadata changed for {spec['name']}")
    try:
        raw = zlib.decompress(body[header_bytes:], wbits=-15)
    except zlib.error as error:
        raise common.R3AV3BError(f"decompress {spec['name']}: {error}") from error
    if (
        len(raw) != spec["uncompressed_bytes"]
        or f"{zlib.crc32(raw) & 0xFFFFFFFF:08x}" != spec["crc32"]
        or common.sha256_bytes(raw) != spec["sha256"]
    ):
        raise common.R3AV3BError(f"decoded integrity changed for {spec['name']}")
    return raw
...
def npy(raw: bytes, label: str) -> np.ndarray:
    try:
        ...
        raise common.R3AV3BError(f"parse {label}: {error}") from error
    ...
    for line in raw.decode("utf-8").splitlines():
        if line.startswith("v "):
            ...
            if len(coordinates) != 3:
                raise common.R3AV3BError("REALIMPACT mesh has a malformed vertex")
            vertices.append(coordinates)
    values = np.asarray(vertices, dtype=np.float64)
    if values.shape != (48_418, 3) or not np.all(np.isfinite(values)):
        raise common.R3AV3BError("REALIMPACT Purple Scoop mesh identity changed")
    return {
...
def derive_contacts(arrays: dict[str, np.ndarray]) -> list[dict[str, Any]]:
    ...
    for name, shape in expected_shapes.items():
        if arrays[name].shape != shape:
            raise common.R3AV3BError(f"{name} shape changed: {arrays[name].shape}")
    if not np.all(np.isfinite(arrays["vertex_xyz"])) or not np.all(
        np.isfinite(arrays["listener_xyz"])
    ):
        raise common.R3AV3BError("REALIMPACT coordinates contain non-finite values")
    ...
    if len(source_order) != common.IMPACT_COUNT:
        raise common.R3AV3BError("Purple Scoop no longer exposes exactly five impacts")
    ...
    for impact_index, (vertex_id, role) in enumerate(
        ...
        if impact_rows.size != common.LISTENERS_PER_IMPACT or selected.size != 1:
            raise common.R3AV3BError(
                "canonical listener is not unique for every impact"
            )
        row = int(selected[0])
        ...
        if not np.all(arrays["vertex_xyz"][impact_rows] == position):
            raise common.R3AV3BError("impact position changes across listeners")
        if canonical_listener is None:
            canonical_listener = listener
        elif not np.array_equal(listener, canonical_listener):
            raise common.R3AV3BError("canonical listener changes across impacts")
        contacts.append(
            {
    ...
    if [item["row_index"] for item in contacts] != [7, 607, 1207, 1807, 2407]:
        raise common.R3AV3BError("Purple Scoop canonical source order changed")
    return contacts
...
def repository_identity(repository: Path) -> dict[str, Any]:
    def git(*arguments: str) -> str:
        ...
        if result.returncode != 0:
            raise common.R3AV3BError(
                f"inspect FlowDec repository: {result.stderr.strip()}"
            )
    ...
    if (
        revision != common.DAC_REPOSITORY_REVISION
        or tree != common.DAC_REPOSITORY_TREE
        or status
    ):
        raise common.R3AV3BError("FlowDec repository revision or cleanliness changed")
    return {
...
def environment_identity() -> dict[str, Any]:
    if sys.version_info[:3] != (3, 11, 15) or platform.system() != "Linux":
        raise common.R3AV3BError("NDAC-75 oracle Python/platform identity changed")
    distributions = {
    ...
    if selected != common.DAC_REQUIRED_PACKAGES:
        raise common.R3AV3BError(f"NDAC-75 oracle package identity changed: {selected}")
    record_sha256 = {}
    for name in common.DAC_REQUIRED_PACKAGES:
        record = distributions[name].read_text("RECORD")
        if record is None:
            raise common.R3AV3BError(f"NDAC-75 dependency has no RECORD: {name}")
...
def build_manifest(
    ...
    return {
        ...
        "source_disjoint_audit": {
            "prior_accessed_dataset_object_ids": list(
                common.PRIOR_ACCESSED_DATASET_OBJECT_IDS
            ),
            "candidate_absent": True,
            "selection_policy": (
                "new REALIMPACT object and non-family-near-duplicate relative to "
                "prior representation-gate development objects"
            ),
        },
        "predecessor_invalid_evidence": {
            ...
            "preflight_report_sha256": (
                "0916f8a7b1b42c285ad6816652220b87153909308109efc3dcd682d91ac85c9e"
            ),
            "extraction_report_sha256": (
                "b259994c3bd77cd524a0308e2304ab41c8835bd9567a5a41b8242e60e9bcd3fb"
            ),
            "development_waveform_accessed": True,
            "sealed_waveform_samples_decoded": 0,
            "quality_metrics_published": False,
            "failure": "ndac_decoded_143992_samples_for_144000_sample_target",
            "repair_evidence": (
                "synthetic_144000_sample_control_only; append_exactly_8_zero_guard_"
                "samples_before_official_preprocess"
            ),
        },
        ...
        "oracle_policy": {
            ...
            "candidate": "flowdec_release_underlying_ndac_75_native_48khz_query_reconstruction",
            "resampling": "none_native_48000_hz",
            "identity_control": "target_compared_to_exact_float64_copy_must_be_zero",
            "decode_guard_samples": common.DAC_DECODE_GUARD_SAMPLES,
            "decode_guard_policy": (
                "append_eight_zeros_before_official_preprocess_then_require_decoded_"
                "length_at_least_target_and_slice_exact_target_length"
            ),
            "evaluation_band_hz": [120, 18_000],
            "flowdec_postfilter": "excluded_stochastic_not_loaded",
...
def run(
    ...
    release_archive_argument: Path,
    weights_argument: Path,
    output_argument: Path,
) -> Path:
    repository = common.external_directory(
        root, repository_argument, "FlowDec repository"
    )
    release_archive = common.external_file(
        root, release_archive_argument, "FlowDec release archive"
    )
    weights = common.external_file(root, weights_argument, "NDAC-75 weights")
    ...
    try:
        ...
        dependency = {
            ...
            "repository_tree": common.DAC_REPOSITORY_TREE,
            "model_type": common.DAC_MODEL_TYPE,
            "model_bitrate": common.DAC_MODEL_BITRATE,
            "model_tag": common.DAC_MODEL_TAG,
            "model_sample_rate_hz": common.DAC_MODEL_SAMPLE_RATE_HZ,
            "model_hop_length": common.DAC_MODEL_HOP_LENGTH,
            "model_codebooks": common.DAC_MODEL_CODEBOOKS,
            "model_codebook_size": common.DAC_MODEL_CODEBOOK_SIZE,
            "weights_url": common.DAC_WEIGHTS_URL,
            "weights_sha256": common.DAC_WEIGHTS_SHA256,
            "release_archive_sha256": common.DAC_RELEASE_ARCHIVE_SHA256,
            "release_archive_audit": common.validate_dac_release_archive(
                release_archive, weights
            ),
            "weights_audit": common.validate_dac_weights(weights),
            "environment": environment_identity(),
        }
        import physical_sound_contact_field_r3a_v3b_codec_oracle as oracle
        ...
        report = {
            ...
            "status": "Validated",
            "decision": "R3AV3BLearnedCodecOracleReady",
            "profile": common.PROFILE_ID,
...
def main() -> None:
    ...
    output = run(
        ...
        arguments.dac_release_archive,
        arguments.dac_weights,
        arguments.output,
    )
    print(f"R3A V3B source/dependency preflight: {output}")
