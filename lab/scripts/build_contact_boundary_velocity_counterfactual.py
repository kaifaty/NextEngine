#!/usr/bin/env python3
"""Build the hash-closed one-scalar TRAIN-4 R100 discriminator."""
...
def main() -> None:
    ...
    if repository["dirty"]:
        raise RuntimeError(
            "boundary-velocity counterfactual requires a clean repository"
        )
    staging = Path(
        tempfile.mkdtemp(
            prefix="nextengine-boundary-velocity-counterfactual-",
            dir=output.parent,
        )
    )
    try:
