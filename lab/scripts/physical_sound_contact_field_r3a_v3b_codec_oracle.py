#!/usr/bin/env python3
"""Run the frozen query-seeing R3A V3B native-48 kHz NDAC oracle."""
...
def configure_torch(torch: Any) -> None:
    ...
    if torch.get_num_interop_threads() != 1:
        try:
            ...
            raise common.R3AV3BError("cannot freeze torch interop threads") from error
...
def load_model(weights: Path) -> tuple[Any, Any]:
    ...
    if identity != expected:
        raise common.R3AV3BError(f"loaded NDAC-75 model identity changed: {identity}")
    return torch, model
...
def reconstruct_codes(
    ...
    if value.ndim != 1 or value.dtype != np.float32 or not np.all(np.isfinite(value)):
        raise common.R3AV3BError("NDAC input must be one finite float32 waveform")
    guarded = np.pad(value, (0, common.DAC_DECODE_GUARD_SAMPLES))
    tensor = torch.from_numpy(np.ascontiguousarray(guarded)).reshape(1, 1, -1)
    with torch.inference_mode():
        ...
        if decoded.shape[-1] < value.size:
            raise common.R3AV3BError("NDAC-75 decoded less than guarded target length")
        decoded = decoded[..., : value.size]
    return (
...
def warm_model(torch: Any, model: Any) -> None:
    ...
    if reconstructed.shape != warmup.shape or codes.shape[:2] != (
        1,
        common.DAC_MODEL_CODEBOOKS,
    ):
        raise common.R3AV3BError("NDAC-75 synthetic warm-up failed")


def synthetic_control(weights: Path) -> dict[str, Any]:
    torch, model = load_model(weights)
    warm_model(torch, model)
    sample_count = common.ANALYSIS_SAMPLES
    ...
    if not np.array_equal(first, second) or not np.array_equal(
        ...
        raise common.R3AV3BError("NDAC-75 is not exact after frozen synthetic warm-up")
    expected_shape = (1, common.DAC_MODEL_CODEBOOKS, common.DAC_ANALYSIS_CODE_FRAMES)
    if first.shape != (sample_count,) or first_codes.shape != expected_shape:
        raise common.R3AV3BError("NDAC-75 guarded analysis-shape control changed")
    return {
        "device": "cpu",
...
def identity_gate(control: dict[str, float]) -> dict[str, Any]:
    endpoints = {endpoint: control[endpoint] == 0.0 for endpoint in PRIMARY_ENDPOINTS}
    return {"passed": all(endpoints.values()), "exact_zero": endpoints}
...
def evaluate(
    ...
    if waveforms.shape != (4, common.SAMPLE_COUNT) or positions.shape != (4, 3):
        raise common.R3AV3BError(
            "oracle requires three fit and one development contact"
        )
    ...
    if not np.isfinite(fit_peak) or fit_peak <= 0.0:
        raise common.R3AV3BError("fit-only normalization peak is invalid")
    normalized = aligned * (0.92 / fit_peak)
    analysis = normalized[:, : common.ANALYSIS_SAMPLES]
    target = np.ascontiguousarray(analysis[3], dtype=np.float32).astype(np.float64)
    distances = np.linalg.norm(positions[:3] - positions[3], axis=1)
    nearest_index = int(np.argmin(distances))
    baseline = analysis[nearest_index]

    ndac_input = np.ascontiguousarray(target, dtype=np.float32)
    identity_control = ndac_input.astype(np.float64)
    torch, model = load_model(weights)
    warm_model(torch, model)
    first, first_codes = reconstruct_codes(torch, model, ndac_input)
    second, second_codes = reconstruct_codes(torch, model, ndac_input)
    exact_repeat = np.array_equal(first, second) and np.array_equal(
        first_codes, second_codes
    )
    if not exact_repeat:
        raise common.R3AV3BError(
            "NDAC-75 target reconstruction is not exact after warm-up"
        )
    candidate = second.astype(np.float64)

    baseline_metrics = metrics(target, baseline)
    identity_metrics = metrics(target, identity_control)
    candidate_metrics = metrics(target, candidate)
    identity_result = identity_gate(identity_metrics)
    codec_result = candidate_gate(candidate_metrics, baseline_metrics)
    if not identity_result["passed"]:
        decision = "INVALID_NATIVE_IDENTITY_CONTROL"
    ...
    return {
        ...
        "native_identity_control": {
            "id": "exact_float32_native_48khz_identity",
            "metrics": identity_metrics,
            "gate": identity_result,
        },
        "learned_codec": {
            "id": "flowdec_release_underlying_ndac_75_800k_7k5bps",
            ...
            "decoded_48k_sha256": hashlib.sha256(second.tobytes()).hexdigest(),
        ...
        "signals": {
            ...
            "native_identity_control": identity_control,
...
def run(
    ...
    manifest_path = common.external_file(root, manifest_argument, "R3A V3B manifest")
    manifest_bytes, manifest = common.load_json(manifest_path, "R3A V3B manifest")
    ...
    common.validate_dac_weights(weights)
    from physical_sound_contact_field_r3a_v3b_preflight import environment_identity

    if environment_identity() != manifest["dac_dependency"]["environment"]:
        raise common.R3AV3BError("NDAC-75 runtime environment changed after preflight")
    ...
    if contacts_directory.is_relative_to(root) or not contacts_directory.is_dir():
        raise common.R3AV3BError("contacts must be an external extraction directory")
    report_bytes, extraction_report = common.load_json(
        contacts_directory / "report.json", "R3A V3B extraction report"
    )
    metadata_bytes, metadata = common.load_json(
        contacts_directory / "contacts-metadata.json", "R3A V3B contacts metadata"
    )
    if (
        ...
        raise common.R3AV3BError("R3A V3B contact extraction lineage changed")
    ...
    try:
        ...
        report = {
            ...
            "native_identity_control_policy": "all_five_metrics_exact_zero",
            "result": result,
...
def main() -> None:
    ...
    _, report = common.load_json(output / "report.json", "R3A V3B oracle report")
    print(f"R3A V3B native-48 kHz NDAC oracle: {output}")
