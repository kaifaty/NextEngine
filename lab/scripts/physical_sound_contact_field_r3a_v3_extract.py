#!/usr/bin/env python3
"""Extract only authorized R3A V3 rows from the pinned raw ZIP deflate member."""
...
def validate_npy_header(header: bytes) -> None:
    if (
        ...
        raise common.R3AV3Error("REALIMPACT transfer NPY header changed")
    descriptor = header[10:].decode("ascii")
    if (
        "'descr': '<f4'" not in descriptor
        or "'fortran_order': False" not in descriptor
        or "'shape': (3000, 209057)" not in descriptor
        or not descriptor.endswith("\n")
    ):
        raise common.R3AV3Error("REALIMPACT transfer NPY descriptor changed")
...
def extract_rows(
    ...
    *,
    ...
    if row_indices != sorted(row_indices) or len(set(row_indices)) != len(row_indices):
        raise common.R3AV3Error("selected audio rows must be unique and ordered")
    if not row_indices or row_indices[-1] >= common.ROW_COUNT:
        raise common.R3AV3Error("selected audio row is outside the frozen array")
    ...
    while raw_offset < stop_raw:
        ...
        if not block:
            raise common.R3AV3Error("compressed audio ended before development row")
        compressed_read += len(block)
        remaining = stop_raw - raw_offset
        try:
            decoded = decoder.decompress(block, max_length=remaining)
        except zlib.error as error:
            raise common.R3AV3Error(f"decompress transfer member: {error}") from error
        if not decoded:
            continue
    ...
    if raw_offset != stop_raw or any(
        ...
        raise common.R3AV3Error("selected transfer rows are incomplete")
    stacked = np.stack(
    ...
    if stacked.shape != (len(row_indices), sample_count) or not np.all(
        ...
        raise common.R3AV3Error("selected transfer rows have invalid samples")
...
def run(
    ...
    if compressed_path.stat().st_size != common.AUDIO_ENTRY["compressed_bytes"]:
        raise common.R3AV3Error("compressed audio member size changed")
    ...
    if (
        ...
        raise common.R3AV3Error("R3A V3 seal boundary changed")
    ...
    try:
        ...
        metadata = {
            "schema": "nextengine.experimental-physical-sound-r3a-v3-contacts.v1",
        ...
        report = {
            ...
            "status": "Validated",
            "decision": "R3AV3AuthorizedContactsExtracted",
            "profile": common.PROFILE_ID,
            "manifest_sha256": common.sha256_bytes(manifest_bytes),
...
def main() -> None:
    ...
    print(f"R3A V3 contact extraction: {output}")
