...
class MotorMirrorTests(unittest.TestCase):
    ...
    def test_forward_start_stop_v3_reward_has_dense_monotonic_signal(self) -> None:
        quaternion = torch.tensor([[0, 0, 0, 1 << 30]], dtype=torch.int64)
        zeros3 = torch.zeros((1, 3), dtype=torch.int64)
        zeros23 = torch.zeros((1, 23), dtype=torch.int64)
        common = {
        ...
        stationary, stationary_total = locomotion_reward_q16_tensor(
            local_linear_velocity_raw=zeros3,
            contacting_foot_count=torch.tensor([2], dtype=torch.int64),
            **common,
        )
        halfway_velocity = zeros3.clone()
        halfway_velocity[:, 2] = 250_000
        halfway, _ = locomotion_reward_q16_tensor(
            local_linear_velocity_raw=halfway_velocity,
            contacting_foot_count=torch.tensor([2], dtype=torch.int64),
            **common,
        )
        tracking_velocity = zeros3.clone()
        tracking_velocity[:, 2] = 500_000
        tracking, tracking_total = locomotion_reward_q16_tensor(
            local_linear_velocity_raw=tracking_velocity,
            contacting_foot_count=torch.tensor([1], dtype=torch.int64),
            **common,
        )
        self.assertEqual(stationary[0, 0].item(), 16_384)
        self.assertEqual(stationary_total.item(), 49_152)
        self.assertEqual(halfway[0, 0].item(), 41_943)
        self.assertLess(stationary[0, 0].item(), halfway[0, 0].item())
