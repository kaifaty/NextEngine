...
impl ApplicationCoordinator {
    ...
    pub fn save_current_prepared_run(&mut self) -> Result<ContentHash, ApplicationError> {
        ...
        let receipt = match prepared.physical_animation.as_ref() {
            Some(physical_animation) => self
                .save_store
            ...
            None => self.save_store.commit_world_checkpoint_with_cognition(
                compatibility,
                &prepared.checkpoint,
                &prepared.streaming,
                prepared.routine.as_ref(),
    ...
    fn prepare_close(&mut self) -> Result<(), ApplicationError> {
        ...
        let prepared = self
            ...
            .ok_or(ApplicationError::NoRunOutcome)?;
        let compatibility = save_compatibility(&self.activated_project, &prepared.checkpoint)?;
        let image = match prepared.physical_animation.as_ref() {
            Some(physical_animation) => self
                .save_store
            ...
            None => self.save_store.prepare_world_checkpoint_with_cognition(
                compatibility,
                &prepared.checkpoint,
                &prepared.streaming,
                prepared.routine.as_ref(),
