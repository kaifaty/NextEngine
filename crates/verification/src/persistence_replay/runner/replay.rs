...
use next_contracts::persistence::ReplayManifestV8;

use crate::run_replay_manifest_v8_with_physics_options;
...
pub(super) fn verify(
    ...
    let replay_bytes = replay_manifest
        .to_jcs_bytes()
        .map_err(|error| PersistenceReplayCheckError::new("encode replay V8", error.to_string()))?;
    let decoded_replay_manifest =
        ReplayManifestV8::from_jcs_bytes(&replay_bytes, CanonicalDecodeLimits::default()).map_err(
            |error| PersistenceReplayCheckError::new("decode replay V8", error.to_string()),
        )?;
    if decoded_replay_manifest != replay_manifest {
        return Err(PersistenceReplayCheckError::condition(
            "replay V8 JCS round trip is exact",
        ));
    }
    let replay = run_replay_manifest_v8_with_physics_options(
