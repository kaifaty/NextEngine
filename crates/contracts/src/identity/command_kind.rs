...
use crate::physics::{
    ...
    WATER_FLOW_COMMAND_SCHEMA_VERSION, WATER_FLOW_PRIORITY_CLASS, WATER_VOLUME_CAPABILITY_ID,
...
impl CommandKindRegistryV1 {
    ...
    fn core(
        ...
        if include_water_flow {
            let (key, value) = entry(
                ...
                WATER_FLOW_COMMAND_SCHEMA_VERSION,
                WATER_FLOW_COMMAND_KIND_ID,
                WATER_FLOW_PRIORITY_CLASS,
                b"nextengine.command-validator.water-flow.v1\0",
                WATER_FLOW_CAPABILITY_ID,
            )?;
            entries.insert(key, value);
        }
        let value = Self {
    ...
    pub fn accepts_payload(entry: &CommandKindRegistryEntryV1, payload: &CommandPayload) -> bool {
        matches!(
            ...
            (NOOP_COMMAND_SCHEMA_ID, CommandPayload::Noop)
                ...
                | (WATER_FLOW_COMMAND_SCHEMA_ID, CommandPayload::WaterFlow(_))
        )
    }
    ...
    pub fn allows_phase(entry: &CommandKindRegistryEntryV1, phase: CommandPhase) -> bool {
        match entry.payload_schema_id.as_str() {
            ...
            WATER_VOLUME_COMMAND_SCHEMA_ID | WATER_FLOW_COMMAND_SCHEMA_ID => true,
            _ => false,
        }
    }
...
mod tests {
    ...
    #[test]
    fn r8d_water_flow_entry_allows_both_phases_and_preserves_r8c() {
        let historical = CommandKindRegistryV1::core_r8c().expect("R8c registry");
        let current = CommandKindRegistryV1::core_r8d().expect("R8d registry");
        assert_eq!(historical.entries.len(), 9);
        assert_eq!(current.entries.len(), 10);
        assert!(
            historical
                .entries
                .iter()
        ...
        let schema = SchemaId::new(WATER_FLOW_COMMAND_SCHEMA_ID).expect("schema");
        let entry = current
            .descriptor(&schema, WATER_FLOW_COMMAND_SCHEMA_VERSION)
            .expect("water-flow command is registered");
        assert_eq!(entry.priority_class, WATER_FLOW_PRIORITY_CLASS);
        assert!(CommandKindRegistryV1::allows_phase(
            entry,
            CommandPhase::Ingress
        ));
