...
std::string sha256_hex(std::string_view input) {
    ...
    for (std::size_t offset = 0; offset < bytes.size(); offset += 64U) {
        ...
        for (std::size_t index = 0; index < 16U; ++index) {
            const std::size_t cursor = offset + 4U * index;
            words[index] = (static_cast<std::uint32_t>(bytes[cursor]) << 24U)
                | (static_cast<std::uint32_t>(bytes[cursor + 1U]) << 16U)
                | (static_cast<std::uint32_t>(bytes[cursor + 2U]) << 8U)
                | static_cast<std::uint32_t>(bytes[cursor + 3U]);
        }
        ...
        for (std::size_t index = 0; index < words.size(); ++index) {
            ...
            const std::uint32_t temporary1 = h + sum1 + choose + ROUND_CONSTANTS[index]
                + words[index];
