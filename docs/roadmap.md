...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, sealed real-transfer protocol, rejected triangle-soup V1 and supported exact-weld geometry V2 `IMPLEMENTED`; current status `PS-2_EXACT_WELD_GEOMETRY_PREFLIGHT_SUPPORTED / V1_REJECTED / BYTE_IDENTICAL_GEOMETRY_BLOCKS / RESERVED_AUDIO_BYTES_ZERO / PITCHER_CALIBRATION_MANIFEST_NEXT / PITCHER_AND_PLANTER_AUDIO_SEALED / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / FALLBACK_OUT_OF_DOMAIN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [V2 evidence](development/physical-sound-realimpact-exact-weld-geometry-preflight-ps2-2026-08-28.md). Manifest `85ca065b…f46f` changes only exact coordinate welding; all welded/reduced topology and 64-mode residual gates pass. Report `c1c86f78…7e5d` and blocks `bcd54087…9acc` / `9310910f…5431` repeat with zero audio. This closes geometry setup only. Next freeze a Pitcher-only calibration manifest before opening its bounded prefix; Planter remains one-shot sealed. All exact-domain, measured transfer, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
