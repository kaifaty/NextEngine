...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `SOURCE_LINE_STOPPED / FCR2_REFERENCE_PASS / FORMULA_RECLOSURE_STOP / FCR3B2_NON_DESCENT / INTEGRATION_BLOCKED`; SPEC-38/ADR-076 `Proposed`; separate `nuv-variational-fcr1` has a slow strict-f64 objective reference but no admissible fast solver/product profile; later ProductChecks remain `NOT_RUN` | W0F/G/H/I roots remain frozen and W3 remains blocked. NPR1-B stopped the source-shaped line; its performance evidence grants no corrected-line credit. The separate [formula-reclosure roadmap](plans/nonlocal-continuum-formula-reclosure/README.md) passes FCR0–FCR2, then localizes corrected SISSM's failure to pressure and rejects the sole fixed Chebyshev remediation at an exact non-descent boundary in [FCR3-B2 evidence](development/nonlocal-continuum-fcr3b2-chebyshev-evidence-2026-08-20.md). FCR3-C through FCR7, profile, CUDA, performance, integration and public contracts are not executed. DFSPH remains the water correctness reference; terrain remains Package 10T. |
