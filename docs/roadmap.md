...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V41](plans/physical-sound-synthesis-roadmap-v41.md) is `ACTIVE / C0_COMPLETE / B0_NEXT / V0_READY / DISCLOSED_LAB_FIRST / PROTECTED_ADMISSION_SOURCE_POWER_OOD / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | The repeat-exact external [C0 corpus](development/physical-sound-v41-c0-disclosed-corpus-result-2026-09-03.md) binds D1 and turns 135 identified recordings plus four REALIMPACT transfers into 278 content-addressed PCM/feature objects, 67 role-disjoint physical parents, complete axis masks and immutable `44 train / 70 development / 25 validator` projections. Extracted modes, damping and transients are explicitly pseudo-targets; absolute force/composition/support remain unobserved. B0 is next and V0 is ready; S0 continues against deficits `13/34` and `13/31`. Only qualified validator, generator holdout and joint shadow may authorize deterministic cooking and the opt-in Steel demo; every failure retains the authored fallback. |
