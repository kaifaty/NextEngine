...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1, PS-2 plan/pilot/`E1` import, bounded internet source/cache/archive/Ogg/RAR5 handling, eight E3 project sources, explicit target/reject-parent roles, five frozen REALIMPACT E2 rows, deterministic split, exact-domain matrix, published-source gate and V1/V2 transfer calibration `IMPLEMENTED`; current status `PS-2_TRANSFER_V2_MODAL_DAMPING_HOLDOUT_PASS / TRANSFER_V1_INVALID / SPATIAL_PARTICIPATION_UNAVAILABLE / EIGHT_EXACT_CLAIMS_OPEN / FALLBACK_OUT_OF_DOMAIN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md), [internet corpus policy](development/physical-sound-internet-corpus-policy-ps2-2026-08-27.md) and [transfer-calibration evidence](development/physical-sound-realimpact-transfer-calibration-ps2-2026-08-28.md). Local capture is retired. Eight E3 projects validate 64 objects/135 recordings; Glass is 16/16 and 46 recordings, reject parents 48/35 and 89. The verified `2/2/2/2` project split and one Blue Bowl bridge remain frozen. Transfer V1 is immutable invalid metric lineage. Injective V2 selects on Shell Plate and crosses every relative modal/damping gate on the fresh Skull Cup holdout, but this is neither glass identity nor perceptual-quality evidence. One listener row per object leaves spatial participation unavailable, and all eight exact-domain blockers remain open. Next acquire typed distinct published listener rows and preregister an object-disjoint spatial discriminator; corpus admission, `Pass`, PS-3, AV-P0D and production P1 remain disabled. |
