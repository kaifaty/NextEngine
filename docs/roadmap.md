...
| Текущая точка | R1–R7 завершены на current Linux product boundary. [R7c Linux performance authority](development/task-state/r7c-linux-performance-authority.md) closed B-12 on exact commit `e48f2eac…`; [R7d final hardening](development/task-state/r7d-final-product-hardening.md) and [R7e distribution closure](development/task-state/r7e-linux-distribution-closure.md) close on exact code commit `919663ff…`. The final native target report is `PASS / release_ready=true` with all eight checks passing, and two independently built PackageManifest V6 trees are byte/mode-identical at manifest `47beaf91…`. R8a condition work is complete; [R8b first learned locomotion](development/task-state/r8b-first-learned-locomotion.md) is the selected post-v1 WIP. ADR-102 biomechanics V3/standing V2 completes one contact-correct GPU episode and five canonical CPU PhysX episodes at the full 3,600-tick bound with zero safety terminal and continuous two-sole CPU contact. The exact 1,024,000-sample run and final checkpoint are hash-closed. [Paired mirror investigation](development/r8b-model-mirror-p1-investigation-2026-09-04.md) now records `MODEL-MIRROR-P1 FAILED`: current joint/root-velocity errors exceed fixed limits and exact action tapes lose the opposite plane before 600 ticks. ADR-102 blocks walking pending an explicit mirror-repair versus `R&D_ONLY` sequencing decision; no runtime authority is promoted. ADR-090 keeps Windows/THOTH/paired evidence outside current scope indefinitely. R141 remains `INVALID / STOP_NO_RETRY` without R142 or authority over R8b. |
...
MODEL_MIRROR_P1_OPEN / NO_RUNTIME_AUTHORITY`. Product priority remains the
...
R8b remains a separately hash-closed lineage, not R142 and not a
...
| PhysX deterministic humanoid substrate | ADR-058/059/062/063/064/065/067/069/071/100/101/102 `Accepted`; immutable Stage 0 V1 and biomechanics V2/standing V1 environments remain negative evidence; biomechanics V3/standing V2 completes its hash-closed 1,024,000-sample run plus one GPU and five canonical CPU full-horizon nominal evaluations; current paired V3 `MODEL-MIRROR-P1` fails fixed joint/root-velocity bounds | Walking is blocked by ADR-102 pending an explicit choice: repair/replace the mirror, or authorize a separately identified `R&D_ONLY` walking discriminator without promotion authority. Runtime promotion remains blocked on the full correspondence/quality matrix. |
...
V3_STANDING_NOMINAL_GATE_PASS / MODEL_MIRROR_P1_OPEN /
...
`b6734160…`; the predeclared final checkpoint reaches one complete contact-
correct GPU episode and five complete canonical CPU episodes with zero safety
terminal and continuous two-sole CPU contact. The next bounded work is paired
V3 `MODEL-MIRROR-P1`, then the distinct forward start/stop environment and run.
The ceiling remains `NO_AUTHORITY`. R8a fracture and embodiment
