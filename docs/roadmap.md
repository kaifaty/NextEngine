...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld REALIMPACT geometry and sealed Pitcher calibration protocol `IMPLEMENTED`; current status `PS-2_PITCHER_CALIBRATION_PROTOCOL_FROZEN / EXACT_WELD_GEOMETRY_SUPPORTED / BYTE_IDENTICAL_PREREGISTRATION / RESERVED_AUDIO_BYTES_ZERO / BOUNDED_CALIBRATION_RUNNER_NEXT / PITCHER_AND_PLANTER_AUDIO_SEALED / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / FALLBACK_OUT_OF_DOMAIN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [calibration preregistration evidence](development/physical-sound-realimpact-pitcher-calibration-preregistration-ps2-2026-08-28.md). Manifest `c60621cc…4a7` and report `2ae1bc0b…9c72` bind the Pitcher geometry, exact 512 MiB prefix, 600-row decoder, mapping, solver, split, controls and stop-before-Planter semantics with zero audio. Next implement parity controls and execute the one-request Pitcher calibration; Planter stays sealed unless a separate immutable holdout manifest is later frozen. All exact-domain, measured transfer, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
