...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V36](plans/physical-sound-synthesis-roadmap-v36.md) is `ACTIVE / R0_A0_F0_COMPLETE / C0_NEXT / FRESH_V36_TARGETS_UNOPENED / V35_QUALITY_UNOBSERVED / REAL_RELEASE_BLOCKED / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V36 F0](development/physical-sound-v36-f0-fresh-role-unchanged-science-result-2026-09-03.md) proves the V35 scientific projection is hash-exact outside eight declared identity paths, while all `1,512 / 15,120` fresh case/row commitments and V32–V35 intersections close twice exactly with zero target/model/prior-value access. C0 must now prove P1 witnesses and local/gate support on every fresh D0/H0 row; X0 and two full-count/full-step D0+H0 surrogate executions must then bind the exact numeric owner before any official capability exists. Synthetic H0 and the independent real source/validator lane must both pass before Steel training; [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker real role six exact-Steel and 27 non-Metal groups short. |
