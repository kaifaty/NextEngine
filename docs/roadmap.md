...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `ACTIVE_ISOLATED_RESEARCH / INTEGRATION_BLOCKED`; SPEC-38/ADR-076 `Proposed`; water `RESEARCH_ONLY`: W1 `LINUX_W1_PASS / CONTINUUM-WATER-REF-P1=PASS` merged at `fe223f9`; current CPU and direct-GPU W2 profiles miss performance; Nonlocal `NR1_BASELINE_MISMATCH / NR1-RC1_SPECIFIED / NR2_BLOCKED / NO_W2_CREDIT`; later ProductChecks `NOT_RUN` | W0F/G/H/I geometry, energy, pressure and reference roots remain frozen. The clean W1 corpus passes, but the eight-worker CPU mean is `269.643 ms` and the optimistic direct GPU proxy p95 is `37.398 ms` `f64` / `18.015 ms` mixed before omitted stages, so W3 integration remains blocked. The [Nonlocal NR1 evidence](development/nonlocal-continuum-nr1-baseline-evidence-2026-08-19.md) passes its independent tiny oracle and fixed water/viscous controls, measures water-48k p95 `13.875 ms`, and blocks NR2 because stiff surface atomics exceed repeated state tolerances from iteration two. [NR1-RC1](plans/nonlocal-continuum/03-nr1-deterministic-accumulation-reclosure.md) now selects a separately identified owner-only directed gather with CPU algebra, tiny CUDA and ten-repeat two-/twenty-iteration surface gates; stable segmented reduction remains a memory-bounded fallback. The `8 ms` cutoff remains research-only and cannot inherit DFSPH roots, change GPU authority or reduce the `4/6 ms` production gate. Independent terrain closure remains Package 10T. Promotion still requires a new explicit solver/profile decision, exact cross-target and exchange identity, one composition DAG/PhysX integration, scheduled epochs, worst-case admission, primary-session fault domain and successor combined budget. |
