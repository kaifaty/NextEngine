...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration and causal audit `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / OBSERVATION_ADMISSION_FAILED / BYTE_IDENTICAL_AUDIT / PLANTER_AUDIO_SEALED / CERAMIC_CUP_OBSERVATION_PROTOCOL_NEXT / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [causal-audit evidence](development/physical-sound-pitcher-causal-audit-ps2-2026-08-28.md). Parent `8bd5323c…1aea` still rejects the combined protocol, but audit `68c79a37…5a57` shows that its observation failed the pre-existing V2 decay gate (`0.4375 < 0.50`) and selected `11/16` peaks below `500 Hz`. Preserve the rejection without uniquely blaming mechanics, keep clip fallback and Planter sealed, then freeze `78_CeramicCup` and run observation admission before physics. All exact-domain, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
