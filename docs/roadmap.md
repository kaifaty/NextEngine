...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, repeated Pitcher/Ceramic rejections and source-derived salience control `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_METHOD_DEVELOPMENT_CLOSED / IRON_SKILLET_METHOD_TRANSFER_REJECTED / FIXED_TAIL_TIMING_MISMATCH_SUPPORTED / SELECTOR_COMPOSITION_MISMATCH_NOT_SUPPORTED / SYNTHETIC_CONTROL_RETAINED / SUBBAND_ESPRIT_SYNTHETIC_CONTROL_NEXT / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [Iron diagnostic](development/physical-sound-realimpact-selector-tail-diagnostic-result-ps2-2026-08-28.md). Repeated report `02551f29…48d1` isolates the fixed 900 ms timing mismatch: source persistence is `0.7059/0.7647/0.6471/0.3529` at `100/200/400/900 ms`, while all-onset adaptive validity passes at `0.7647`. Do not promote an early observed tail. Next prove a synthetic subband common-pole estimator; no new object, retry, mechanics or Planter. |
