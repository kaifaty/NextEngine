...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration/causal audit and Ceramic Cup observation execution/diagnostic `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_ADAPTIVE_COUNTERFACTUAL_REJECTED / CERAMIC_METHOD_DEVELOPMENT_CLOSED / CURRENT_PEAK_SELECTOR_UNSUPPORTED / SALIENCE_SELECTOR_SYNTHETIC_CONTROL_NEXT / INDEPENDENT_OBJECT_REQUIRED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [Ceramic adaptive result](development/physical-sound-ceramic-cup-adaptive-counterfactual-result-ps2-2026-08-28.md). Repeated report `2ec3b03e…d6b7` rejects with only `6/16` valid fits and improvement `0.1875`. Close Ceramic to further method development. Freeze a synthetic source-faithful ±`10%` salience-selector control, then use a preregistered unopened object. Admission, mechanics and Planter remain blocked. |
