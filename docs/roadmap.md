...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V35](plans/physical-sound-synthesis-roadmap-v35.md) is `ACTIVE / R0_F0_COMPLETE / C0_NEXT / FRESH_ROLES_FROZEN_UNOPENED / REAL_RELEASE_BLOCKED / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V35 F0](development/physical-sound-v35-f0-geometry-hybrid-profile-freeze-result-2026-09-02.md) freezes the coverage-gated geometry/contact hybrid twice exactly with `1,859` parameters, nine controls, three ablations, fresh `1,512`-case roles and zero target/model/prior-value/signal access. C0 must now prove P1 witnesses, finite support and both-expert reachability before B0 or any target. Synthetic H0 and the independent real source/validator lane must both pass before Steel training; [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker real role six exact-Steel and 27 non-Metal groups short. |
