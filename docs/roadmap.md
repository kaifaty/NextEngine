...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V40](plans/physical-sound-synthesis-roadmap-v40.md) is `ACTIVE / I0_PROJECT_EXPOSURE_AUDIT_NEXT / DISCLOSED_ML_UNBLOCKED / PROTECTED_FRONTIER_UNESTABLISHED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | The [V40 correction](development/physical-sound-v40-project-independence-rebaseline-2026-09-03.md) preserves V39 F0/F1 as exact historical mechanics but withdraws `6/23` as current admission power: ObjectFolder and the opened YCB revision were credited by the old planner despite prior signal access. I0 must freeze a whole-project exposure/alias ledger and recompute the clean frontier. Disclosed ML development then proceeds on permanently spent projects while metadata-only clean-source discovery continues in parallel. Independent validator qualification, method holdout and joint shadow still gate deterministic cooking and the opt-in Steel demo; every failure retains the authored fallback. |
