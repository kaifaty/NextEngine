...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V43](plans/physical-sound-synthesis-roadmap-v43.md) is `ACTIVE / C1A_COMPLETE / D2_C0R_NEXT / V0_BLOCKED_BY_IDENTITY_AND_POWER / M0_BLOCKED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | The repeat-exact [C1A lineage audit](development/physical-sound-v43-c1a-lineage-audit-result-2026-09-03.md) confirms two physical-parent leaks between current `generator_train` and `validator_calibration`; object `80` also has a `Wood`/`Ceramic` label conflict. D2/C0R immutable role repair is next and must move all 20 derived AV-MSF records out of validator calibration, merge aliases and quarantine the conflicting label. Old B0/R0 remain exact historical controls, but corrected B0R/R0R must re-freeze the global floor and power target. Fresh independent internet sources are required for V0; only later qualified validator, holdout and joint shadow evidence may authorize deterministic cooking and the opt-in Steel demo, while every failure retains the authored fallback. |
