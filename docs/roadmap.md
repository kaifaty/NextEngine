...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V40](plans/physical-sound-synthesis-roadmap-v40.md) is `ACTIVE / I0_COMPLETE_SOURCE_POWER_OOD / D0_NEXT / S0_CLEAN_GROWTH_OPEN / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | The repeat-exact [I0 audit](development/physical-sound-v40-i0-project-exposure-result-2026-09-03.md) accounts all `11/11` candidate projects, permanently discloses nine opened families and quarantines ObjectFolder/YCB from protected power. The clean pool is only `9 exact-Steel / 7 non-Metal` in nine projects; its best protected roles preserve five reserves but miss `13/34` and `13/31`, so S1 remains `SourcePowerOOD`. D0 disclosed ML development and metadata-only S0 clean-source growth may now proceed in parallel. Independent validator qualification, method holdout and joint shadow still gate deterministic cooking and the opt-in Steel demo; every failure retains the authored fallback. |
