...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V12 targets `ACQUISITION_COVERAGE_CERTIFICATE / EXPLICIT_MODAL_TRANSFER / CONSTRAINED_OFFLINE_ML / INDEPENDENT_VALIDATOR / BAKED_CLIP_ATLAS`; current status `A1R_FORCE_ONSET_PASS / V9_REAL_FIT_REJECTED / B1R3_REPEAT_REJECTED / C1_RUNNER_IMPLEMENTED / C1_EVIDENCE_NEXT / ZERO_DECODE_SOURCE_SEARCH_OPEN / REAL_PCM_CLOSED / VALIDATOR_NOT_RELEASED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V12](plans/physical-sound-synthesis-roadmap-v12.md), the [C1 protocol](development/physical-sound-r3a-v12-c1-acquisition-coverage-oracle-protocol-2026-08-31.md) and [B1R3 result](development/physical-sound-r3a-v11-b1r3-local-modal-support-result-2026-08-31.md). B1R3 predicts unseen responses at `0.007180` mean NRMSE and recovers accurate `6/7` locally observable poles, but its force bank has an incidental `6,643 Hz` hole. C1 runner and focused tests are implemented without a numeric modal precheck: evidence must now reproduce the force-only `200…9,500 Hz` certificate, all five leave-one-profile-out fits and acquisition/query OOD separation before holdout can open. Real PCM, ML, validator release and runtime stay closed; production requires a concrete consumer and ADR-046 gate. |
