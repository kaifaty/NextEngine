...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V41](plans/physical-sound-synthesis-roadmap-v41.md) is `ACTIVE / D1_ALIAS_REPAIR_COMPLETE / C0_NEXT / DISCLOSED_LAB_FIRST / PROTECTED_ADMISSION_SOURCE_POWER_OOD / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | The zero-signal [D1 repair](development/physical-sound-v41-d1-alias-component-roster-result-2026-09-03.md) supersedes only D0's role projection after detecting that REALIMPACT `6_Bowl` and ObjectFolder object `6` are one Blue Bowl across generator/validator roles. D1 co-locates both families in generator-train, moves independent AV-MSF to validator-calibration and preserves component-disjoint `5/2/2`, zero clean/protected access and immutable D0 evidence. C0 is next; metadata-only S0 continues against deficits `13/34` and `13/31`. Only qualified validator, generator holdout and joint shadow may authorize deterministic cooking and the opt-in Steel demo; every failure retains the authored fallback. |
