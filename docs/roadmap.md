...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration/causal audit and Ceramic Cup observation preflight `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / OBSERVATION_ADMISSION_FAILED / CERAMIC_CUP_DISCOVERY_VERIFIED / OBSERVATION_PROTOCOL_FROZEN / THREE_REQUESTS_AUTHORIZED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [Ceramic Cup observation evidence](development/physical-sound-realimpact-ceramic-cup-observation-preflight-ps2-2026-08-28.md). Parent `8bd5323c…1aea` still rejects the combined protocol; audit `68c79a37…5a57` shows upstream observation failure. Discovery `2b183dae…782b` / `cd68ba79…1b0b` and manifest `71123b21…cae5` now authorize exactly two metadata ranges and one fixed `512 MiB` prefix for `78_CeramicCup`. Execute once and run unchanged V2 observation admission before physics; keep clip fallback and Planter sealed. All exact-domain, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
