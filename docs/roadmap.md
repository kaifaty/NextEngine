...
| Physical sound source synthesis | SPEC-45 `Proposed`; V2 roadmap strategy `NEURAL_TRANSFER_FIELD / OFFLINE_TRAINING / DETERMINISTIC_COOKED_RUNTIME`; Q30/DCT, FEM/BEM and previous real-data work remain baselines, controls and negative knowledge; current status `R0_COMPLETE / R1_TRANSFER_CONTROLS_FROZEN / R2_NEXT / NO_TRAINED_MODEL / ADMISSION_SHADOW_SEALED / SELECTIVE_PASS_DISABLED / AUTHORED_CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V2](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). The repeated real projection separates recorded-waveform and force-deconvolved-transfer semantics; the Green Goblet fixed-impact listener split now has frozen nearest/linear transfer controls and preregistered primary metrics. Next train only the R2 fixed-impact listener field before attempting full impact/listener few-shot or shared-object transfer. The independent validator and admission shadow remain isolated; production can consume only a bounded cooked record after a future consumer/ADR gate. |
