...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V16 status `L0_TRUTH_ORACLE_NEXT / SOURCE_GROWTH_PARALLEL / ADMISSION_BLOCKED / METAL_FIRST / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | [V16](plans/physical-sound-synthesis-roadmap-v16.md) separates known-truth capability, disclosed real candidate quality and fresh protected admission. L0 can now falsify the structured modal-field hypothesis without waiting for source growth. [S0c](development/physical-sound-v15-s0c-source-sufficiency-role-freeze-result-2026-09-01.md) still blocks real admission: all materials have zero eligible role assignments under the unchanged `4/1/1/1/1` shape. Exposure-role recovery and published-source adapters proceed independently; only a passing truth/real tournament plus a frozen protected Metal split may open validator release, one-shot shadow, deterministic cooker and demo. Raw-force transfer, runtime inference and production promotion remain separate blocked claims. |
