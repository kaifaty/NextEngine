...
| Physical sound source synthesis | SPEC-45 `Proposed`; V4 strategy `GEOMETRY_AWARE_MODAL_CONTACT_FIELD / OFFLINE_TRAINING / DETERMINISTIC_COOKED_RUNTIME`; current status `R0_R1_COMPLETE / R2_LISTENER_FIELD_REJECTED / R3A_NEW_DATA_REPRESENTATION_NEXT / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V4](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). R2E fits context exactly but loses every grouped held-listener endpoint; a repeated post-reject projection oracle also exposes a rank-96 query representation ceiling. The opened listener split is retired. Next audit and freeze a new internet-only multi-object/multi-impact canonical-listener corpus, then pass a compact modal/residual representation oracle before training one geometry-aware exact-object contact field. The validator and admission shadow remain isolated; production still requires a future consumer/ADR gate. |
