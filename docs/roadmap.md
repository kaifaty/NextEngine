...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1, PS-2 plan/pilot/`E1` import, bounded internet source/cache/archive/Ogg/RAR5 handling, eight E3 project sources (AV-MSF, YCB, Heller, two Freesound, ObjectFolder-Real-demo, Kronland and SoundPacks), explicit target/reject-parent roles, five frozen REALIMPACT E2 rows and deterministic split-feasibility audit `IMPLEMENTED`; current status `PS-2_AGGREGATE_E3_COVERAGE_COMPLETE / GLASS_16_OF_16 / REJECT_PARENTS_38_OF_35 / PROJECT_SPLIT_INFEASIBLE_3_OF_4_REJECT_PROJECTS / FIVE_REALIMPACT_E2_OBJECTS / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md), [internet corpus policy](development/physical-sound-internet-corpus-policy-ps2-2026-08-27.md), prior linked E3/E2 evidence and the [SoundPacks Glass E3 plus split audit](development/physical-sound-soundpacks-glass-e3-and-split-audit-ps2-2026-08-28.md). Local capture is retired. Two fresh online source caches, an offline replay and two complete combined audits repeat byte-identically. Eight E3 projects validate 54 objects/125 recordings; Glass reaches 16/16 and 46 recordings, while explicit roles identify 38/35 non-Glass parents and 79 recordings. Aggregate count minimums are complete, but only three projects carry reject parents for four required `dev/calibration/holdout/shadow` partitions, so the exact split decision is `ProjectDisjointSplitInfeasible` and all entries remain `dev`. The next PS-2 increment is at least one independent reject-bearing project, followed by the same gate; target-only Glass cannot remove the blocker. Freeze no split and start no PS-3/AV-P0D work before feasibility and separately hash-closed partition evidence. Five REALIMPACT rows remain fallback-only, bounded rejected source routes remain rejected, R&D stays PresentationOnly and production P1 gates are unchanged. |
