...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V15 status `S0A_COMPLETE / S0B_YCB_CAPABILITY_NEXT / METAL_FIRST / WOOD_PENDING / GLASS_FALLBACK_ONLY_SOURCE_GROWTH / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | [S0a](development/physical-sound-v15-s0a-revision-aware-identity-exposure-result-2026-09-01.md) resolves 100 historical + 100 current ObjectFolder rows and 50 RealImpact names into 130 physical groups, preserves 30 revision conflicts and carries numeric/path/exact-name exposure without signal access. Repeat-exact fresh metadata groups are now Glass/Wood/Metal `2/0/12`, so Metal retains the unchanged eight-group potential while S0b builds the YCB metadata/cost capability certificate. Raw-force transfer, runtime inference and production promotion remain separate blocked claims. |
