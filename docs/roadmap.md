...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C and PS-1 amplitude-envelope consensus `IMPLEMENTED`; current status `PS-1_COMPLETE / PS-2_NEXT / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md): PS-1 closes the frozen shuffled-envelope blind spot on the controlled pack, but three parent groups per split leave the Wilson upper false-pass risk at `0.5615`. Broaden exact-domain real families and pre-register grouped risk/coverage in PS-2 before AV-P0D. R&D remains PresentationOnly and non-blocking for R7. Production impact waits for an explicit post-v1 slot, concrete consumer, Accepted promoting ADR, complete engine-owned contact projection, cooked acoustic content/fallback and exact PCM/root checks; rolling/scraping remain later. |
