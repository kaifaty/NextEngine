...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V44](plans/physical-sound-synthesis-roadmap-v44.md) is `ACTIVE / B0R_R0R_COMPLETE / CORRECTED_CORPUS_SIGNAL_INSUFFICIENT / C1_NEXT / V0S_OPEN / V0_BLOCKED_BY_POWER / FIRST_PACK_SELECTION_PENDING / TRAINING_BLOCKED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | Repeat-exact [B0R/R0R](development/physical-sound-v44-b0r-r0r-corrected-baseline-result-2026-09-03.md) on the corrected 65-parent corpus freezes global median `1.312187716`, finds material transfer `-0.098113339`, strong project identity and a `49`-parent planning deficit. C1 runtime-descriptor contract/source growth is next while metadata-first V0S grows independent validator sources. The first demo material is selected signal-blind from source power and descriptor coverage rather than fixed as Steel; only B1, frozen V0, one-shot protected admission and deterministic cooking can authorize an opt-in prop, and every failure retains the authored fallback. |
