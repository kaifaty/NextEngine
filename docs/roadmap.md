...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `PLANNED / NOT_ACTIVE`; SPEC-38/ADR-076 `Proposed`; original water W0B `INVALIDATED_BY_RC1`, W0C `IN_PROGRESS / GHOST_CANDIDATE_REJECTED`, W1 `IMPLEMENTED / BLOCKED_ON_W0C`; all ProductChecks `NOT_RUN` | Exact W0C diagnostics reject a ceiling-only repair: the unchanged profile still misses the threshold at iteration 320 and proposes boundary escape. `ghost-cell-shell-v1` fixes the initial partition and passes step 1/free-fall, but fails the hydro soak on step 2; ceilings 100/160 only defer failure. W0C now evaluates deterministic pre-equilibrated initialization; do not start W2 or integration. Main-roadmap activation still requires `CONTINUUM-WATER-REF-P1 = PASS`; promotion also requires exact exchange identity, one composition DAG/PhysX integration, scheduled epochs, worst-case admission, primary-session fault domain and successor combined budget. |
