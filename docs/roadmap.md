...
| R8. Post-v1 tracks | `IN_PROGRESS / R8B_FIRST_LEARNED_LOCOMOTION_SELECTED / WALKING_ACTION_BASIS_AUDIT_REQUIRED` | отдельные программы | R8a closes the bounded intact-topology condition/capability subset. R8b standing passes; walking V1/V2/V3 are rejected for no meaningful canonical travel. Further walking optimizer work is stopped pending a translation-invariant CPU/Isaac action-basis audit, then a separately isolated gait-credit discriminator. Other optional tracks remain gated. |
...
Further optimizer work is stopped until a walking-only translation-invariant
reference passes a deterministic paired CPU/Isaac alternating-support audit.
Only then may a separate discriminator replace the binary one-tick support
predicate with duration/alternation-aware gait credit. Mirror dynamics remain
...
3. no-training walking action-basis audit: a walking-only,
   translation-invariant reference changes no body, PD/safety, command, reward
   or optimizer fact; deterministic paired CPU/Isaac tapes must demonstrate
...
| PhysX deterministic humanoid substrate | ADR-058/059/062/063/064/065/067/069/071/100/101/102/103/104/105 `Accepted`; biomechanics V3/standing V2 passes its nominal GPU/CPU standing gate; walking V1/V2/V3 are rejected; paired V3 `MODEL-MIRROR-P1` fails fixed joint/root-velocity bounds; post-failure audit isolates an origin-anchored walking action base with no demonstrated single-support reachability | Stop optimizer work. First run the translation-invariant paired action-basis audit; only after it passes isolate duration/alternation-aware gait credit. Broader curriculum and runtime promotion remain blocked until the full CPU quality/correspondence matrix passes. |
...
Next is the translation-invariant paired action-basis audit, not another
optimizer run. The ceiling remains `NO_AUTHORITY`, and mirror repair remains
