...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V27](plans/physical-sound-synthesis-roadmap-v27.md) status `ADOPTED / R0_PASS / R1_A_RESOURCE_TIMEOUT / NO_CANONICAL_OUTPUT / RUN_B_NOT_STARTED / QUALITY_UNOBSERVED / GENERATOR_BRANCH_CLOSED / Q0_SIGNAL_BLIND_NEXT`; no public schema, content role, runtime model or production gate promoted | [R1-A](development/physical-sound-v27-r1-official-feasibility-result-2026-09-02.md) remained CPU-active until the frozen `1,800 s` limit and published no weights, predictions, reports or decision; B did not run and M0b is spent. Continue only signal-blind validator/source inventory while a fresh value-independent resource hypothesis is researched. R2, Metal admission, cooker/demo and Glass/Wood remain blocked; authored fallback stays authoritative. |
