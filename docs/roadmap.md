...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V46](plans/physical-sound-synthesis-roadmap-v46.md) is `ACTIVE / EXECUTION_ROADMAP / FOUNDATION_D0_COMPLETE / NO_TRUSTED_EXTERNAL_SYNTHETIC_TEACHER / D1_CORPUS_COMPILER_NEXT / RESEARCH_ONLY / OFFLINE_ML / AUTHORED_FALLBACK`; no public schema, runtime model or production gate promoted | V46 keeps V45's repeat-exact source ledger, `189`-coordinate Recipe V3 and 36-group Clatter comparator, plus the V44 real baseline `1.312187716` and `71/105` supported real parents. [D0](development/physical-sound-v46-d0-synthetic-source-preflight-result-2026-09-03.md) repeats exactly and rejects NISR (`DeclaredGenerationDocumentUnavailable`) and VibraVerse (`GenerationLineageIncomplete`) before any NPZ/tar/audio/modal-value access; neither source may supervise the model. D1 now builds the external corpus compiler with an explicit zero-row external synthetic lane, after which B0 compares analytic and Clatter controls. Independent real-evidence/validator growth (E0/PSEL/B1/V0) continues; only one disclosed LabWinner may open one-shot A0 admission, and only A0 Pass may open deterministic K0 cooking plus an opt-in fallback-safe P0 demo. |
