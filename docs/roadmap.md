...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, repeated real-method rejections and source-derived/Gabor common-pole synthetic controls `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_METHOD_DEVELOPMENT_CLOSED / IRON_SKILLET_METHOD_TRANSFER_REJECTED / FIXED_TAIL_TIMING_MISMATCH_SUPPORTED / SUBBAND_COMMON_POLE_CORE_SUPPORTED / BROADBAND_DISCOVERY_CONTROL_NEXT / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [common-pole result](development/physical-sound-subband-common-pole-control-result-ps2-2026-08-28.md). Repeated synthetic report `3fcb5fc8…2284f` recovers six common poles and the node-channel counterexample with max frequency/decay errors `0.043 Hz/0.259/s`, but uses declared Gabor bands. Next prove input-driven band discovery, duplicate pruning, amplitude fit and reconstruction on new synthetic truth; no real object/reuse, retry, mechanics or Planter. |
