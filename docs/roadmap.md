...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V45](plans/physical-sound-synthesis-roadmap-v45.md) is `ACTIVE / MULTI_FIDELITY_REBASELINE / R0_SOURCE_CLAIM_LEDGER_NEXT / 71_REAL_ACOUSTIC_PARENTS / DEFICIT_34 / CLATTER_CONTROL_ONLY / NISR_SYNTHETIC_ONLY / STRUCTURAL_TRANSFER_OPEN / V0S_OPEN / PSEL_BLOCKED / REAL_TRAINING_BLOCKED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, runtime model or production gate promoted | [V45 research](development/physical-sound-v45-multifidelity-evidence-rebaseline-2026-09-03.md) finds no new all-in-one public descriptor/force/microphone corpus at sufficient independent-parent power, but identifies complementary evidence lanes: Clatter as an external empirical-modal control, NISR as an ObjectFolder-derived synthetic modal teacher, TU Delft/cello measurements as structural-transfer controls, and CMU/three-object recordings as prospective acoustic/validator sources. Each source now receives an explicit claim mask; synthetic and structural rows grant zero real-acoustic parent or validator credit. The V44 real baseline remains `1.312187716`; G0B1c1 remains `71/105`, deficit `34`, and one IETeasy project cannot close PSEL. R0/T0 and bounded teacher controls may proceed while second-project growth continues, but real fitting still waits for PSEL/B1 and a frozen independent validator; only one-shot protected admission plus deterministic cooking may authorize an opt-in prop, and every failure retains the authored fallback. |
