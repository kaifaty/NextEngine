...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, sealed Pitcher calibration protocol and hash-closed execution preflight `IMPLEMENTED`; current status `PS-2_PITCHER_EXECUTION_PREFLIGHT_SUPPORTED / ONE_PREFIX_REQUEST_AUTHORIZED / RESERVED_AUDIO_BYTES_ZERO / PITCHER_ACQUIRE_NEXT / PLANTER_AUDIO_SEALED / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / FALLBACK_OUT_OF_DOMAIN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [execution evidence](development/physical-sound-realimpact-pitcher-execution-preflight-ps2-2026-08-28.md). Manifest `8e791327…ba45` and preflight `94d5e1e6…9b32` repeat; environment, original bbox origin, directions, Bempp/cooker, exact range/decoder and gates are closed with zero audio access. Next make the single 512 MiB Pitcher request without retry or growth. Planter stays sealed. All exact-domain, measured transfer, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
