...
| Animation/motor | Procedural projection, deterministic 23-DoF standing motor и canonical `humanoid-flat-command.v1` CPU environment реализованы: engine command schedule, fixed PD/safety, 240/60 scheduling, partial reset, terminal lifecycle, checkpoint/replay, motor-lab v2 и внешний NPZ v2 recorder. Isaac mirror v2 имеет exact CPU golden и correspondence gates. | Нет skeleton graph, retargeting, IK, root-motion intent, trained/learned evaluator, выполненного Isaac GPU correspondence, Linux parity и полного Stage 0/hard-performance gate set. |
...
cleanup и R3 завершены; следующий WIP=1 — R4.
...
Lab/Sim, GPU correspondence и Linux отсутствовали (`NOT_RUN`). Подробная
матрица evidence и remaining gates находится в
