...
- R8d water mechanics: the exact flow network (two vessels, a gated pipe,
  ...
  per tick and the flow command (`CONTINUUM-WATER-FLOW-P1 = PASS`); WP1
  presentation stage in the game root (surfaces on exact levels, flux
  ripple, gate jet through the ADR-101/102 paths,
  `CONTINUUM-WATER-PRESENT-P1 = PASS`); WR1 exact free-body boxes and WB1
  buoyancy batch (`CONTINUUM-WATER-BUOYANCY-P1 = PASS`): the four ADR-104
  water checks pass, the promotion decision is next, the particle lanes
  stay research;
...
|---|---|---|
| Contracts/runtime/ledger | Реализован фундамент и current `core_r8c` nine-command/four-system registry/schedule/profile closure (R5c eight kinds plus the ADR-100 water level command); R5c root-motion ingress сохраняет прежние 12 stages и проходит common archive/receipt/physical admission | Расширять только вместе с реальным gameplay use case; не строить второй runtime framework. |
...
| RPG | Частично: тринадцать aggregate payload kinds и одиннадцать операций, включая bounded commitment/atomic systemic settlement and ADR-098 BodyCondition impairment/treatment | Нет fracture/topology, полного faction/membership, broad status/effect, quest-graph, reward, debt/market или divine command lifecycle. |
| Mechanics/packages | Частично: contact melee, governed Luau/Wasm examples and the player/NPC BodyCondition compiler use the same capability/effect/command validators | Нет общего ability phase/cost/cooldown/status lifecycle or public arbitrary injury reducer; project-local breadth requires a concrete future consumer. |
| Content/cooker | Data-first `projects/reference-alpha` проходит typed V7/V8 authoring/cook/direct-lock activation; package содержит 125 canonical entries, 39 roots и 64 chunk bindings. Independent `projects/creator-smoke` uses only file-backed public authoring data, deterministically cooks/activates 18 entries, 16 roots and three chunks, runs through the generic application path and builds an exact current Creator Project Package V1 with required NOTICE. The cold Creator SDK workflow creates, edits, validates, cooks and packages another namespace through public commands. | Нет broad animation graph/retarget content or polygon navmesh; further templates/inspectors/scenarios require concrete consumers. |
...
| Physics | PhysX 5.9.0 остаётся выбранным production backend; current procedural reference profile имеет upright capsule, at most one fixed-local carried box, static/dynamic Box, sensors, exact contacts/`ClosestPoint` and checkpoint reconstruction. R5j proves trip/carry/melee through the production catalog without a second body owner. R8c adds the ADR-100 authoritative water table (one reference basin) as field 4 of the physics checkpoint with an exact submersion query and a level command; `CONTINUUM-WATER-VOLUME-P1 = PASS`. R8d adds the ADR-103 water flow network (two reference vessels, a gated pipe, a source and a sink) as field 5 of the physics checkpoint (schema 3) with an exact per-tick step and the flow command kind; `CONTINUUM-WATER-FLOW-P1 = PASS`. | General attachment/drop, mass transfer, active articulation cutover, broader shape/query consumers and optional learned Stage 0 promotion evidence remain post-baseline. |
| Animation/motor | Completed R5a–R5j procedural baseline remains current. R8a additionally binds one optional unilateral anatomy profile to the existing `BodySchemaV1`, derives revision-bound directional capacity from RPG BodyCondition and clamps positive left-knee fixed-PD effort for identical player/NPC paths without changing topology, tensor layout or default no-envelope behavior. | Stable/retained fracture, ordinary limp/fall/crawl adaptation, UI/severity surfaces, creator-authored/multi-profile injury LOD, active articulation and learned injury routes remain later R8 increments. |
...
- R7d and R7e close on exact code commit `919663ff…`: the physical Vulkan
  platform row and all eight native-gate checks pass, `release_ready=true`, and
  two complete V6 package trees are byte/mode-identical at manifest
  `47beaf91…`.
...
beta and R7 are complete. Exact code commit `919663ff…` passes the physical
Vulkan platform row and the complete eight-check Linux native gate with
`release_ready=true`; reproducible PackageManifest V6 A/B trees close the
...
**Статус:** `COMPLETE / LINUX_ONLY / V1.0.0_PUBLISHED`.
...
4. **R7d — final product hardening (`COMPLETE / FINAL_LINUX_PASS`):** the
   retained application, verification, gameplay, replay, audio and 3,600-tick
   soak matrix remains green. Exact code commit `919663ff…` additionally passes
   the physical Vulkan `platform` row and complete native acceptance without
   an open release-blocking defect.
5. **R7e — distribution closure (`COMPLETE / V6_REPRODUCIBLE_PASS`):** ADR-097
   and strict PackageManifest V6 bind release 1.0.0, notices/provenance, 44
   selected dependency records, 105 copied license files, packaged guides,
   portable release paths and a passing protected-data receipt. Independent
   266-file package trees A/B are byte/mode-identical at manifest
   `47beaf91…`; exact code commit `919663ff…` passes all eight native checks
   with target-report `144c3ab2…` and `release_ready=true`.
6. **Publication (`COMPLETE / GITHUB_V1.0.0`):** annotated tag and release
   `v1.0.0` resolve to exact validated commit `919663ff…`; the release carries
   the 11,366,177-byte Linux archive plus canonical package manifest and target
   report. Archive SHA-256 is `c9cda739…1461d`.
...
**Hard blocker status:** `NONE OBSERVED`. The representative slice, save/replay,
physical display, installation, provenance and protected-data boundaries all
pass on the exact release candidate. Any future P0/P1 data-loss, security,
determinism, offline or installation defect reopens release readiness as a new
observed regression, not as unfinished R7 ceremony.
...
ten transitions and exact intact/partial/zero/recovered effort matrix. Stable
fracture/topology, ordinary limp/fall/crawl, UI/visual severity and workload LOD
...
**R8c authoritative water volume and presentation water** remains active in
parallel; its current implementation and next increment are recorded in
[water-volume-authority.md](development/task-state/water-volume-authority.md).
The water branch's recorded water-volume, water-flow, water-present and
water-buoyancy results retain their original evidence scope; this merge does
not grant new ProductCheck or release credit.
...
MODEL_MIRROR_P1_FAILED / NO_RUNTIME_AUTHORITY`. Product priority remains the
...
colliders and 23 collinear X-axis hinges with identical symmetric ROM; it is
not an acceptable learned-humanoid foundation. The admitted successor uses
biomechanics BodySchema V3 and material-complete compiler. Pair-complete GPU
contact classification and the canonical CPU consumer reject both V2-body
checkpoints; the final fresh policy self-collides at ticks `139/135`. ADR-102
raises neutral pelvis/forearm clearance from `405` to `45,405 um` and improves
the serial-carrier solver projection without changing anatomical joint axes or
source aggregate mass/inertia. Its fresh 1,024,000-sample run closes on commit
...
ADR-104's V2 run also fails: five GPU horizons achieve only `0.123/7.258 m`,
and no saved CPU checkpoint walks. ADR-105's dense V3 run consumes its exact
budget and also fails: five GPU horizons achieve only `0.104/7.258 m`, while
canonical CPU falls at tick `156` in all five episodes and moves `-0.822 m`.
The post-failure audit records a `600/600`-tick double-support GPU trace, no
single-support state in `128` fixed/random or `56` periodic CPU action probes,
and a procedural standing ankle reference whose absolute root-forward term
progressively removes walking action authority over the `7.258 m` command.
ADR-106 now implements the translation-invariant V4 reference and V5's
BodySchema V4 leg-proxy clearance plus fourfold residual. Canonical CPU safely
reaches left/right single support for 31/34 ticks in a 105-tick probe. Isaac GPU
...
The direct canonical run has now completed all 1,024,000 samples and failed:
all five final episodes travel 0.824657 m then fall at tick 299, without a
single-support tick. The repaired adapter is exact against 5,120 native
transitions/399 resets and Linux host-check passes. Noise counterfactuals do
not produce foot release; the next step is a safe weight-transfer/lift/place
lesson and isolated gait credit, not an unchanged optimizer retry.
...
1. no-training preflight: exact standing/flat-command BodySchema, environment,
   ...
   output exists, freeze the V2 walking schedule whose command-side closure
   includes at least `3 m` cumulative forward target and a final exact
   zero-command interval of `180` motor ticks;
...
3. no-training walking action-basis audit: a walking-only,
   translation-invariant V4 reference is isolated first; ADR-106's V5 then
   adds a separately identified leg-proxy/range diagnostic with unchanged
...
4. learned forward start/stop checkpoint: ADR-107 permits one direct canonical
   CPU V5 discriminator and ADR-108 its diagnosed V6 periodic-credit successor
   after exact adapter controls, without claiming the
...
5. only after both checkpoints may a later package consider turns, broader
...
| PhysX deterministic humanoid substrate | ADR-058/059/062/063/064/065/067/069/071/100/101/102/103/104/105/106/107 `Accepted`; V3 nominal standing passes; V1/V2/V3 walking and the direct canonical V5 run are rejected; native adapter/reset control and Linux host-check pass; Isaac correspondence still fails | Localize stance-ankle/return safety and isolate weight-transfer/lift/place curriculum. Final V5 falls at 299 after 0.824657 m without single support. No optimizer is active; original gait and MODEL-MIRROR gates remain open. |
...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V46](plans/physical-sound-synthesis-roadmap-v46.md) is `ACTIVE / EXECUTION_ROADMAP / B0_COMPLETE / NO_USEFUL_TEACHER / E0_V0_NEXT / M0_BLOCKED / RESEARCH_ONLY / OFFLINE_ML / AUTHORED_FALLBACK`; no public schema, runtime model or production gate promoted | V46 keeps the repeat-exact source ledger, `189`-coordinate Recipe V3, 36-group Clatter comparator and `71/105` supported real parents. [D0](development/physical-sound-v46-d0-synthetic-source-preflight-result-2026-09-03.md) rejects NISR/VibraVerse before payload; [D1](development/physical-sound-v46-d1-corpus-compiler-result-2026-09-03.md) compiles 154 real rows without content access. [B0](development/physical-sound-v46-b0-lane-aware-control-tournament-result-2026-09-03.md) then repeat-exactly reproduces the C0R global floor `1.312187716`, performs zero cross-contract comparisons and returns `NoUsefulTeacher`: Clatter/analytic remain controls, IETeasy is train-only, and trusted synthetic/structural rows are zero. M0 stays closed. E0/V0 internet-source growth is next; only one disclosed LabWinner may later open one-shot A0, deterministic K0 and an opt-in fallback-safe P0 demo. |
...
Water passed its serial CPU oracle (`CONTINUUM-WATER-REF-P1 = PASS`) but the
CPU lane is `67x` over budget; under ADR-100 (Accepted 2026-09-03) the integration track
becomes an exact CPU `WaterVolume` for gameplay plus presentation-only surface
dynamics (GPU or otherwise) that never feed back. Terrain follows its
...
authority, and no GPU state is gameplay authority.
...
dormant historical context, not active debt. R1/B-01 and R7 are closed on the
...
The selected post-v1 queue contains two active packages: R8b below and R8c water.
**R8b first learned locomotion (`ACTIVE_R&D /
V3_STANDING_NOMINAL_GATE_PASS / R&D_ONLY_WALKING_V1_V2_V3_FAILED /
WALKING_ACTION_BASIS_AUDIT_REQUIRED / MODEL_MIRROR_P1_FAILED /
NO_RUNTIME_AUTHORITY`)**. Its durable
resume surface is
...
controls. ADR-100's bounded Q16 objective fixes the catastrophic reward scale
and the second run completes all `4,096,000` transitions, but deterministic
seed-1001 policies still fall at ticks `100/97`. Frame-by-frame review and the
exact descriptor/USD/source then expose the more fundamental constraint: all
24 physical colliders are spheres and all 23 revolute joints share axis X and
the same symmetric `±1.5 rad` ROM. The viewer adds cosmetic links, but does not
invent this physics. Further optimizer, noise or checkpoint tuning on the
Stage 0 V1 body is stopped. Pair-complete contact classification and the
canonical CPU consumer then reject both biomechanics V2/standing V1 policies:
the fresh contact-correct run learns to `143.61` final mean episode ticks, but
its final policy self-collides at GPU tick `139` and CPU tick `135`, while the
neutral body has only `405 um` pelvis/forearm clearance. ADR-102 selects
biomechanics V3/standing V2 with `45,405 um` clearance and a source-conserving
...
is rejected. ADR-104 V2 also survives on GPU without walking and has no passing
CPU checkpoint. ADR-105 dense V3 likewise survives five GPU horizons while
travelling only `0.104/7.258 m`; canonical CPU falls at tick `156` in all five
episodes. The completed post-failure research isolates the origin-anchored
...
18. **R7d final product hardening (`COMPLETE / FINAL_LINUX_PASS`):** the green
   application/replay/audio/long-session matrix and final physical Vulkan
   desktop row close on the current Linux candidate.
19. **R7e Linux distribution closure (`COMPLETE / V6_REPRODUCIBLE_PASS`):** two
   exact 266-file V6 package trees are byte/mode-identical; final exact-commit
   native acceptance passes all eight checks with `release_ready=true`.
...
complete. R7 is complete under ADR-090: R7a–R7e are closed on the current Linux
product boundary, including physical display, reproducible V6 A/B packages and
final exact-commit native acceptance.
