...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V24](plans/physical-sound-synthesis-roadmap-v24.md) status `ADOPTED / V23_BOUNDED_CLOSEOUT / DATA_FIRST_ML / INTERNET_ONLY_EVIDENCE / AUTOMATED_VALIDATOR_REQUIRED / CLIP_FALLBACK / P3_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Finish the frozen [V23 P2a](development/physical-sound-v23-p2a-fixed-feature-ridge-protocol-2026-09-01.md) once as the deterministic baseline, then shift the critical path to one hash-closed synthetic-plus-internet corpus, an exact-object contact-conditioned neural student, an independently frozen automatic validator, protected Metal admission and deterministic clip cooking for one demo prop. Glass and Wood repeat admission independently; no user recording or per-sound human approval is required. |
