...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V37](plans/physical-sound-synthesis-roadmap-v37.md) is `ACTIVE / R0_COMPLETE / N0_NEXT / V36_D0_SEALED_AND_UNOPENED / ML_FIRST / INTERNET_DATA_ONLY / AUTOMATIC_VALIDATION / OFFLINE_COOKING / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | V37 preserves the exact V36 D0/H0 experiment as its nearest gate, then separates internet corpus, physics-locked neural generator, independent validator, one-shot admission, deterministic cooker and opt-in demo. The official D0 provider/runner are committed and fresh values remain unopened. Synthetic H0 plus the independent real source/validator lane must pass before real generator training; [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker source role six exact-Steel and 27 non-Metal groups short. |
