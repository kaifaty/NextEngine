...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration/causal audit and Ceramic Cup observation execution `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_CUP_OBSERVATION_REJECTED / REPEATED_DECAY_GATE_FAILURE / OFFLINE_CAUSAL_DIAGNOSTIC_FROZEN / TWENTY_SEVEN_ROWS_AUTHORIZED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [diagnostic preflight](development/physical-sound-ceramic-cup-observation-diagnostic-preflight-ps2-2026-08-28.md). Repeated observation failure blocks another object and mechanics. Manifest `e7b952fe…375b` and preflight `6e97bc0a…066c` now bind 27 already decoded height/angle/distance rows, unchanged V2 gates and diagnostic classification. Execute twice offline; do not select a replacement row, tune, denoise, fetch, run physics or open Planter. All exact-domain, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
