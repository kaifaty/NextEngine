...
| Текущая точка | R1–R7 завершены на current Linux product boundary. [R7c Linux performance authority](development/task-state/r7c-linux-performance-authority.md) closed B-12 on exact commit `e48f2eac…`; [R7d final hardening](development/task-state/r7d-final-product-hardening.md) and [R7e distribution closure](development/task-state/r7e-linux-distribution-closure.md) close on exact code commit `919663ff…`. The final native target report is `PASS / release_ready=true` with all eight checks passing, and two independently built PackageManifest V6 trees are byte/mode-identical at manifest `47beaf91…`. R8a condition work is complete; [R8b first learned locomotion](development/task-state/r8b-first-learned-locomotion.md) is the selected post-v1 WIP. Its bounded-reward run validates the new numerical scale, but a frame and exact-schema audit retires the frozen Stage 0 V1 body from learned-humanoid optimization: it is a 24-sphere, 23-collinear-X-hinge toy substrate. The next prerequisite is to close neutral-sole contact on the existing biomechanics V2 design and admit a new command-only standing generation; no further V1-body run is authorized. ADR-090 keeps Windows/THOTH/paired evidence outside current scope indefinitely. R141 remains `INVALID / STOP_NO_RETRY` without R142 or authority over R8b. |
...
BIOMECHANICS_SUCCESSOR_REQUIRED / NO_AUTHORITY`. Product priority remains the
...
not an acceptable learned-humanoid foundation. R8b must use an admitted
successor of the existing biomechanics BodySchema V2, after its neutral-sole
contact failure is closed. R8b remains a separately hash-closed lineage, not R142 and not a
...
| PhysX deterministic humanoid substrate | ADR-058/059/062/063/064/065/067/069/100 `Accepted`; immutable Stage 0 V1 environments remain exact toy/regression evidence; biomechanics V2 exists but its neutral-sole contact test fails; Stage 0 product evidence remains incomplete | Close the biomechanics neutral-pose contact invariant and create a distinct hash-closed command-only standing identity before further optimization. CPU PhysX remains final evidence authority; Isaac may accelerate only after exact successor descriptor/golden and reset/reward preflight. |
...
**R8b first learned locomotion (`ACTIVE_R&D / STAGE0_V1_BODY_RETIRED /
BIOMECHANICS_SUCCESSOR_REQUIRED / NO_AUTHORITY`)**. Its durable
resume surface is
...
Stage 0 V1 body is stopped. The next bounded work is to reproduce and close the
existing biomechanics V2 neutral-sole contact failure, admit a distinct
command-only standing profile with the bounded reward, and pass CPU
reset/reward/zero-action preflight before new GPU compute. The ceiling remains
`NO_AUTHORITY`; `MODEL-MIRROR-P1` is also `NOT_RUN`. R8a fracture and embodiment
