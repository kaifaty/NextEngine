...
| Physical sound source synthesis | SPEC-45 `Proposed`; V3 roadmap strategy `SELF_VALIDATING_NEURAL_TRANSFER_FIELD / OFFLINE_TRAINING / DETERMINISTIC_COOKED_RUNTIME`; Q30/DCT, FEM/BEM and previous real-data work remain baselines, controls and negative knowledge; current status `R0_COMPLETE / R1_TRANSFER_CONTROLS_FROZEN / R2_TIME_DOMAIN_FAMILY_REJECTED / R2B_DENSE_DATA_READY / R2C_COMPLEX_FIELD_REJECTED / R2D_TRAINABILITY_GATE_NEXT / ADMISSION_SHADOW_SEALED / SELECTIVE_PASS_DISABLED / AUTHORED_CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V3](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). The R2C data-only and Helmholtz candidates repeat byte-identically but collapse toward silence and lose four of five endpoints; context diagnostics show rank 96 retains `99.64%` energy while the trained objective is worse than zero prediction. Next run a query-free energy/objective/optimizer micro-overfit gate, then—only if it passes—freeze one context-basis coordinate-to-coefficient field for one grouped query evaluation. The independent validator and admission shadow remain isolated; production can consume only a bounded cooked record after a future consumer/ADR gate. |
