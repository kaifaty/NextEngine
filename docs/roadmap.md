...
3. **R7c — Linux performance authority
   (`IN_PROGRESS / V6_IMPLEMENTED / R3_R4_R5_GATES_PASS / R2_DEFERRED`):** ADR-091 accepts
   ...
   fixed gates on the single commit `8498001`; remaining work is only R2 once
...
| B-12 | `OPEN / R3_R4_R5_CLOSED / R2_DEFERRED`: ADR-091/092/093/094 accept exact Linux authority, Performance V6/v11 CI-gated warnings, canonical R2–R5 budgets, dimensional comparison and deterministic R5 worker placement (`r5-physics-16.v3`). R3, R4 and R5 hold hard `PASS` ten-run baselines plus isolated fixed gates on the single commit `8498001`; R2 is deferred until a stable desktop presentation path (session restart recommended). | R7c | On one exact clean commit collect compatible ten-run baselines and one isolated fixed three-run hard gate for R2–R5, without retry-to-green. Historical reports cannot be relabelled; virtual/software R2 does not count. |
