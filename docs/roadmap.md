...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V11 closes V9 real reconstruction and targets `FORCE_NORMALIZATION / EXPLICIT_MODAL_TRANSFER / CONSTRAINED_OFFLINE_ML / BAKED_CLIP_ATLAS`; current status `A1R_FORCE_ONSET_PASS / V9_REAL_FIT_REJECTED / B1_V1_REPEAT_REJECTED / B1R_INVALID_PRECHECK / NOISE_AWARE_FRF_NEXT / REAL_DATA_CLOSED / VALIDATOR_NOT_RELEASED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V11](plans/physical-sound-synthesis-roadmap-v11.md), the [B1 exact result](development/physical-sound-r3a-v11-b1-force-response-oracle-result-2026-08-31.md) and [B1R noise research](development/physical-sound-r3a-v11-b1r-precheck-rejection-and-frf-noise-research-2026-08-31.md). B1 V1 predicts unseen force at `0.025706` mean NRMSE and recovers `6/7` accurate poles but rejects its combined mask. B1R then stops before runner/evidence because an in-band noise-relative force normalization marks the weak-force high band `100%` observable. Next B1R2 must bind noise-only ensembles and compare H1/H2 with a noise-aware errors-in-variables/TLS or local-rational estimator on fresh truth. Fresh internet-source work remains blocked until repeat-exact `PASS_KNOWN_TRUTH_FRF`. ML remains offline and runtime still receives only clips; production requires a concrete consumer and ADR-046 gate. |
