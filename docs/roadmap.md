...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1, PS-2 internet corpus/evidence, spatial-representation rejection chain, converged elastic FEM→Bempp/full-angular synthetic controls and hash-closed REALIMPACT geometry-spatial-transfer preregistration `IMPLEMENTED`; current status `PS-2_REAL_SPATIAL_TRANSFER_PROTOCOL_FROZEN / BYTE_IDENTICAL_REPEATS / RESERVED_AUDIO_BYTES_ZERO / PITCHER_CALIBRATION_SEALED / PLANTER_ONE_SHOT_HOLDOUT_SEALED / GEOMETRY_PREFLIGHT_NEXT / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / FALLBACK_OUT_OF_DOMAIN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md), [internet corpus policy](development/physical-sound-internet-corpus-policy-ps2-2026-08-27.md) and [REALIMPACT preregistration evidence](development/physical-sound-realimpact-geometry-spatial-transfer-preregistration-ps2-2026-08-28.md). Manifest `5be5f195…e576` freezes exact prerequisite/payload identities, Pitcher calibration, Planter one-shot holdout, a `90/510` 3D listener split, geometry-spectral Bempp/cooker candidate, controls, gates and fallback. Reports repeat at `c2f51cff…01ef` with zero network requests and zero reserved audio bytes. Credit is protocol/open-order only. Next implement and repeat the geometry-only preflight; any topology, numeric or repeat failure stops before Pitcher audio. All eight exact-domain blockers, measured real 3D transfer, corpus admission, `Pass`, PS-3, AV-P0D and production P1 remain disabled. |
