...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V31](plans/physical-sound-synthesis-roadmap-v31.md) status `ADOPTED / CAUSAL_BASELINE_NEXT / BOUNDED_SOURCE_SCOUTING / SYNTHETIC_VALIDATOR_MECHANICS_ALLOWED / REAL_ADMISSION_BLOCKED_BY_SOURCE_POWER / OFFLINE_ML_ONLY / NO_PER_SOUND_HUMAN_REVIEW / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) remains exact: combined raw power is `10 / 32 / 73`, but the best second protected role still lacks `6` exact-Steel and `27` non-Metal groups while five projects stay reserved. V31 preserves that real gate and makes source scouting bounded/resumable; it moves the immediate implementation boundary to the deterministic causal modal owner, synthetic truth/mutations and validator shell. Real calibration, training, admission and cooking remain sealed until a fresh whole-project role freeze passes; authored fallback remains authoritative. |
