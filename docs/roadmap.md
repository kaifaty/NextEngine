...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V36](plans/physical-sound-synthesis-roadmap-v36.md) is `ACTIVE / R0_A0_F0_C0_COMPLETE / X0_NEXT / FRESH_V36_TARGETS_UNOPENED / V35_QUALITY_UNOBSERVED / REAL_RELEASE_BLOCKED / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V36 C0](development/physical-sound-v36-c0-fresh-structural-census-result-2026-09-03.md) uses a dedicated truth-free structural profile, reproduces all F0 roots and closes P1 witnesses plus 216/215 local support over all `15,120` rows twice exactly; both experts are reachable, OOD count and every forbidden access remain zero. X0 must now detect malformed containers, lifecycle/trace violations and every atomic-publication failpoint; two full-count/full-step D0+H0 surrogate executions must then bind the exact numeric owner before any official capability exists. Synthetic H0 and the independent real source/validator lane must both pass before Steel training; [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker real role six exact-Steel and 27 non-Metal groups short. |
