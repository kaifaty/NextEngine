...
| Physical sound source synthesis | SPEC-45 `Proposed`; V8 strategy `EXPLICIT_MODAL_PARAMETERS / NEURAL_CONTACT_TO_MODE_SHAPE_FIELD / BOUNDED_RESIDUAL / OFFLINE_BAKED_CLIP_ATLAS`; current status `R2_LISTENER_FIELD_REJECTED / R3A_V1–V5_NEGATIVE_OR_INCONCLUSIVE / R3A_V8_SYNTHETIC_PASS / FRESH_REAL_PROTOCOL_NEXT / R3B_NOT_AUTHORIZED / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V8](plans/physical-sound-synthesis-roadmap.md), its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md) and the [V8-SYNTH result](development/physical-sound-r3a-v8-synthetic-preflight-result-2026-08-31.md). V5 remains rejected on fresh real development. V8 modal recovery and two held NISR FEM mode-shape fields pass with byte-identical reports (`0.262x` and `0.097x` nearest RMSE), but this is synthetic substrate evidence only. No fresh real V8 object, representation holdout, real contact field or baked atlas is authorized, and authored clips remain authoritative. The next step is a zero-waveform-decode ObjectFolder Real source/gate freeze. Production still requires a future concrete consumer and ADR-046 gate. |
