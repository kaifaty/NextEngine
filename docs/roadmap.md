...
| Physical sound source synthesis | SPEC-45 `Proposed`; V7 strategy `TASK_SPECIFIC_NEURAL_RATE_DISTORTION / CONTACT_TO_LATENT_FIELD / OFFLINE_BAKED_CLIP_ATLAS`; current status `R2_LISTENER_FIELD_REJECTED / R3A_V1_REPRESENTATION_REJECTED / R3A_V2_INCONCLUSIVE_RESAMPLING_CONTROL / R3A_V3B_NATIVE_NDAC_REJECTED / R3A_V4_FIT_REPRESENTATION_REJECTED / R3A_V5_DEVELOPMENT_REJECTED / R3B_NOT_AUTHORIZED / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V7](plans/physical-sound-synthesis-roadmap.md), its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md) and the [V5-C result](development/physical-sound-r3a-v5c-capacity-frontier-and-development-result-2026-08-31.md). Paired `6/12/24 kbps` checkpoints pass internal anti-collapse and byte budgets, but the frozen development evaluator repeats report `74a6f4ea…bbb4` with spectrum/modal failure on all `12/12` comparisons and decay failure on `11/12`. No capacity, representation holdout, contact-to-latent field or baked atlas is authorized; authored clips remain authoritative. A new revision requires a materially different preregistered representation plus source-disjoint development evidence. Production still requires a future concrete consumer and ADR-046 gate. |
