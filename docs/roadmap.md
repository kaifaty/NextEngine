...
| Текущая точка | R1–R7 завершены на current Linux product boundary. [R7c Linux performance authority](development/task-state/r7c-linux-performance-authority.md) closed B-12 on exact commit `e48f2eac…`; [R7d final hardening](development/task-state/r7d-final-product-hardening.md) and [R7e distribution closure](development/task-state/r7e-linux-distribution-closure.md) close on exact code commit `919663ff…`. The final native target report is `PASS / release_ready=true` with all eight checks passing, and two independently built PackageManifest V6 trees are byte/mode-identical at manifest `47beaf91…`. R8a condition work is complete; [R8b first learned locomotion](development/task-state/r8b-first-learned-locomotion.md) is the selected post-v1 WIP. ADR-101 retires the 24-sphere/23-collinear-X Stage 0 body from optimization and admits a separate command-only standing environment over anatomical biomechanics V2/material V3. Native PhysX proves two sole contacts plus a 32-tick reset/step/safety/reward preflight; exact Isaac generation/smoke and the bounded first optimizer run are next. ADR-090 keeps Windows/THOTH/paired evidence outside current scope indefinitely. R141 remains `INVALID / STOP_NO_RETRY` without R142 or authority over R8b. |
...
| PhysX deterministic humanoid substrate | ADR-058/059/062/063/064/065/067/069/071/100/101 `Accepted`; immutable Stage 0 V1 environments remain exact toy/regression evidence; current biomechanics standing manifest `1b60550d…73bb` passes native two-sole and 32-tick reset/step/safety/reward preflight; Stage 0 product evidence remains incomplete | Create the exact external generation, pass Isaac descriptor/USD reset-reward smoke and execute only the bounded first optimizer discriminator. CPU PhysX remains final evidence authority; `MODEL-MIRROR-P1` and learned quality remain open. |
...
**R8b first learned locomotion (`ACTIVE_R&D / BIOMECHANICS_STANDING_PREFLIGHT /
...
its 32-tick reset/step/safety/reward path. The next bounded work is exact
descriptor-to-USD generation, Isaac reset/reward smoke and the 1,024,000-
transition optimizer discriminator. The ceiling remains
