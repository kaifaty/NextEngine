...
| Physical sound source synthesis | SPEC-45 `Proposed`; V6 strategy `TASK_SPECIFIC_MODAL_BOTTLENECK / NEURAL_CONTACT_GAIN_FIELD / DETERMINISTIC_COOKED_RUNTIME`; current status `R2_LISTENER_FIELD_REJECTED / R3A_V1_REPRESENTATION_REJECTED / R3A_V2_INCONCLUSIVE_RESAMPLING_CONTROL / R3A_V3B_NATIVE_NDAC_REJECTED / R3A_V4_MODAL_BOTTLENECK_NEXT / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V6](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). R3A V3A records a fail-closed eight-sample NDAC decoder deficit without quality metrics; V3B fixes it from synthetic evidence on a new Purple Scoop split. Two native-48 kHz V3B runs pass identity exactly but reject NDAC-75 because spectrum and modal-frequency endpoints fail. General perceptual codecs are closed as the next family. R3A V4 must first prove a bounded task-specific global-modal/contact-gain/learned-residual representation plus deterministic inverse on already-opened development data, then freeze one new unopened discriminator. No neural field training is authorized; production still requires a future consumer/ADR gate. |
