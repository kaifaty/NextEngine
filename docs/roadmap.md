...
| Nonlocal R63Y--R63ZH arithmetic research note | `REPORT_ONLY / TWOFOLD_AFFINE_IMAGE_CANDIDATE / BLOCK_OPERATOR_FROZEN / EXPORTED_FACTOR_CONSUMPTION_CANDIDATE / REVIEWED_RECURRENCE_LADDER_REJECTED / REVIEWED_OPERATOR_INPUT_FACTORIAL / REVIEWED_SINGLE_OPERATOR_SCHEDULE / REVIEWED_DENOMINATOR_STEP / REVIEWED_RAW_SIGN_ALIAS / REVIEWED_AFFINE_IMAGE_CENTER_RESIDUAL_SUFFICIENT / CENTER_PRODUCER_EXACT_STOP_CONTRACT_FROZEN`; no owner promotion | R63Y selects fixed twofold binary64 verification; R63Z/R63ZA close common-block products and exported-factor consumption. Reviewed R63ZB--R63ZF isolate the ladder loss through operator/input, schedule, denominator/step and unchanged raw signs. Reviewed R63ZG shows that the common affine-image center alone reproduces rejection for both immutable solution representations, while its radius alone passes and the `1.00914x` contraction amplification changes no observed boundary. Frozen R63ZH independently tests every center enclosure and the exact identity `r_c-r_t=-M(x_c-x_t)`; a closed identity stops certificate repair and restricts common-operator work to original-equation-preserving acceleration/preconditioning. Portable repair, dynamic builder, corpus, timing, GPU and production remain blocked. |
