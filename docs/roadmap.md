...
| Physical sound source synthesis | SPEC-45 `Proposed`; isolated P0/P0.5 experiment `IMPLEMENTED / LOCAL_EXPERIMENT_PASS`; no public schema, content role or production gate promoted | Fixed-point steel/wood/glass audition and feature-gated reference-demo contact adapter may continue only as PresentationOnly experiments. Production impact waits for a concrete consumer, Accepted promoting ADR, complete engine-owned contact projection, cooked acoustic content/fallback and exact PCM/root checks; rolling/scraping remain later. |
