...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration/causal audit and Ceramic Cup observation execution/diagnostic `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / ADAPTIVE_DECAY_SYNTHETIC_CONTROL_SUPPORTED / CERAMIC_ADAPTIVE_COUNTERFACTUAL_FROZEN / TWO_READ_ONLY_ANALYSES_AUTHORIZED / INDEPENDENT_VALIDATION_REQUIRED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [Ceramic adaptive preflight](development/physical-sound-ceramic-cup-adaptive-counterfactual-preflight-ps2-2026-08-28.md). Manifest `48001fb7…ff9a` and repeated preflight `4358d4da…220f` bind unchanged rows, adaptive method, fixed comparator and seven checks. Execute twice after commit; any failure rejects, while a pass still needs unopened-object validation. Admission, mechanics and Planter remain blocked. |
