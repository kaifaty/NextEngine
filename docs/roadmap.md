...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | SPEC-36/ADR-072 `Proposed`; research and implementation specifications only | Not a v1 blocker. Start with a bounded CPU DFSPH water consumer after the current R4/R5 critical path; GPU is correspondence-only until separately promoted, and MLS-MPM dry terrain precedes wet mud. |
