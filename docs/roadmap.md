...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1 consensus and PS-2 corpus-plan contract `IMPLEMENTED`; current status `PS-2_CONTRACT_COMPLETE / PS-2_ACQUISITION_NEXT / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md): exact axes, split keys, OOD policy and numeric grouped risk/coverage are now pre-registered. The frozen 40-object glass-vessel plan is power-sufficient (35 reject parents required for 95% Wilson upper risk <= `0.10`; 16 in-domain groups for coverage lower bound >= `0.80`), but recordings and independent real-family evidence remain open. Complete controlled acquisition before PS-3 or AV-P0D. R&D remains PresentationOnly and non-blocking for R7. Production impact still waits for an explicit post-v1 slot, concrete consumer, Accepted promoting ADR, complete engine-owned contact projection, cooked acoustic content/fallback and exact PCM/root checks; rolling/scraping remain later. |
