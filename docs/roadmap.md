...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V44](plans/physical-sound-synthesis-roadmap-v44.md) is `ACTIVE / C0R_COMPLETE / B0R_R0R_NEXT / C1_AND_V0S_OPEN / V0_BLOCKED_BY_POWER / FIRST_PACK_SELECTION_PENDING / TRAINING_BLOCKED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | Repeat-exact C1A and [D2/C0R](development/physical-sound-v43-d2-c0r-identity-repair-result-2026-09-03.md) provide the corrected 65-parent disclosed corpus. B0R/R0R now re-freezes the numerical floor while C1 and metadata-first V0S grow runtime descriptors and independent validator sources. The first demo material is selected signal-blind from source power and descriptor coverage rather than fixed as Steel; only B1, frozen V0, one-shot protected admission and deterministic cooking can authorize an opt-in prop, and every failure retains the authored fallback. |
