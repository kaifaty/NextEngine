...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V35](plans/physical-sound-synthesis-roadmap-v35.md) is `ACTIVE / R0_F0_C0_B0_COMPLETE / I0_NEXT / FRESH_TARGETS_UNOPENED / REAL_RELEASE_BLOCKED / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V35 B0](development/physical-sound-v35-b0-local-gate-conformance-result-2026-09-03.md) executes all `6,480` discarded official-shape train rows twice exactly: every partition has `216` rows, group leave-out retains `215`, all five geometry/contact axes refine continuously, train permutations are byte-exact and the strict OOD/blend gates pass without opening an official target or model value. I0 must now import this owner and the frozen V34 terminal publisher to prove all complete-owner terminal paths before D0. Synthetic H0 and the independent real source/validator lane must both pass before Steel training; [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker real role six exact-Steel and 27 non-Metal groups short. |
