...
The next bounded discriminator is exact max/sum merit backtracking on the
simultaneous V8 constraint-QP direction under the already supported R75 step
cap. No stencil/cap/penalty sweep, fresh PhysX or training is authorized while
this offline blocker remains.
...
7. **R5 humanoid movement TRAIN-4 clip-global prototype (`ACTIVE_R&D / CURRENT WIP`):**
   ...
   `cmu05`/`cmu16` and rejects `cmu139`; R74–R79 select exact-merit
   globalization as the bounded current increment. До unchanged all-three
