...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V37](plans/physical-sound-synthesis-roadmap-v37.md) is `ACTIVE / R0_N0_NR_RESEARCH_COMPLETE / QSO_V0_SELECTED / A0_REPEAT_EXACT_PASS / F0_NEXT / V36_CLOSED / H0_UNOPENED / ML_FIRST / AUTOMATIC_VALIDATION / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V36 D0](development/physical-sound-v36-d0-fresh-development-result-2026-09-03.md) closes the scalar hybrid on contact transfer. [QSO A0](development/physical-sound-v37-a0-query-surface-contract-result-2026-09-03.md) repeat-exactly freezes typed immutable CSR/query containers, separate structural/scientific lifecycles and `Pass`-only candidate authority with zero target/truth/model/official access. F0 next allocates fresh disjoint operator identities and freezes truth/roles without evaluating values. The independent source lane remains six exact-Steel and 27 non-Metal groups short. |
