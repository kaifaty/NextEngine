...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V34](plans/physical-sound-synthesis-roadmap-v34.md) status `ADOPTED / R0_F0_C0_T0_D0_COMPLETE / H0_NEXT / CANDIDATE_FROZEN / METHOD_HOLDOUT_UNOPENED / VALIDATOR_FIRST / REAL_RELEASE_BLOCKED / OFFLINE_ML_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V34 D0](development/physical-sound-v34-d0-development-tournament-result-2026-09-02.md) repeats ten artifacts exactly and passes every hard/metric/ablation/resource gate. Contact beats raw MLP `0.327x`, nearest `0.448x` and spectral ridge `0.293x`; all geometry/contact/joint strata pass, `585` nodal rows are exact and holdout remains zero. The frozen candidate may open H0 once after its owner freezes. This is synthetic representation evidence only. [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker real role six exact-Steel and 27 non-Metal groups short, so real calibration, training, admission and cooking remain sealed. |
