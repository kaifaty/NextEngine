...
| Physical sound source synthesis | SPEC-45 `Proposed`; V3 strategy `SELF_VALIDATING_NEURAL_TRANSFER_FIELD / OFFLINE_TRAINING / DETERMINISTIC_COOKED_RUNTIME`; current status `R0_R1_COMPLETE / R2C_REJECTED / R2D_V2_TRAINABILITY_PASS / R2E_ONE_FROZEN_CANDIDATE_NEXT / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V3](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). R2D V2 changes only fixed AdamW rate to a half-cosine schedule and passes every unchanged context coefficient/energy/cooker gate twice; normalized reports, checkpoints and emitted WAVs repeat exactly with zero query reads. Next freeze one data-only context-basis coordinate-to-coefficient R2E field, repeat it without query feedback, then evaluate it once on the 180 grouped queries against all unchanged controls/endpoints. The validator and admission shadow remain isolated; production still requires a future consumer/ADR gate. |
