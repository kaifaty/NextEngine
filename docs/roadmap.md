...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V27](plans/physical-sound-synthesis-roadmap-v27.md) status `ADOPTED / P0A_FROZEN / R0_PREPROCESSOR_NEXT / MODEL_QUALITY_UNOBSERVED / AUTOMATIC_VALIDATOR_REQUIRED / METAL_FIRST / OFFLINE_COOKED_CLIPS / RUNTIME_ML_NOT_AUTHORIZED`; no public schema, content role, runtime model or production gate promoted | V27 makes an external learned generator the scalable authoring path and retains formulas as teacher, deterministic renderer, constraints and fallback. First finish the P0a-bound surface/alignment owner, then run one compact-representation feasibility decision. A separate real-data validator gates one protected Metal candidate before deterministic cooking and one fallback-safe demo; Glass and Wood repeat admission independently. Evidence is internet-only, with no user recording or per-sound human approval. |
