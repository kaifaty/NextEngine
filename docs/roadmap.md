...
| Текущая точка | R3, reference-project vertical, functional R4 и procedural R5a–R5j остаются `COMPLETE`. [R6a creator project CLI](development/task-state/r6a-creator-project-cli.md), [R6b creator run/package](development/task-state/r6b-creator-run-package.md), [R6c creator inspect/diff](development/task-state/r6c-creator-project-inspect-diff.md) и [R6d RPG starter](development/task-state/r6d-creator-project-template.md) завершены: public `next project create/validate/cook/run/package/inspect/diff`, deterministic cold project, generic one-tick Application Session/final save, exact self-verifying Creator Project Package V1 and source-neutral stable-ID projection/diff проходят production checks без engine-crate edit или reference-game fixture constructor. R6 остаётся `IN_PROGRESS`: scenarios/minimization, replay/domain inspectors и broader SDK ещё открыты; следующий bounded increment — R6e public scenario validate/run/minimize. Native Linux x86_64 остаётся active development host по ADR-082; Windows/THOTH, same-commit compare, B-12 и PhysX Stage 0 readiness отложены до explicit pre-R7 Windows bring-up. R141 остаётся `INVALID / STOP_NO_RETRY` без R142/downstream authority. |
...
| Tooling | Repository `xtask`, fixed R5 conformance commands and public R6a–R6d `next project create/validate/cook/run/package/inspect/diff` are implemented with a built-in cold RPG starter, separately versioned one-object path-free JSON, bounded reads, confined fresh output, exact package inventory/NOTICE, production runtime/final-save proof and source-neutral stable-ID project projection. `content-package` generates a project and requires authoring/package projection parity with an empty diff. | Нет broader RPG/replay inspectors, generic scenario/minimizer, fresh clean ten-run V5 THOTH baselines и complete external SDK workflow. |
...
| R6. Creator beta | `IN_PROGRESS / R6D_COMPLETE` | L–XL | Built-in RPG starter creates a second namespaced project that validates/cooks/runs/packages and source/package inspect/diff matches without engine-crate edits or reference-game bootstrap; scenario/replay-inspector breadth remains. |
...
cleanup, R3, functional R4a–R4d и procedural R5a–R5j завершены. R6a–R6d public
create/validate/cook/run/package/inspect/diff and cold second-project workflow
are complete; следующий bounded WIP slot — R6e public scenario
validate/run/minimize vertical.
...
**Текущий checkpoint:** `IN_PROGRESS / R6D_COMPLETE`. ADR-083 retains
...
but does not complete R6; R6e should add public scenario validate/run/minimize.
...
SPEC-24, ADR-008, ADR-014, ADR-025, ADR-083, ADR-084, ADR-085.
...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `SOURCE_LINE_STOPPED / FCR2_REFERENCE_PASS / FORMULA_RECLOSURE_STOP / NEWTON_KRYLOV_NSR2C2_PASS / NSR3_B2_PASS / NSR3_B3_FAIL / NSR3_B3D_ACTIVE_REACTION_PASS_INACTIVE_CERT_REJECT / NSR3_B3D1_DISPLACEMENT_PASS_ENERGY_FLOOR_FAIL / NSR3_B3D2_STATIONARITY_MERIT_SELECTED / NSR3_B3D3_ONE_TRIAL_FAIL / NSR3_B3D4_OWNED_GRADIENT_PASS / NSR3_B3D5_OWNED_RESIDUAL_PASS / NSR3_B3R_STATIC_BOUNDARY_PASS / NSR3_B4A_CLOSED_BOX_PASS / NSR3_B4B2_TINY_PRESSURE_PASS / NSR3_B4C0_EXACT_WORK_FAIL / NSR3_B4C0R_JOINT_NEIGHBOR_PASS / NSR3_B4C3MAR_ADAPTIVE_MACRO_PASS / NSR3_B4C3MC0_MEASUREMENT_PASS / NSR3_B4C3MC1_TINY_ACCURACY_PASS / NSR3_B4E2D7R6_FAIL_INNER_NUMERICAL_FLOOR / NSR3_B4E2D7R7_PASS_MODEL_DERIVATIVE_RECLOSURE / NSR3_B4E2D7R8_PASS_BINARY64_ENERGY_EVALUATION / NSR3_B4E2D7R9_FAIL_INVALID_CONTROL / NSR3_B4E2D7R9R1_PASS_DIVIDED_DIFFERENCE / NSR3_B4E2D7R10_PASS_PRECISION_CERTIFICATE_REQUIRED / NSR3_B4E2D7R11_PASS_OFFLINE_SIGN_CERTIFICATE / NSR3_B4E2D7R12_PASS_PRIVATE_PRESSURE_STATE_CONFIRMED / NSR3_B4E2D7R13_PASS_FULL_PRIVATE_PRESSURE_STATE_CONFIRMED / NSR3_B4E2D7R14_PASS_SPARSE_AL_WORKSPACE_CANDIDATE / NSR3_B4E2D7R15_PASS_NOMINAL_AL_PREREQUISITES / NSR3_B4E2D7R16_PASS_NOMINAL_TRANSACTION_BACKEND / NSR3_B4E2D7R17_PASS_CLASSIFIED_STRUCTURAL_WATCHDOG / NOMINAL_SOLVER_NOT_CONFIRMED / NSR3_B4E2D7R18_PASS_CLASSIFIED_NONDIMENSIONAL_HVP_MISMATCH / NSR3_B4E2D7R18R1_PASS_NONDIMENSIONAL_TRANSACTION / NSR3_B4E2D7R18R2_PASS_CLASSIFIED_INNER_POLICY_INSUFFICIENT / NSR3_B4E2D7R18R3_PASS_PRECANCELLATION_CANDIDATE / NSR3_B4E2D7R18R4_FAIL_WORK_LIFECYCLE / NSR3_B4E2D7R18R4R1_PASS_RETAINS_SECOND_ITERATION / NSR3_B4E2D7R18R4R2_PASS_CONFIRMED / NSR3_B4E2D7R19_FAIL_PRECISION_LEDGER / NSR3_B4E2D7R19R1_PASS_RUNTIME_TOPOLOGY_PRECISION_CANDIDATE / NSR3_B4E2D7R19R2_PASS_STRUCTURAL_WATCHDOG / NSR3_B4E2D7R19R3_PASS_FORCING_CONVERGED_33 / NSR3_B4E2D7R19R4_PASS_RESIDUAL_MODEL_IMAGE / NSR3_B4E2D7R19R5_PASS_SIXTH_TRIAL_ACCEPTANCE / NSR3_B4E2D7R19R6_PASS_LATER_GUARD_DENIED / NSR3_B4E2D7R19R7_PASS_FORCING_CONVERGED_34 / NSR3_B4E2D7R19R8_PASS_RESIDUAL_MODEL_CANDIDATE / NSR3_B4E2D7R19R9_PASS_TOTAL_HVP_EXHAUSTED / NSR3_B4E2D7R19R10_PASS_SAFE_PROGRESSING / NSR3_B4E2D7R19R11_PASS_FORCING_CONVERGED_24 / NSR3_B4E2D7R19R12_PASS_ACCEPTANCE_CANDIDATE / NSR3_B4E2D7R19R13_PASS_OUTER_COMPLETE_NOT_ADMISSIBLE / NSR3_B4E2D7R19R14_PASS_OUTER_BOUNDARY_SUSPENDED / NSR3_B4E2D7R19R15_PASS_V1_RESOURCE_LEDGER_COLLISION / NSR3_B4E2D7R19R16_PASS_V2_VALIDATION_CANDIDATE / NSR3_B4E2D7R19R17_PASS_OWNER_EPOCH_TRANSITION_CANDIDATE / NSR3_B4E2D7R19R18_PASS_RESUME_EQUIVALENT_NOT_ADMISSIBLE / NSR3_B4E2D7R19R19_PASS_OUTER7_GRANT_CANDIDATE / NSR3_B4E2D7R19R20_PASS_OUTER7_EQUIVALENT_NOT_ADMISSIBLE / NSR3_B4E2D7R19R21_PASS_OUTER8_GRANT_CANDIDATE / NSR3_B4E2D7R19R22_PASS_OUTER8_EQUIVALENT_NOT_ADMISSIBLE / NSR3_B4E2D7R19R23_PASS_OUTER9_GRANT_CANDIDATE / NSR3_B4E2D7R19R24_PASS_OUTER9_EQUIVALENT_NOT_ADMISSIBLE / NSR3_B4E2D7R19R25_PASS_OUTER10_GRANT_CANDIDATE / NSR3_B4E2D7R19R26_PASS_OUTER10_EQUIVALENT_NOT_ADMISSIBLE / NSR3_B4E2D7R19R27_PASS_OUTER11_GRANT_CANDIDATE / NSR3_B4E2D7R19R28_PASS_OUTER11_EQUIVALENT_NOT_ADMISSIBLE / NSR3_B4E2D7R19R29_PASS_LINEARIZED_FEASIBILITY_OPERATOR / NSR3_B4E2D7R19R30_FROZEN_RANGE_PROJECTION_IMPLEMENTATION_NEXT / NSR3_RESEARCH_ACTIVE / INTEGRATION_BLOCKED`; SPEC-38/ADR-076 `Proposed`; later ProductChecks remain `NOT_RUN` | W0F/G/H/I roots remain frozen and W3 remains blocked. NPR1-B and FCR3-B2 remain exact negative evidence. The separate [nonlinear-solver roadmap](plans/nonlocal-nonlinear-solver-research/README.md) proves the analytic HVP, exact canonical neighborhoods, guarded trust-region solve, box KKT path and deterministic nominal CPU transaction. Query-evidence separation, canonical topology reuse, invariant HVP coefficients, fused evaluation/tape, fixed owner-computes parallelism and directed scratch reuse reduce the one-macro research path, but shared-host CPU/wall selection is stopped pending a dedicated lane. D7R10--D7R12 repair and certify the cancelled actual-reduction path. D7R13 confirms the unchanged pressure state from the original private state with one consistent `eta=1e-10` solver: outer 11/12 confirm, outer 13 holds out, and the exact work baseline is 19 acceptances/38 HVPs. Wider runtime state is not selected. D7R14 proves the same solver over a separate sparse AL tape and exact current/trial pair union: tiny evaluation/HVP/divided/full-transaction outputs are binary64 exact, and the nominal frame-zero mapping replaces `116,301,000` dense candidates by `342,502` selected pairs (`339.563x` structural gap). D7R15 passes with explicit aligned-substep `dt`, all four D7R13 binary128 roots reproduced through a sparse union, and one identity-bound nominal static index. D7R16 integrates sparse long double, sparse binary128 and one identity-bound static index through the complete transaction: 38 accepted audits, eight candidate audits, all complete/inner/precision roots exact and zero candidate all-pair calls. D7R17 then safely exhausts its 16-update watchdog: all inner solves converge in one accepted trial/two HVPs and final stationarity is `1.49e-12`, but primal violation decreases only from `2.332e-7` to `2.285e-7`, so no state is selected. D7R18 exposes `kappa` and preserves exact `kappa*dt^2`, then isolates dimensional HVP accumulation as the only strict scaling mismatch. D7R18R1 closes that representation boundary with direct `u=lambda/kappa`, `theta=kappa*dt^2/M`: dense/sparse and cross-scale normalized roots are byte-exact, both extended-precision signs agree, and dimensional reconstruction satisfies the frozen componentwise forward bound. D7R18R2 proves cross-scale complete normalized roots are exact, but active outer 1/trial 2 loses a true positive `3.163325e-22` reduction as `-3.071473e-19` because the normalized divided formula omits pairwise density-delta precancellation. D7R18R3 restores that signal as `3.163242e-22`, within `1.18e-5` relative of resolved-positive binary128 and exact across normalized profiles; its replay accepts nothing. D7R18R4 confirms all states and signs but fails exact work: outer 11/trial 0 adds one Krylov HVP per active run (`39` versus frozen `38`). R4R1 confirms the inherited scale defect, and R4R2 reproduces the pre-derived five-run ledger with 57 explicit dimensionless trust solves and 117 HVPs. D7R19 then fails hard before route selection: three of five accepted outer-0 long-double audits recompute compact-support membership differently, totalling 10,989 repeated observations; all three signs remain resolved positive and minimum horizon margin is zero. Its sixth trust solve also reaches the unchanged 32-HVP cap, but precision failure has precedence. D7R19R1 proves every mismatch lies in the frozen C2 shell and all topology lanes preserve positive signs. D7R19R2 then keeps the complete binary64 trace exact under binary64-owned audit membership and exposes the unchanged sixth-solve 32-HVP structural watchdog after five accepted trials. D7R19R3 proves the exact sixth recurrence converges on HVP 33 with positive curvature, negligible recurrence error and moderate Ritz condition 36.13; a preconditioner is not justified for this barrier. D7R19R4 selects residual-derived r_final-g: direct and candidate quadratic/predicted scalars are bit-exact with 1.24e-15 image L2 error, eliminating the separate model HVP candidate. D7R19R5 proves the guarded sixth trial is accepted at divided ratio 0.9999993088 with positive precision sign, exact radius and zero candidate model HVP. D7R19R6 is frozen as one full private first-substep transaction: ordinary solves retain the historical direct model HVP, while only a solve actually using guarded HVP 33 may use r_final-g. Require exact R2 trials 0-4, exact R5 trial 5, explicit work ownership, unchanged total caps and no public commit or second substep. D7R19R7--R13 then close the guarded completion path through one accepted outer-5 boundary at projected total HVP 523. D7R19R14 suspends there without work; R15 proves its v1 token incomplete; R16 replaces it with an exact private 540-byte v2 validation envelope. D7R19R17 consumes the separate source owner once and issues an epoch-1 grant through an exact copy-on-write transaction: all 15 routes pass, slice HVP resets to zero, cumulative HVP stays 523, and no resume or outer 6 executes. D7R19R18 then proves byte-exact slice/unsliced outer-6 equivalence: two accepted trials consume 50 HVP, four workspaces and two precision audits; the finite result remains non-admissible at primal 2.5139e-8. D7R19R19 proves a zero-work one-use outer-7 grant inside epoch 1: all canonical roots and 14/14 atomic routes are exact while slice/cumulative remain 50/573. R20 proves exact candidate/unsliced-oracle outer-7 equivalence at nonzero budget offsets: two accepted trials consume 52 HVP, primal improves to 2.1404e-8 but remains non-admissible, and slice/cumulative become 102/625. R21 proves a zero-work one-use outer-8 grant inside epoch 1 with all canonical roots and 14/14 atomic routes exact at slice/cumulative 102/625. R22 proves exact outer-8 candidate/oracle equivalence: three accepted trials use 80 HVP, primal improves only 4.9% to 2.03584e-8, last trial reaches 32/34 HVP, and slice/cumulative become 182/705. R23 proves the zero-work one-use outer-9 grant: all 14 routes and canonical roots are exact at slice/cumulative 182/705. R24 proves exact outer-9 candidate/oracle equivalence: two accepted trials use 49 HVP, primal improves 4.454% to 1.94516e-8 but remains non-admissible, and stationarity rises to 9.58664e-11. R25 proves the zero-work one-use outer-10 grant: all 14 routes and canonical roots are exact at slice/cumulative 231/754. R26 proves exact outer-10 candidate/oracle equivalence: two accepted trials use 51 HVP, stationarity returns to 2.08190e-14, and primal improves only 1.0032% to 1.92565e-8. R27 proves the zero-work one-use outer-11 grant: all 14 routes and canonical roots are exact at slice/cumulative 282/805. R28 proves exact outer-11 candidate and oracle equivalence: two accepted trials use 51 HVP, primal improves 4.4290 percent to 1.84036e-8 and stationarity stays small. R29 closes the dimensionless constraint-Jacobian and adjoint prerequisite with byte-exact pair-once/directed JVP, finite-difference and adjoint controls; R30 freezes column-scaled LSMR range projection over the exact violated rows, while any correction, floor classification, tuning and a following outer remain forbidden. Viscosity walls, surface calibration, internal aperture, broad corpus, CUDA/GPU, runtime integration and public contracts remain blocked. DFSPH remains the correctness reference; terrain remains Package 10T. |
| Nonlocal R63Y--R63ZN arithmetic research note | `REPORT_ONLY / TWOFOLD_AFFINE_IMAGE_CANDIDATE / BLOCK_OPERATOR_FROZEN / EXPORTED_FACTOR_CONSUMPTION_CANDIDATE / REVIEWED_RECURRENCE_LADDER_REJECTED / REVIEWED_OPERATOR_INPUT_FACTORIAL / REVIEWED_SINGLE_OPERATOR_SCHEDULE / REVIEWED_DENOMINATOR_STEP / REVIEWED_RAW_SIGN_ALIAS / REVIEWED_AFFINE_IMAGE_CENTER_RESIDUAL_SUFFICIENT / REVIEWED_COMMON_SOLUTION_OUTSIDE_VERIFIER_AFFINE_MODEL / CERTIFICATE_REPAIR_STOPPED / ORIGINAL_OPERATOR_TWOFOLD_RECURRENCE_AUTHOR_REJECTED / PRODUCT_PRECISION_LOCALIZATION_INCONCLUSIVE / R63ZK_INCONCLUSIVE / R63ZL_INCONCLUSIVE / R63ZM_REVIEWED_FIXED_BINARY_TRACE_BOUNDARY_CANDIDATE / R63ZN_EARLY_REJECTION_MATRIX_PASS_LATE_CONTROLS_PENDING`; no owner promotion | R63Y selects fixed twofold binary64 verification; R63Z/R63ZA close common-block products and exported-factor consumption. Reviewed R63ZB--R63ZF isolate the ladder loss through operator/input, schedule, denominator/step and unchanged raw signs. Reviewed R63ZG/R63ZH stop certificate repair. R63ZI contains all exact tangent-Gram coefficients/products but all K2 states reject, so dense width-two storage is author-rejected. R63ZJ and R63ZK remain `INCONCLUSIVE` because their resealed DTO/work boundaries are incomplete; R63ZL also closes `INCONCLUSIVE` after its single repair. R63ZM revision 4 was rejected on typed-field, route-authority, work-ledger and layout-proof defects. The single revision-5 repair received independent `GO`: the exact 1,033,625-byte cache, six bit-exact products, fixed 12,916-byte artifact and independent 420-byte checker audit close with `56/56` controls and distinct `57/57` audits. R63ZN revision 2 freezes only the first real consumer prefix. Independent accepted-path reconstruction exposed and repaired repeated input hashing, an omitted parent-set root and incomplete work categories. The candidate/checker now reproduce exact Release 1,216-byte receipts and 548-byte audits with zero package allocations. Fixed candidate receipts and independently verified routes pass for the complete 12-case missing/short/trailing/same-size input matrix. Late `x0`/arithmetic/nonpositive receipts, remaining work/event controls and fresh review are pending. Complete recurrence, certificates, representation-family generalization, dynamic builder, corpus, runtime stop, timing, GPU, Rust integration and production remain blocked and ProductChecks remain `NOT_RUN`. |
...
| B-09 | `IN_PROGRESS / R6D_COLD_AUTHORING_COMPLETE`: public path-free `next project create/validate/cook/run/package/inspect/diff`, independent data-only `creator-smoke` and generated namespaced RPG starter now pass exact cook/activation, generic one-tick Application Session/final save, reproducible Creator Project Package V1 rerun and source-neutral authoring/package parity through `content-package`; fresh-output generation adds no engine-crate/reference-game constructor, ID override, private storage dump or mutable inspector. Scenarios/minimization, broader domain/replay inspectors and complete SDK documentation remain absent. | R6, R7 | R6e adds public scenario validate/run/minimize; later increments add replay/domain inspection and remaining SDK workflow before closure. |
...
14. **R6 creator CLI and second project (`IN_PROGRESS / R6D_COMPLETE`):** public
   ...
   plus source-neutral stable-ID projection and deterministic RPG starter are
   complete; scenarios/minimization and broader inspectors remain.
...
R6e public scenario validate/run/minimize is the next bounded WIP slot.
