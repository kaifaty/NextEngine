...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `ACTIVE_ISOLATED_RESEARCH / INTEGRATION_BLOCKED`; SPEC-38/ADR-076 `Proposed`; water W1 `LINUX_W1_PASS / CONTINUUM-WATER-REF-P1=PASS` at `fe223f9`; CPU/direct-GPU W2 miss; Nonlocal NR4 `NONLOCAL_48K_RECLOSURE_CANDIDATE / NO_W2_CREDIT`; performance reclosure `NP0_COMPLETE / NP1_P1_NEXT`; later ProductChecks `NOT_RUN` | W0F/G/H/I roots remain frozen and W3 remains blocked. [NR4](development/nonlocal-continuum-nr4-decision-2026-08-20.md) closes the original 48k branch. [NP0](development/nonlocal-continuum-np0-evidence-2026-08-20.md) freezes a reproducible exact-50k coherent/permuted/advected corpus: conditioned duplicate decisions pass p99 6 ms but miss p95 4 ms at worst-case coherent `4.869248 ms` and advected `4.494592 ms`; stable-ID permutation alone makes adjacent p95 `1.94x` slower. P1 pair-term fusion is next, followed by compact CSR, dynamic locality and certified neighbor reuse; convergence and scale work remain separately credited. This does not close W2, reduce the `4/6 ms` target, grant GPU authority or change runtime/public contracts. Promotion still requires an explicit consumer-backed decision, cross-target and exchange identity, one composition DAG/PhysX integration, scheduled epochs, admission/fault semantics and the successor combined budget. Independent terrain closure remains Package 10T. |
