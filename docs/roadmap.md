...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V22](plans/physical-sound-synthesis-roadmap-v22.md) status `ADOPTED / V21_F1A_RESOURCE_REJECT / QUALITY_UNOBSERVED / P1R_PROTOCOL_NEXT / FRESH_VALUES_SEALED / SOURCE_GROWTH_PARALLEL / ADMISSION_BLOCKED / CLIP_FALLBACK / P2_BLOCKED`; no public schema, content role, runtime model or production gate promoted | [V21 F1a](development/physical-sound-v21-f1a-continuous-field-result-2026-09-01.md) exceeded its 30-minute gate before atomic publication, so no quality metric/model/tree exists and `1801…1912` are spent. V22 permits only preregistered execution-equivalent batching/precomputation on fresh `2201…2512`, while preserving P1a's six candidates, 1,500 updates and strict `0.50`; [S0c](development/physical-sound-v15-s0c-source-sufficiency-role-freeze-result-2026-09-01.md) independently keeps real admission blocked under unchanged `4/1/1/1/1`; authored clips remain complete fallback. |
