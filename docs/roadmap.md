...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V10 keeps V9 `EXPLICIT_MODES / NEURAL_TIME_VARYING_RESIDUAL / OFFLINE_BAKED_CLIP_ATLAS`; current status `V9_SYNTHETIC_PASS / A0_REAL_SOURCE_PASS / A1_REJECTED_AT_ONSET / A1R_SOURCE_SEMANTICS_NEXT / REAL_QUALITY_NOT_PROVEN / VALIDATOR_NOT_RELEASED / ADMISSION_SHADOW_SEALED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V10](plans/physical-sound-synthesis-roadmap.md), the [A0 source/role freeze](development/physical-sound-r3a-v10-objectfolder-real-source-and-role-freeze-2026-08-31.md) and the [A1 exact result](development/physical-sound-r3a-v10-beer-glass-real-fit-result-2026-08-31.md). Two fit runs repeat report `7d7bb630…9db9b`: the frozen onset threshold is greater than the entire waveform peak for contacts `18/29`, so no representation fit or metrics ran and every protected read remains zero. A1R must obtain a published timestamp/force onset on a new source revision before another fit; current-contact threshold repair is forbidden. No real quality, development, holdout, atlas or runtime authority exists; authored clips remain authoritative. Production requires a future concrete consumer and ADR-046 gate. |
