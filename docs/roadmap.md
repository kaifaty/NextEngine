...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V12 targets `ACQUISITION_COVERAGE_CERTIFICATE / EXPLICIT_MODAL_TRANSFER / CONSTRAINED_OFFLINE_ML / INDEPENDENT_VALIDATOR / BAKED_CLIP_ATLAS`; current status `A1R_FORCE_ONSET_PASS / V9_REAL_FIT_REJECTED / B1R3_REPEAT_REJECTED / COVERAGE_CERTIFICATE_NEXT / ZERO_DECODE_SOURCE_SEARCH_OPEN / REAL_PCM_CLOSED / VALIDATOR_NOT_RELEASED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V12](plans/physical-sound-synthesis-roadmap-v12.md) and the [B1R3 exact result](development/physical-sound-r3a-v11-b1r3-local-modal-support-result-2026-08-31.md). B1R3 fixes the broad-mask and calibration-interference defects, predicts unseen responses at `0.007180` mean NRMSE and recovers accurate `6/7` locally observable poles. It still rejects before holdout because the frozen force bank has an incidental `6,643 Hz` spectral hole and the single-notch control therefore loses two modes. V12 separates acquisition coverage, identifiable object modes and query-force relevance; it permits one fresh coverage-certified oracle plus a parallel zero-decode source inventory. Real PCM, ML, validator release and runtime stay closed; production requires a concrete consumer and ADR-046 gate. |
