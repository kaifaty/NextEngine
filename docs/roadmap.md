...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V37](plans/physical-sound-synthesis-roadmap-v37.md) is `ACTIVE / R0_N0_NR_RESEARCH_COMPLETE / QSO_V0_SELECTED / A0_NEXT / V36_CLOSED / H0_UNOPENED / ML_FIRST / AUTOMATIC_VALIDATION / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V36 D0](development/physical-sound-v36-d0-fresh-development-result-2026-09-03.md) closes the scalar hybrid on contact transfer. [QSO-v0 research](development/physical-sound-v37-query-surface-operator-research-2026-09-03.md) selects the smallest materially different successor: a query-conditioned graph/surface operator over a P1-owned canonical probe field, with no local/neural scalar blend. A0 next implements typed CSR/query containers and makes candidate freeze structurally Pass-only before any new role/value. The independent source lane remains six exact-Steel and 27 non-Metal groups short. |
