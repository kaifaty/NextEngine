...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `ACTIVE_ISOLATED_RESEARCH / INTEGRATION_BLOCKED`; SPEC-38/ADR-076 `Proposed`; water W1 `LINUX_W1_PASS / CONTINUUM-WATER-REF-P1=PASS` at `fe223f9`; CPU/direct-GPU W2 miss; Nonlocal performance reclosure `NONLOCAL_50K_FIXED_WORK_RECLOSURE_CANDIDATE / NO_W2_CREDIT`; later ProductChecks `NOT_RUN` | W0F/G/H/I roots remain frozen and W3 remains blocked. [The performance decision](development/nonlocal-continuum-performance-decision-2026-08-20.md) retains exact fused owner-term traversal plus compact `u16` CSR: two fresh 512-sample processes per exact-50k profile pass the standalone `4/6 ms` gate, with worst p95/p99 `3.211200/3.227296 ms` coherent and `3.149024/3.189504 ms` advected. Dynamic cell locality is useful only for deliberately permuted IDs and was rolled back on coupled regression; certified Verlet reuse was stopped by a reproduced active-order mismatch. Conditional convergence/scale rescue branches were not activated. This authorizes only a later Proposed solver reclosure and broader corpus; it does not close W2, grant GPU authority or change runtime/public contracts. Promotion still requires an explicit consumer-backed decision, cross-target and exchange identity, one composition DAG/PhysX integration, scheduled epochs, admission/fault semantics and the successor combined budget. Independent terrain closure remains Package 10T. |
