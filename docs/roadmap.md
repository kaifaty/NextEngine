...
| Physical sound source synthesis | SPEC-45 `Proposed`; frozen Q30 modal/DCT baseline, internet corpus/evidence adapters, synthetic FEM→Bempp controls and repeated real-method rejections `IMPLEMENTED`; strategy `NEURAL_FIRST / OFFLINE_MODEL / DETERMINISTIC_COOKED_RUNTIME`; current status `PS-2N0_CONTRACT_IMPLEMENTED / SYNTHETIC_FIXTURE_PASS / REAL_PROJECTION_AND_N0.2_NEXT / ADMISSION_SHADOW_SEALED / SELECTIVE_PASS_DISABLED / AUTHORED_CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md), [PS-2N0 evidence](development/physical-sound-neural-data-plane-ps2n0-2026-08-30.md), [neural strategy](development/physical-sound-neural-acoustic-field-strategy-2026-08-30.md) and [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). Next project a permitted real development slice and export the deterministic Q30/DCT benchmark, then evaluate object-specific few-shot and shared geometry-conditioned fields on held-out impact/listener/object axes. Validator and admission shadow remain independent; production consumes only a bounded cooked record after a future consumer/ADR gate. |
