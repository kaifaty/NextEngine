...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V29](plans/physical-sound-synthesis-roadmap-v29.md) status `ADOPTED / V28_M0C_CLOSED / METAL_ROLE_REBASELINE_NEXT / VALIDATOR_FIRST / CAUSAL_HYBRID_GENERATOR_RESEARCH / INTERNET_ONLY / AUTOMATIC_ADMISSION / CLIP_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V29 research](development/physical-sound-v29-validator-first-ml-rebaseline-research-2026-09-02.md) preserves V28's repeat-exact representation reject and finds that the frozen target corpus is Glass-specific, so it cannot validate Metal. Q0-M metadata/role/power freeze is next; in parallel P0 may freeze one physics-locked modal-plus-learned-correction family. Metal training remains blocked until validator qualification and synthetic remesh/counterfactual truth pass; authored fallback stays authoritative. |
