...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V39](plans/physical-sound-synthesis-roadmap-v39.md) is `ADOPTED / F0_NEXT / TWO_LANE_ML_FOUNDRY / PROTECTED_ADMISSION_BLOCKED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | V39 starts a permanent disclosed-data ML lane now while preserving a separate one-shot admission lane. F0 first freezes the target-free foundry/access/report boundary, then F1 reproduces the exact [V38 S0](development/physical-sound-v38-s0-gap-directed-source-result-2026-09-03.md) `6 exact-Steel / 23 non-Metal` source deficit. Disclosed projects may support automatic validator and generator development but can never return to the protected pool. Strict whole-project role power, independent validator qualification, method holdout and joint shadow still gate deterministic cooking and the opt-in Steel demo; every failure retains the authored fallback. |
