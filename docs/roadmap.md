...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1 and PS-2 plan/pilot/acquisition bundle `IMPLEMENTED`; current status `PS-2_ACQUISITION_CONTRACT_COMPLETE / PS-2_PHYSICAL_CAPTURE_NEXT / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md): exact axes, risk policy and complete raw synchronized microphone/force/metrology bundle are executable. One 48 kHz REALIMPACT transfer correctly falls back for missing source components and its report remains byte-stable; a complete control is only research-eligible. Physical capture must now scale from one full bundle to 40 objects, 35 reject parents and 16 coverage groups before PS-3 or AV-P0D. R&D remains PresentationOnly and non-blocking for R7. Production impact still waits for an explicit post-v1 slot, concrete consumer, Accepted promoting ADR, complete engine-owned contact projection, cooked acoustic content/fallback and exact PCM/root checks; rolling/scraping remain later. |
