...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V27](plans/physical-sound-synthesis-roadmap-v27.md) status `ADOPTED / R0_REPEAT_EXACT_PASS / OFFICIAL_CONTEXT_PREFLIGHT_PASS / R1_NEXT / MODEL_QUALITY_UNOBSERVED / AUTOMATIC_VALIDATOR_REQUIRED / METAL_FIRST / OFFLINE_COOKED_CLIPS / RUNTIME_ML_NOT_AUTHORIZED`; no public schema, content role, runtime model or production gate promoted | [R0](development/physical-sound-v27-r0-preprocessing-owner-result-2026-09-02.md) passes full-entry A/B, official-shape mutations and actual internet-source contexts at root `d013ec35…f456` without opening a model or protected/query role. Next freeze one official M0b manifest and run the compact-representation decision once. A separate real-data validator then gates one Metal candidate before cooking/demo; Glass and Wood repeat admission independently. No user recording or per-sound human approval. |
