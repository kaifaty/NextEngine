...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `SOURCE_LINE_STOPPED / FCR2_REFERENCE_PASS / FORMULA_RECLOSURE_STOP / NEWTON_KRYLOV_NSR2C2_PASS / NSR3_B2_PASS / NSR3_B3_FAIL / NSR3_B3D_ACTIVE_REACTION_PASS_INACTIVE_CERT_REJECT / NSR3_B3D1_DISPLACEMENT_PASS_ENERGY_FLOOR_FAIL / NSR3_B3D2_STATIONARITY_MERIT_SELECTED / NSR3_B3D3_ONE_TRIAL_FAIL / NSR3_B3D4_OWNED_GRADIENT_PASS / NSR3_RESEARCH_ACTIVE / INTEGRATION_BLOCKED`; SPEC-38/ADR-076 `Proposed`; later ProductChecks remain `NOT_RUN` | W0F/G/H/I roots remain frozen and W3 remains blocked. NPR1-B and FCR3-B2 remain exact negative evidence. The separate [nonlinear-solver roadmap](plans/nonlocal-nonlinear-solver-research/README.md) proves the full analytic HVP, exact canonical neighborhoods and bounded guarded trust-region path through 4096 particles; fine-state-owned adaptive composition passes the manufactured `0.05 s` horizon, and the split fixed-support/contact B2 formula passes derivative, layer and static reaction gates. B3 fails before face contact. D1 removes world-position cancellation but reaches an unresolved energy floor; D2 selects stationarity merit and D3 rejects a one-trial trajectory policy. D4 proves the remaining fine overshoot came from a legacy position-subtraction inertia gradient and selects capped owned-gradient residual iteration next. Boundary trajectory composition, physical corpus, CUDA, performance, integration and public contracts remain blocked. DFSPH remains the water correctness reference; terrain remains Package 10T. |
