...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V26](plans/physical-sound-synthesis-roadmap-v26.md) status `ADOPTED / M0A_E_PREPROCESS_CONFORMANCE_REJECT / NO_MODEL_VALUES_OPENED / BARYCENTRIC_SUCCESSOR_SELECTED / M0B_PROTOCOL_REQUIRED / AUTOMATIC_VALIDATOR_REQUIRED / OFFLINE_COOKED_CLIPS / P3_BLOCKED`; no public schema, content role, runtime model or production gate promoted | [M0a-E](development/physical-sound-v25-m0a-official-evaluation-result-2026-09-02.md) stopped before model construction because a valid official impact was not a coarse-mesh vertex; run B was not started and no quality value opened. [Bounded research](development/physical-sound-v26-off-vertex-field-transfer-research-2026-09-02.md) selects deterministic triangle location plus barycentric interpolation, not a looser tolerance or nearest-vertex snap. Next is P0 protocol freeze, an all-official-profile structural fixture, one execution-equivalent M0b A/B decision, independent validator, protected Metal admission, deterministic cooker and one fallback-safe demo prop. No user recording or per-sound human approval. |
