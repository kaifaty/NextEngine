...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V38](plans/physical-sound-synthesis-roadmap-v38.md) is `ADOPTED / V37_QSO_V0_D0_REPEAT_EXACT_METRIC_REJECT / H0_PERMANENTLY_CLOSED / DATA_AND_VALIDATOR_FIRST / DISCLOSED_DEVELOPMENT_BAKEOFF / OFFLINE_ML_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V37 D0](development/physical-sound-v37-d0-fresh-development-result-2026-09-03.md) closes QSO-v0 after byte-identical `MetricReject`: all `11/11` hard and `5/5` resource gates pass, but only `11/22` metric gates pass; candidate authority is false and H0 remains unopened. The next gate is a metadata-only internet-source batch against the exact frontier of six exact-Steel and 27 non-Metal groups. After role-power closure, independent validator qualification and a bounded disclosed-development generator bake-off precede one-shot method holdout, joint admission, deterministic cooking and demo fallback. |
