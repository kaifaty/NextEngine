...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V28](plans/physical-sound-synthesis-roadmap-v28.md) status `ADOPTED / H0_RESOURCE_CAUSE_CONFIRMED / R0_PREFIX_PROTOCOL_NEXT / Q0_SIGNAL_BLIND_READY / M0B_SPENT`; no public schema, content role, runtime model or production gate promoted | [V28 H0](development/physical-sound-v28-prefix-bounded-training-research-2026-09-02.md) proves the frozen loss observes only a `4,096`-sample prefix: five random loss/gradient cases are exact and a rendered training step is `83.7×` faster. Freeze a fresh M0c protocol and pass its full-owner resource oracle before official values; Q0 validator inventory may proceed independently. Metal admission, cooker/demo and Glass/Wood remain blocked; authored fallback stays authoritative. |
