...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V42](plans/physical-sound-synthesis-roadmap-v42.md) is `ACTIVE / B0_COMPLETE / R0_NEXT / V0_READY / DESCRIPTOR_SIGNAL_REQUIRED / PROTECTED_ADMISSION_SOURCE_POWER_OOD / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | The repeat-exact [B0 baseline](development/physical-sound-v41-b0-grouped-baseline-result-2026-09-03.md) runs six methods on C0's isolated `44 train / 70 development` records. Global prototype median parent RMSE `1.337593650` beats ridge `1.443077073` and pointwise MLP `1.484006315`; coarse material alone therefore cannot justify training `StructuredRecipeNet`. R0 must separate source-domain shift from missing object information, then C1/T0/B1 must prove that runtime-available descriptors beat the global floor. V0 may independently freeze validator mechanics; protected admission remains short by `13/34` and `13/31`. Only qualified validator, generator holdout and joint shadow may authorize deterministic cooking and the opt-in Steel demo; every failure retains the authored fallback. |
