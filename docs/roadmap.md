...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry and repeated Pitcher calibration `IMPLEMENTED`; current status `PS-2_PITCHER_GEOMETRY_SPATIAL_CALIBRATION_REJECTED / BYTE_IDENTICAL_REPEAT / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / BOUNDED_RESEARCH_NEXT / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [calibration evidence](development/physical-sound-realimpact-pitcher-calibration-ps2-2026-08-28.md). Report `8bd5323c…1aea` and exact Rust projection repeat. Thirteen modes pass coverage and solver residuals are healthy, but frequency mapping misses by `0.566/1.626` octave median/p90; all held strata fail at `19.9–23.4 dB` median, and candidate median is `2.70×` RBF with `+27.58 dB` p90 regression. Retire this proxy, preserve clip fallback and keep Planter sealed. Next research thickness/interior, excitation and metadata counterfactuals on an unopened development object. All exact-domain, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
