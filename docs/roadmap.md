...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1, PS-2 plan/pilot/`E1` import, bounded internet source/cache/archive/Ogg/RAR5 handling, eight E3 project sources, explicit target/reject-parent roles, five typed REALIMPACT E2 rows, exact-domain/source gates, V1/V2 transfer calibration, listener acquisition, spatial-representation rejection chain and analytical boundary-solver/quadrature harness `IMPLEMENTED`; current status `PS-2_SEVEN_POINT_QUADRATURE_REJECTED / ABSOLUTE_GATES_PASS / REFINEMENT_AND_COMPARISON_FAIL / NO_FRESH_DATA_OPENED / TWO_HOLDOUT_OBJECTS_PRESERVED / INDEPENDENT_BEMPP_CONTROL_NEXT / THREE_DIMENSIONAL_SPATIAL_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / FALLBACK_OUT_OF_DOMAIN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md), [internet corpus policy](development/physical-sound-internet-corpus-policy-ps2-2026-08-27.md) and [quadrature evidence](development/physical-sound-bem-quadrature-discriminator-ps2-2026-08-28.md). Local capture is retired. Seven-point regular quadrature leaves fine/coarse median error at `2.6109` against `0.8` and makes fine median error `1.0447x` the three-point control against `0.75`; reports repeat at `7460750e…b48d` and V1 stays exact. Open no fresh data. Next run an independent Bempp-cl `0.4.2` Galerkin/singular-quadrature sphere control at revision `a1eaaef9…e1c0`. All eight exact-domain blockers, 3D spatial credit, corpus admission, `Pass`, PS-3, AV-P0D and production P1 remain disabled. |
