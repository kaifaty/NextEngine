...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1, PS-2 plan/pilot/`E1` import, bounded internet source/cache/archive/Ogg/RAR5 handling, eight E3 project sources, explicit target/reject-parent roles, five typed REALIMPACT E2 rows, exact-domain/source gates, V1/V2 transfer calibration, listener acquisition, spatial-representation rejection chain, analytical boundary harness and independent Bempp control `IMPLEMENTED`; current status `PS-2_INDEPENDENT_BEMPP_ANALYTICAL_CONTROL_SUPPORTED / ALL_SEVEN_GATES_PASS / BYTE_IDENTICAL_REPEAT / NO_FRESH_DATA_OPENED / TWO_HOLDOUT_OBJECTS_PRESERVED / NONSPHERICAL_SURFACE_MODE_NEXT / THREE_DIMENSIONAL_SPATIAL_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / FALLBACK_OUT_OF_DOMAIN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md), [internet corpus policy](development/physical-sound-internet-corpus-policy-ps2-2026-08-27.md) and [independent Bempp evidence](development/physical-sound-bempp-independent-control-ps2-2026-08-28.md). Local capture is retired. The 512-panel independent sphere reaches max complex error `1.2710%`, magnitude `0.1111 dB`, phase `0.4844°`, direction span `0.0074 dB` and refinement `0.2625x`; all gates and repeats pass at `ba638a21…01f1`. Credit is analytical sphere only. Open no fresh data. Next freeze one non-spherical prescribed surface mode and cross-check the repository cooker. All eight exact-domain blockers, real 3D spatial credit, corpus admission, `Pass`, PS-3, AV-P0D and production P1 remain disabled. |
