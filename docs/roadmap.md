...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V35](plans/physical-sound-synthesis-roadmap-v35.md) is `ACTIVE / R0_F0_C0_COMPLETE / B0_NEXT / FRESH_TARGETS_UNOPENED / REAL_RELEASE_BLOCKED / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V35 C0](development/physical-sound-v35-c0-witness-and-coverage-census-result-2026-09-02.md) solves all `1,512 / 15,120` P1 structural cases/rows twice exactly: each of `8,640` evaluation rows has `216` train neighbors, both experts reachable and zero OOD, while all `6,480` train rows retain `215` neighbors after causal case-group leave-out. B0 must now execute continuity, permutation, leave-out, zero-distance, bounded-blend and OOD probes on discarded analytic fields before I0 or any official target/model value. Synthetic H0 and the independent real source/validator lane must both pass before Steel training; [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker real role six exact-Steel and 27 non-Metal groups short. |
