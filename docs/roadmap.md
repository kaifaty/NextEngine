...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V15 status `S0_IDENTITY_AND_SCOPE_FREEZE_NEXT / METAL_FIRST / WOOD_PENDING / GLASS_FALLBACK_ONLY_SOURCE_GROWTH / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | [V15 rebaseline](development/physical-sound-v15-progressive-material-admission-rebaseline-2026-09-01.md) concludes that bounded published metadata cannot fill the unchanged protected shape for all three materials simultaneously. [Roadmap V15](plans/physical-sound-synthesis-roadmap-v15.md) keeps `4/1/1/1/1` for every admitted material, attempts Metal first, and adds Wood/Glass only through the identical frozen pipeline. S0 now resolves revision-aware identity/exposure and YCB capability before any waveform access; AV-MSF is a representation hypothesis, not an unpublished dependency. Raw-force transfer, runtime inference and production promotion remain separate blocked claims. |
