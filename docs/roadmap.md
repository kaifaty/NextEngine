...
| PhysX deterministic humanoid substrate | ADR-058/059/062/063/064/065/067/100 `Accepted`; immutable standing V1, bounded standing V2, flat-command V1 and curriculum V2 CPU environments exist, Stage 0 evidence remains incomplete | R8b starts with the applicable current Linux CPU preflight; CPU PhysX remains final evidence authority. Isaac may accelerate an experiment only after its exact descriptor/golden and reset/reward preflight. Full Stage 0/default promotion still requires the outstanding correspondence and Linux gates; Windows remains outside scope. |
...
ticks. V1 checkpoints are excluded from evaluation/resume. ADR-100 and
