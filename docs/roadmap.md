...
| Physical sound source synthesis | SPEC-45 `Proposed`; V9 strategy `EXPLICIT_MODAL_PARAMETERS / NEURAL_TIME_VARYING_NOISE_BAND_RESIDUAL / OFFLINE_BAKED_CLIP_ATLAS`; current status `R2_LISTENER_FIELD_REJECTED / R3A_V1–V5_NEGATIVE_OR_INCONCLUSIVE / R3A_V8_REAL_FIT_REJECTED / V9_SYNTHETIC_PASS / SOURCE_DISJOINT_REAL_PROTOCOL_NEXT / R3B_NOT_AUTHORIZED / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V9](plans/physical-sound-synthesis-roadmap.md), the [V8 exact rejection/research](development/physical-sound-r3a-v8-object91-fit-result-and-v9-residual-research-2026-08-31.md) and the [V9 exact synthetic result](development/physical-sound-r3a-v9-time-varying-residual-synthetic-result-2026-08-31.md). V9 known-truth renderer, compact records and held-contact neural field repeat and pass under `25,888/104` shared/contact bytes; this is not real quality. Select and freeze new internet-published, source-disjoint data with exact contact coordinates before any real decode; object `91` development `20`, sealed `27` and R3B remain closed. No holdout, baked atlas or runtime authority exists; authored clips remain authoritative. Production still requires a future concrete consumer and ADR-046 gate. |
