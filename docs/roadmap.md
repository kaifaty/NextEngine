...
| Physical sound source synthesis | SPEC-45 `Proposed`; V7 strategy `TASK_SPECIFIC_NEURAL_RATE_DISTORTION / CONTACT_TO_LATENT_FIELD / OFFLINE_BAKED_CLIP_ATLAS`; current status `R2_LISTENER_FIELD_REJECTED / R3A_V1_REPRESENTATION_REJECTED / R3A_V2_INCONCLUSIVE_RESAMPLING_CONTROL / R3A_V3B_NATIVE_NDAC_REJECTED / R3A_V4_FIT_REPRESENTATION_REJECTED / R3A_V5_TRAINING_CONTROLS_COMPLETE / THREE_CAPACITY_TRAINING_NEXT / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V7](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). V5 CUDA controls repeat manifest `75ed5e06…2679`, report `a4e30d87…b1e1` and checkpoint `74fccfac…7659` exactly; full loss/backward, active train RVQ and checkpoint continuation pass with zero development reads and zero real capacity steps. Exactly three frozen capacity runs are now authorized before a one-shot development selection. A later representation pass may authorize an external contact-to-latent field and offline baked clip atlas, never runtime neural inference. Production still requires a future concrete consumer and ADR-046 gate. |
