...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V10 keeps V9 `EXPLICIT_MODES / NEURAL_TIME_VARYING_RESIDUAL / OFFLINE_BAKED_CLIP_ATLAS`; current status `V9_SYNTHETIC_PASS / A0_REAL_SOURCE_PASS / A1_REJECTED_AT_ONSET / A1R_FORCE_SOURCE_PASS / A1R_FIT_NEXT / REAL_QUALITY_NOT_PROVEN / VALIDATOR_NOT_RELEASED / ADMISSION_SHADOW_SEALED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V10](plans/physical-sound-synthesis-roadmap.md), the [A1 exact result](development/physical-sound-r3a-v10-beer-glass-real-fit-result-2026-08-31.md) and the [A1R source freeze](development/physical-sound-r3a-v10-a1r-object51-force-source-freeze-2026-08-31.md). Two zero-decode A1R runs repeat manifest `3041c19d…6ed5` and bind object-disjoint `51 / Fruit_Bowl / Glass` raw force plus microphone to processed audio, coordinates and point cloud. Only four fit microphone/force pairs are authorized next; development `9` and sealed `18` remain closed. The object shares ObjectFolder project/archive family and grants no project-disjoint validation. No real quality, atlas or runtime authority exists; authored clips remain authoritative. Production requires a future concrete consumer and ADR-046 gate. |
