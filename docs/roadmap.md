...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V36](plans/physical-sound-synthesis-roadmap-v36.md) is `ACTIVE / R0_COMPLETE / A0_NEXT / FRESH_V36_TARGETS_UNOPENED / V35_QUALITY_UNOBSERVED / REAL_RELEASE_BLOCKED / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V35 D0](development/physical-sound-v35-d0-development-tournament-result-2026-09-03.md) repeat-exactly closes after opening all `6,480` train and `4,320` development targets: four models train, then the owner atomically reports `TypeError` at development prediction because `RoleData` has no implicit length. No metric, prediction or weight is published, so quality is unobserved; V35 roles are spent and H0 stays exact-zero unopened. V36 preserves every scientific knob on fresh roles but first requires one typed owner/provider path, mutation/access-order conformance and two full-count/full-step surrogate executions to create a byte-sealed D0 capability. Synthetic H0 and the independent real source/validator lane must both pass before Steel training; [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker real role six exact-Steel and 27 non-Metal groups short. |
