...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B and AV-P0C substrate `IMPLEMENTED`; current status `SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md): close the frozen shuffled-envelope blind spot, broaden exact-domain real families and pre-register grouped risk/coverage before AV-P0D. R&D remains PresentationOnly and non-blocking for R7. Production impact waits for an explicit post-v1 slot, concrete consumer, Accepted promoting ADR, complete engine-owned contact projection, cooked acoustic content/fallback and exact PCM/root checks; rolling/scraping remain later. |
