...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V46](plans/physical-sound-synthesis-roadmap-v46.md) is `ACTIVE / EXECUTION_ROADMAP / FOUNDATION_COMPLETE / D0_SOURCE_PREFLIGHT_NEXT / RESEARCH_ONLY / OFFLINE_ML / AUTHORED_FALLBACK`; no public schema, runtime model or production gate promoted | V46 keeps V45's repeat-exact source ledger, `189`-coordinate Recipe V3 and 36-group Clatter comparator, plus the V44 real baseline `1.312187716` and `71/105` supported real parents. It now organizes the remaining work as two converging tracks: a reproducible ML lab (D0/D1/B0/M0) and independent real-evidence/validator growth (E0/PSEL/B1/V0). Only one disclosed LabWinner may open one-shot A0 admission; only A0 Pass may open deterministic K0 cooking and an opt-in fallback-safe P0 demo. D0 bounded NISR/VibraVerse preflights are next, with no bulk acquisition or inferred power credit. V46 remains the planning authority until `MVP_PASS` or honest `MVP_FALLBACK_ONLY`; individual negative experiments produce result records rather than new roadmap versions. |
