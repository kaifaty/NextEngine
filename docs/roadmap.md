...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `PLANNED / NOT_ACTIVE`; SPEC-38/ADR-076 `Proposed`; original water W0B `INVALIDATED_BY_RC1`, W0C `IN_PROGRESS / INITIALIZATION_CANDIDATE_REJECTED`, W1 `IMPLEMENTED / BLOCKED_ON_W0C`; all ProductChecks `NOT_RUN` | Exact W0C diagnostics reject ceiling-only repair and `ghost-cell-shell-v1`. The independently reproduced `zero-velocity-settle-v1` generator then shows increasing iteration demand/penetration and its last diagnostic state fails the first production-ceiling step. Persistent-problem escalation now permits only a distinct analytical non-particle boundary discriminator; if it fails, reassess the track as `RESEARCH_ONLY`. Do not start W2 or integration. Main-roadmap activation still requires `CONTINUUM-WATER-REF-P1 = PASS`; promotion also requires exact exchange identity, one composition DAG/PhysX integration, scheduled epochs, worst-case admission, primary-session fault domain and successor combined budget. |
