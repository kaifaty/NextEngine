...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V43](plans/physical-sound-synthesis-roadmap-v43.md) is `ACTIVE / C1A_NEXT / C0_IDENTITY_REPAIR_REQUIRED / V0_BLOCKED_BY_IDENTITY_AND_POWER / M0_BLOCKED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | Official AV-MSF lineage and matching ObjectFolder IDs expose a physical-parent leak between current `generator_train` and `validator_calibration`; object `80` also has a `Wood`/`Ceramic` label conflict. C1A metadata-only audit and D2/C0R immutable role repair now precede descriptors, baselines, validator calibration and training. Old B0/R0 remain exact historical controls, but corrected B0R/R0R must re-freeze the global floor and power target. Fresh independent internet sources are required for V0; only a qualified validator, generator holdout and joint shadow may authorize deterministic cooking and the opt-in Steel demo, while every failure retains the authored fallback. |
