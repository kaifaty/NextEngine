...
| Physical sound source synthesis | SPEC-45 `Proposed`; V3 strategy `SELF_VALIDATING_NEURAL_TRANSFER_FIELD / OFFLINE_TRAINING / DETERMINISTIC_COOKED_RUNTIME`; current status `R0_R1_COMPLETE / R2C_REJECTED / R2D_V1_FIXED_STEP_REJECTED / R2D_V2_DECAY_NEXT / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V3](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). R2D V1 repeats byte-identically and proves the rank-96 basis, objective direction and cooker, but fails only small/full log-energy convergence. Next retain every V1 input/gate and replace fixed AdamW rate with one deterministic decay schedule; query remains closed. Only a passing R2D may authorize one context-basis coordinate-to-coefficient R2E field. The validator and admission shadow remain isolated; production still requires a future consumer/ADR gate. |
