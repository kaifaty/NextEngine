...
integer FK and is rejected/restored. R84 now tests the trial-point Jacobian
with no slack or limit change. No fresh PhysX or training is authorized while
...
7. **R5 humanoid movement TRAIN-4 clip-global prototype (`ACTIVE_R&D / CURRENT WIP`):**
   ...
   `cmu05`/`cmu16` and rejects `cmu139`; R74–R83 select hard trial-Jacobian
   restoration after old-Jacobian phase-I diverges from exact FK. До unchanged all-three
