...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, repeated real-method rejections and source-derived/Gabor common-pole synthetic controls `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_METHOD_DEVELOPMENT_CLOSED / IRON_SKILLET_METHOD_TRANSFER_REJECTED / FIXED_TAIL_TIMING_MISMATCH_SUPPORTED / BROADBAND_COMMON_POLE_SYNTHETIC_CONTROL_SUPPORTED / EXISTING_IRON_COUNTERFACTUAL_NEXT / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [broad-band control result](development/physical-sound-broadband-common-pole-control-result-ps2-2026-08-28.md). Repeated synthetic report `dcd83252…0094` passes all 15 discovery-to-resynthesis gates with 7 retained modes, maximum frequency/decay errors `0.095876 Hz/0.190671/s` and post-transient NRMSE `0.049273`. Next hash-close a read-only counterfactual on the already acquired Iron rows; no new payload/object, retry, mechanics or Planter. |
