...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration/causal audit and Ceramic Cup observation execution/diagnostic `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_CUP_SHARED_DECAY_MISMATCH_SUPPORTED / MULTIOUTPUT_SYNTHETIC_CONTROL_SUPPORTED / CERAMIC_COUNTERFACTUAL_FREEZE_NEXT / REAL_ANALYSIS_BLOCKED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [multi-output control result](development/physical-sound-multioutput-decay-control-result-ps2-2026-08-28.md). Repeated report `a099f50d…e8bea` passes the frozen synthetic counterexample with spatial/node decay fractions `1.0/0.0` and known-decay error `0.35979 dB/s`. Next hash-close a read-only all-microphone counterfactual over the existing Ceramic Cup fixed impact; do not execute before commit or fetch, select rows, run mechanics or access Planter. Clip fallback and all exact-domain, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
