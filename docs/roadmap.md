...
| Navigation/audio | Частично: neutral `NeutralAudioV1` clip contract + cook/activation plumbing (A1), audio scene contracts + deterministic extraction (A2), baseline software mixer + canonical PCM sink + production live wiring + AUDIO-02-style `audio-scene` check (A3) | Нет desktop audio device adapter (A4), subtitle fallback (A5), navigation cooker и baseline nav adapters. |
...
2. **Baseline audio (`NEXT`, A1–A3 `DONE_LOCAL_WINDOWS`):** clips, emitters/listener, priority/voice limits,
   ...
   byte-exact). A4 (desktop device adapter), A5 (subtitle fallback) и
