...
| Текущая точка | R1–R7 завершены на current Linux product boundary. [R7c Linux performance authority](development/task-state/r7c-linux-performance-authority.md) closed B-12 on exact commit `e48f2eac…`; [R7d final hardening](development/task-state/r7d-final-product-hardening.md) and [R7e distribution closure](development/task-state/r7e-linux-distribution-closure.md) close on exact code commit `919663ff…`. The final native target report is `PASS / release_ready=true` with all eight checks passing, and two independently built PackageManifest V6 trees are byte/mode-identical at manifest `47beaf91…`. R8a condition work is complete; [R8b first learned locomotion](development/task-state/r8b-first-learned-locomotion.md) is the selected post-v1 WIP. ADR-101 retires the 24-sphere/23-collinear-X Stage 0 body and admits anatomical biomechanics V2/material V3. Native PhysX preflight, exact Isaac generation/smoke and the 1,024,000-sample optimizer discriminator pass; predeclared `model_249.pt` reaches nominal timeout in five deterministic GPU evaluations. CPU policy evaluation, GPU contact-terminal closure and `MODEL-MIRROR-P1` remain open, so no learned/runtime authority is promoted. ADR-090 keeps Windows/THOTH/paired evidence outside current scope indefinitely. R141 remains `INVALID / STOP_NO_RETRY` without R142 or authority over R8b. |
...
FIRST_LEARNED_LOCOMOTION / STAGE0_V1_BODY_RETIRED /
BIOMECHANICS_GPU_STANDING_REPEATABILITY_PASS / CPU_ADMISSION_OPEN /
NO_AUTHORITY`. Product priority remains the
...
biomechanics BodySchema V2 and material V3; native PhysX now proves neutral
two-sole contact and its current reset/step path. Its closed 1,024,000-sample
GPU run produces `model_249.pt`, which reaches the nominal full timeout in all
five deterministic evaluations. These identical evaluations prove repeatable
nominal standing, not seed-dependent perturbation robustness or CPU authority.
...
| PhysX deterministic humanoid substrate | ADR-058/059/062/063/064/065/067/069/071/100/101 `Accepted`; immutable Stage 0 V1 environments remain exact toy/regression evidence; current biomechanics standing manifest `1b60550d…73bb` passes native two-sole and 32-tick reset/step/safety/reward preflight; its exact GPU generation/smoke, 1,024,000-sample run and five nominal timeout evaluations complete; Stage 0 product evidence remains incomplete | Preserve the GPU candidate, implement canonical CPU policy evaluation and paired `MODEL-MIRROR-P1`, and close GPU hard-impact/self-collision classification before any learned-standing or runtime claim. Do not start forward-command optimization yet. |
...
Stage 0 V1 body is stopped. ADR-101 now admits the distinct material-complete
biomechanics standing profile after native PhysX proves both sole contacts and
its 32-tick reset/step/safety/reward path. Exact generation/smoke and the
1,024,000-sample run complete; the predeclared final checkpoint reaches nominal
timeout in five deterministic GPU evaluations. Because those evaluations share
the same neutral reset and the direct GPU environment does not yet close hard-
impact/self-collision classification, the next bounded work is CPU policy
evaluation plus paired `MODEL-MIRROR-P1`, not walking or another optimizer run.
