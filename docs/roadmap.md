...
**Пакет R8c (в работе параллельно):** `ACTIVE /
CONTINUUM-WATER-VOLUME-P1=PASS / PRESENTATION_NEXT`. Under ADR-100 (Accepted 2026-09-03)
the reference scene owns one exact CPU water basin inside the physics world
checkpoint (SPEC-26 2.5): authored extent and level, a level command with
stable rejections, an exact submersion query, the player wading/swimming
HUD classification, a still basin surface that follows the authoritative
level through the ordinary presentation snapshot, and save/replay closure
proven by `xtask water-volume`, `play` and `persistence-replay`. The next
increment is the ADR-101 basin surface in the game root fed by the
presentation solver stream; the CUDA research tool stays outside the
workspace.
...
The selected post-v1 queue now contains two active packages:
**R8b first learned locomotion (`ACTIVE_R&D / PREFLIGHT_NEXT`)**, whose
durable resume surface is
[r8b-first-learned-locomotion.md](development/task-state/r8b-first-learned-locomotion.md)
and whose next action is the no-training identity/resource/correspondence
preflight (no optimizer run is authorized until it passes), and
**R8c authoritative water volume and presentation water
(`ACTIVE / CONTINUUM-WATER-VOLUME-P1=PASS / PRESENTATION_NEXT`)**, whose
resume surface is
[water-volume-authority.md](development/task-state/water-volume-authority.md)
and whose next increment is the solver-fed ADR-101 basin surface inside
the game root (player classification and the still surface are done). R8a fracture and embodiment
expansion is deferred, and the stopped R141 reference-tracking lineage
remains immutable and outside this queue.
