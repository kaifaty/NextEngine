...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V36](plans/physical-sound-synthesis-roadmap-v36.md) is `ACTIVE / R0_A0_COMPLETE / F0_NEXT / FRESH_V36_TARGETS_UNOPENED / V35_QUALITY_UNOBSERVED / REAL_RELEASE_BLOCKED / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V36 A0](development/physical-sound-v36-a0-typed-owner-contract-result-2026-09-03.md) freezes one immutable concrete role batch with explicit `row_count`, no implicit length, strict positive/negative type proof, 10 lifecycle mutations and complete D0/H0 stage/callable topology hashes. Four artifacts/stdout repeat exactly and every official/fresh target/model access stays zero. F0 must now copy every V35 scientific field unchanged into fresh material/geometry/contact/truth/role identities; C0/X0 and two full-count/full-step D0+H0 surrogate executions must bind the exact numeric owner before any official capability exists. Synthetic H0 and the independent real source/validator lane must both pass before Steel training; [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker real role six exact-Steel and 27 non-Metal groups short. |
