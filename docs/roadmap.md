...
| Текущая точка | R1–R7 завершены на current Linux product boundary. [R7c Linux performance authority](development/task-state/r7c-linux-performance-authority.md) closed B-12 on exact commit `e48f2eac…`; [R7d final hardening](development/task-state/r7d-final-product-hardening.md) and [R7e distribution closure](development/task-state/r7e-linux-distribution-closure.md) close on exact code commit `919663ff…`. The final native target report is `PASS / release_ready=true` with all eight checks passing, and two independently built PackageManifest V6 trees are byte/mode-identical at manifest `47beaf91…`. R8a condition work is complete; [R8b first learned locomotion](development/task-state/r8b-first-learned-locomotion.md) is the selected post-v1 WIP. Pair-complete CPU/GPU contact evaluation rejects the biomechanics V2 standing lineage: its final fresh policy self-collides at ticks `135/139`, and the neutral pelvis/forearm clearance is only `405 um`. ADR-102 selects separately identified biomechanics V3/standing V2 with `45,405 um` clearance and a source-conserving serial-carrier projection; its fresh standing optimizer run is next. No learned/runtime authority is promoted. ADR-090 keeps Windows/THOTH/paired evidence outside current scope indefinitely. R141 remains `INVALID / STOP_NO_RETRY` without R142 or authority over R8b. |
...
FIRST_LEARNED_LOCOMOTION / BODY_V3_SUCCESSOR_READY /
STANDING_RETRAIN_NEXT / NO_AUTHORITY`. Product priority remains the
...
source aggregate mass/inertia. Zero-residual controls now show no immediate
self-contact; the fresh V3 standing optimizer run is next.
...
| PhysX deterministic humanoid substrate | ADR-058/059/062/063/064/065/067/069/071/100/101/102 `Accepted`; immutable Stage 0 V1 and biomechanics V2/standing V1 environments remain negative evidence; current biomechanics V3/standing V2 successor removes the immediate neutral self-contact in CPU/GPU controls; Stage 0 product evidence remains incomplete | Run one fresh hash-closed V3 standing discriminator, then evaluate its predeclared final checkpoint on GPU and canonical CPU before paired `MODEL-MIRROR-P1`. Do not start forward-command optimization yet. |
...
BODY_V3_SUCCESSOR_READY / STANDING_RETRAIN_NEXT / NO_AUTHORITY`)**. Its durable
resume surface is
...
carrier projection. CPU and Isaac zero-residual controls no longer have
immediate self-contact. The next bounded work is one fresh 250-iteration V3
standing run followed by GPU and CPU final-checkpoint evaluation, not walking.
