...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V45](plans/physical-sound-synthesis-roadmap-v45.md) is `ACTIVE / R0_T0_C0_COMPLETE / CLATTER_84_LABELS_36_UNIQUE_PRIORS / S0_NEXT / 10_SOURCES / ZERO_NEW_POWER_CREDIT / 71_REAL_ACOUSTIC_PARENTS / DEFICIT_34 / PSEL_BLOCKED / REAL_TRAINING_BLOCKED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, runtime model or production gate promoted | [R0](development/physical-sound-v45-r0-source-claim-ledger-result-2026-09-03.md) partitions ten sources with zero external access or power credit. [T0](development/physical-sound-v45-t0-recipe-v3-result-2026-09-03.md) freezes the value-free `189`-coordinate Recipe V3 and masks twice exactly. [C0](development/physical-sound-v45-c0-clatter-external-control-result-2026-09-03.md) pre-access-seals Clatter, then reproduces 84 external priors/WAVs twice exactly at tree root `90019ff5…2514`; the labels contain only 36 unique modal priors and remain baseline-only. The V44 real baseline remains `1.312187716`; G0B1c1 remains `71/105`, deficit `34`, and one IETeasy project cannot close PSEL. S0 bounded NISR/VibraVerse preflights are next while second-real-project growth continues. Real fitting still waits for PSEL/B1 and a frozen independent validator; only one-shot protected admission plus deterministic cooking may authorize an opt-in prop, and every failure retains the authored fallback. |
