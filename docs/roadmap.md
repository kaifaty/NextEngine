...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V28](plans/physical-sound-synthesis-roadmap-v28.md) status `ADOPTED / H0_COMPLETE / R0_PROTOCOL_FROZEN / R1_IMPLEMENTATION_NEXT / Q0_SIGNAL_BLIND_READY / M0B_SPENT`; no public schema, content role, runtime model or production gate promoted | [R0 protocol](development/physical-sound-v28-r0-m0c-prefix-resource-protocol-2026-09-02.md) `28cab428…9000` freezes M0c's sole `144,000 → 4,096` training delta, exact equivalence/complete-entry gates and two isolated 10,000-step runs under `600 s`/`3.5 GiB`. Official values remain sealed until R1 passes; Q0 validator inventory may proceed independently. Metal admission, cooker/demo and Glass/Wood remain blocked; authored fallback stays authoritative. |
