...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V41](plans/physical-sound-synthesis-roadmap-v41.md) is `ACTIVE / D0_COMPLETE / C0_NEXT / DISCLOSED_LAB_FIRST / PROTECTED_ADMISSION_SOURCE_POWER_OOD / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | The repeat-exact [D0 roster](development/physical-sound-v40-d0-disclosed-roster-result-2026-09-03.md) permanently assigns all nine exposed families as `5 generator-train / 2 generator-development / 2 validator-calibration`, with whole-family commitments, no clean-family spend and zero protected access. V41 now separates an iterative disclosed lab from one-shot independent admission: C0/B0/V0/M0/M1 may produce an external `LabWinner` or `NoCandidate`, while metadata-only S0 continues against exact deficits `13/34` and `13/31`. Only qualified validator, generator holdout and joint shadow may authorize deterministic cooking and the opt-in Steel demo; every failure retains the authored fallback. |
