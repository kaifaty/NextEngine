...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V30](plans/physical-sound-synthesis-roadmap-v30.md) status `ADOPTED / E2_REPEAT_EXACT_ZERO_SIGNAL / E3_WHOLE_PROJECT_POWER_OOD / E4_GAP_DIRECTED_NEXT / VALIDATOR_FIRST / PHYSICS_LOCKED_ML / INTERNET_ONLY / NO_PER_SOUND_HUMAN_REVIEW / CLIP_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) captures all frozen public metadata and audits twice byte-exactly with zero signal access. Q1a adds `8` projects, `9` exact-Steel and `3` non-Metal groups; combined raw power is `10 / 32 / 73`, but whole-project allocation is OOD because the best second protected role still lacks `6` exact-Steel and `27` non-Metal groups while five projects stay reserved. E4 now targets only those deficits; Q1R, validator and disclosed-real training remain blocked. P0 classical/physics-locked work may proceed independently; authored fallback remains authoritative. |
