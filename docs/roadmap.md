...
| Physical sound source synthesis | SPEC-45 `Proposed`; V7 strategy `TASK_SPECIFIC_NEURAL_RATE_DISTORTION / CONTACT_TO_LATENT_FIELD / OFFLINE_BAKED_CLIP_ATLAS`; current status `R2_LISTENER_FIELD_REJECTED / R3A_V1_REPRESENTATION_REJECTED / R3A_V2_INCONCLUSIVE_RESAMPLING_CONTROL / R3A_V3B_NATIVE_NDAC_REJECTED / R3A_V4_FIT_REPRESENTATION_REJECTED / R3A_V5_PREFLIGHT_A_COMPLETE / TRAINING_ENVIRONMENT_FREEZE_NEXT / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V7](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). V5 preflight A repeats manifest `1cc23496…fb31` and report `38176a41…3bf3`, freezing a 73-clip/17-group external corpus inventory, three task-specific RVQ capacities and deterministic CPU controls with zero real training or development reads. Next freeze the GPU environment and runner controls; only then may three capacity runs begin. A later representation pass may authorize an external contact-to-latent field and offline baked clip atlas, never runtime neural inference. Production still requires a future concrete consumer and ADR-046 gate. |
