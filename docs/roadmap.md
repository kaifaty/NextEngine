...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `PLANNED / NOT_ACTIVE`; SPEC-38/ADR-076 `Proposed`; water `RESEARCH_ONLY`: original W0B `INVALIDATED_BY_RC1`, W0C/W0D closed, W0E local survivor, W0F solver/geometry roots frozen, W0G impact-energy roots frozen, W1 `IMPLEMENTED / LINUX_CORPUS_IN_PROGRESS / REFERENCES_PENDING`; all ProductChecks `NOT_RUN` | W0F binds shared integer outer/internal geometry, two-layer density support, swept contact and a `32,768` static capacity. W1 then falsified the inherited two-sided energy gate for explicitly inelastic impact; W0G keeps W0F operations unchanged, retains absolute drift for equilibrium/control scenarios and blocks positive energy excess for named impact cases while publishing deficit/stage accounting. Linux dam-break completes `720/720` internally with zero excess but remains reference-pending. Windows is outside current W1 scope and deferred, not waived, for promotion. Do not start W2 or integration before W1 PASS. Independent terrain closure remains Package 10T. Main-roadmap activation still requires `CONTINUUM-WATER-REF-P1 = PASS`; promotion also requires exact exchange identity, one composition DAG/PhysX integration, scheduled epochs, worst-case admission, primary-session fault domain and successor combined budget. |
