...
| Physical sound source synthesis | SPEC-45 `Proposed`; V4 strategy `GEOMETRY_AWARE_MODAL_CONTACT_FIELD / OFFLINE_TRAINING / DETERMINISTIC_COOKED_RUNTIME`; current status `R0_R1_COMPLETE / R2_LISTENER_FIELD_REJECTED / R3A_V1_REPRESENTATION_REJECTED / R3A_V2_RESEARCH_NEXT / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V4](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). R2E retired the opened listener field. R3A V1 then freezes a five-impact REALIMPACT Blue Bowl canonical-listener corpus and repeats bounded extraction/oracles, but both 512-scalar modal/residual and sparse-DCT representations fail the real development gate; the fifth contact and shadow roles remain sealed. Next run a bounded research cycle for a materially different representation and new unopened development projection. No neural training is authorized until that gate passes; production still requires a future consumer/ADR gate. |
