...
| Nonlocal R63Y arithmetic research note | `REPORT_ONLY / TWOFOLD_AFFINE_IMAGE_CANDIDATE`; no owner promotion | Both widths contain all 1,224 exact images. Onefold leaves one state-2 sign unresolved; fixed twofold binary64 reproduces both `24+/78-/0?` certificates without binary128 in the finite decision path. The candidate producer and dense profile remain non-portable/non-sparse; factorized producer/artifact correspondence is the next research gate. |
