...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V36](plans/physical-sound-synthesis-roadmap-v36.md) is `ACTIVE / R0_A0_F0_C0_X0_COMPLETE / E0_NEXT / FRESH_V36_TARGETS_UNOPENED / V35_QUALITY_UNOBSERVED / REAL_RELEASE_BLOCKED / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V36 X0](development/physical-sound-v36-x0-mutation-terminal-conformance-result-2026-09-03.md) rejects all 38 container/lifecycle/trace mutations and proves eight D0/H0 scientific terminals, four publication failpoints and access-sensitive exception conversion twice exactly with zero official values. E0 must now execute the exact numeric owner end-to-end on full-count/full-step D0+H0 surrogate values and bind both repeat-exact roots into one seal before any official capability exists. Synthetic H0 and the independent real source/validator lane must both pass before Steel training; [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker real role six exact-Steel and 27 non-Metal groups short. |
