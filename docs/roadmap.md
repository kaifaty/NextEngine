...
| Navigation/audio | Частично: neutral `NeutralAudioV1` clip contract + cook/activation plumbing (A1), audio scene contracts + deterministic extraction (A2), baseline software mixer + canonical PCM sink + production live wiring + AUDIO-02-style `audio-scene` check (A3), SDL playback stream + bounded ring + device-loss counters в desktop adapter (A4) | Нет subtitle fallback (A5), checks closure (A6), navigation cooker и baseline nav adapters. |
...
2. **Baseline audio (`NEXT`, A1–A4 `DONE_LOCAL_WINDOWS`):** clips, emitters/listener, priority/voice limits,
   ...
   `NOT_RUN` headlessly). A5 (subtitle fallback) и
