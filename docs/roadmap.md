...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V18](plans/physical-sound-synthesis-roadmap-v18.md) status `V17_G0_CLOSED_NO_COMPLEXITY_MARGIN / P0B_PROTOCOL_NEXT / SOURCE_GROWTH_PARALLEL / ADMISSION_BLOCKED / METAL_FIRST / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | [V17 G0](development/physical-sound-v17-g0-scale-separated-global-oracle-result-2026-09-01.md) passes absolute quality, raw-dimensional scale transfer, all 72 mutations and exact repeat, but its neural head misses the required complexity margin over degree-two ridge. V18 therefore verifies an unchanged deterministic scale-separated scaffold on fresh rows and reserves ML for the intrinsic contact field. [S0c](development/physical-sound-v15-s0c-source-sufficiency-role-freeze-result-2026-09-01.md) independently keeps real admission blocked under unchanged `4/1/1/1/1`; runtime inference and production promotion remain separately blocked. |
