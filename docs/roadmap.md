...
| Текущая точка | R1–R7 завершены на current Linux product boundary. R8a condition work is complete; [R8b first learned locomotion](development/task-state/r8b-first-learned-locomotion.md) is the selected post-v1 WIP. Biomechanics V3 standing passes one complete contact-correct GPU and five canonical CPU PhysX episodes. [Paired mirror investigation](development/r8b-model-mirror-p1-investigation-2026-09-04.md) records `MODEL-MIRROR-P1 FAILED`. Walking V1/V2 are rejected; ADR-105 admits one V3 dense-tracking counterfactual with all other experiment boundaries unchanged. CPU evaluation and the unchanged mirror gate still block runtime promotion and broader commands. ADR-090 keeps Windows outside current scope indefinitely. R141 remains `INVALID / STOP_NO_RETRY` without authority over R8b. |
...
| R8. Post-v1 tracks | `IN_PROGRESS / R8B_FIRST_LEARNED_LOCOMOTION_SELECTED` | отдельные программы | R8a closes the bounded intact-topology condition/capability subset. R8b standing passes; walking V1/V2 are rejected for no meaningful travel. ADR-105 selects one dense-tracking V3 counterfactual after exact no-training controls; mirror repair remains independent. Other optional tracks remain gated. |
...
R&D_ONLY_WALKING_V1_FAILED / WALKING_V2_AUTHORIZED / MODEL_MIRROR_P1_FAILED /
...
and no saved CPU checkpoint walks. ADR-105 admits one exact V3 optimizer budget
that changes only the zero-gradient compact tracking kernel to a dense bounded
kernel. R8b remains a
...
| PhysX deterministic humanoid substrate | ADR-058/059/062/063/064/065/067/069/071/100/101/102/103/104/105 `Accepted`; biomechanics V3/standing V2 passes its nominal GPU/CPU standing gate; walking V1/V2 are rejected; paired V3 `MODEL-MIRROR-P1` fails fixed joint/root-velocity bounds | Run the one ADR-105 dense-tracking V3 discriminator only after its no-training controls. Broader curriculum and runtime promotion remain blocked until the full CPU quality/correspondence matrix passes. |
...
CPU checkpoint. ADR-105 selects one exact dense-tracking V3 counterfactual;
PPO/body/controller/safety and the fixed start/stop schedule remain
unchanged. The ceiling remains `NO_AUTHORITY`, and mirror repair remains an
independent promotion gate. R8a fracture and embodiment
