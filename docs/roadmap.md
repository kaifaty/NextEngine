...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1, PS-2 plan/pilot/`E1` import, bounded internet source/cache/archive/Ogg/RAR5 handling, eight E3 project sources, explicit target/reject-parent roles, five typed REALIMPACT E2 rows, exact-domain/source gates, V1/V2 transfer calibration, listener acquisition, spatial-representation rejection chain and analytical boundary-solver harness `IMPLEMENTED`; current status `PS-2_BEM_ANALYTICAL_CONTROL_REFINEMENT_REJECTED / FOUR_OF_FIVE_NUMERIC_GATES_PASS / NO_FRESH_DATA_OPENED / TWO_HOLDOUT_OBJECTS_PRESERVED / INDEPENDENT_SYNTHETIC_CONTROL_NEXT / THREE_DIMENSIONAL_SPATIAL_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / FALLBACK_OUT_OF_DOMAIN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md), [internet corpus policy](development/physical-sound-internet-corpus-policy-ps2-2026-08-27.md) and [analytical boundary-solver evidence](development/physical-sound-bem-analytical-control-ps2-2026-08-28.md). Local capture is retired. The 320-panel synthetic sphere passes absolute complex (`2.1251%`), magnitude (`0.1827 dB`), phase (`0.2993°`) and direction (`0.0002 dB`) gates and repeats at `6f74a309…a689`, but its median error is `2.8589x` the 80-panel result against `<=0.8x`; no converged BEM/FFAT oracle credit exists. Open no fresh data. Next distinguish geometry/quadrature error from a boundary-equation defect on synthetic or independently solved evidence. All eight exact-domain blockers, 3D spatial credit, corpus admission, `Pass`, PS-3, AV-P0D and production P1 remain disabled. |
