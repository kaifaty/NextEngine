...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V32](plans/physical-sound-synthesis-roadmap-v32.md) status `ADOPTED / P0_P1_COMPLETE / T0_NEXT / VALIDATOR_FIRST / BOUNDED_SOURCE_SCOUTING / REAL_RELEASE_BLOCKED_BY_SOURCE_POWER / OFFLINE_ML_ONLY / NO_PER_SOUND_HUMAN_REVIEW / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [P1](development/physical-sound-v31-p1-deterministic-modal-owner-result-2026-09-02.md) emits two byte-identical 56-file synthetic modal/render trees: V24 analytic equivalence, nine remesh pairs, energy bounds, seven isolated interventions and six typed fallback probes pass with zero signal/model/network access. [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the best second protected role six exact-Steel and 27 non-Metal groups short. V32 makes T0→V0→M0/M1 the immediate automatic validation/ML path while bounded source scouting proceeds independently; real calibration, training, admission and cooking remain sealed. |
