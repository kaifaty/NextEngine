...
| Physical sound source synthesis | SPEC-45 `Proposed`; V8 strategy `EXPLICIT_MODAL_PARAMETERS / NEURAL_CONTACT_TO_MODE_SHAPE_FIELD / BOUNDED_RESIDUAL / OFFLINE_BAKED_CLIP_ATLAS`; current status `R2_LISTENER_FIELD_REJECTED / R3A_V1–V5_NEGATIVE_OR_INCONCLUSIVE / R3A_V8_SYNTHETIC_PREFLIGHT_NEXT / R3B_NOT_AUTHORIZED / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V8](plans/physical-sound-synthesis-roadmap.md), its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md) and the [V8 rebaseline](development/physical-sound-v8-explicit-modal-neural-rebaseline-2026-08-31.md). V5 remains rejected on fresh real development. V8 first tests deterministic damped-mode recovery and a coordinate-to-3D-mode-shape MLP on two opened exact-hash NISR FEM label files; this is synthetic substrate evidence only. No fresh real V8 object, representation holdout, real contact field or baked atlas is authorized, and authored clips remain authoritative. A V8-SYNTH pass may only freeze a new ObjectFolder Real protocol. Production still requires a future concrete consumer and ADR-046 gate. |
