...
| Physical sound source synthesis | SPEC-45 `Proposed`; V2 roadmap strategy `NEURAL_TRANSFER_FIELD / OFFLINE_TRAINING / DETERMINISTIC_COOKED_RUNTIME`; Q30/DCT, FEM/BEM and previous real-data work remain baselines, controls and negative knowledge; current status `R0_COMPLETE / R1_TRANSFER_CONTROLS_FROZEN / R2_TIME_DOMAIN_FAMILY_REJECTED / R2B_DENSE_COMPLEX_FIELD_DATA_READY / R2C_COMPLEX_TRAINING_NEXT / ADMISSION_SHADOW_SEALED / SELECTIVE_PASS_DISABLED / AUTHORED_CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V2](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). Direct and phase-aligned R2 fields remain frozen rejections. The full 600-position R2B acquisition, grouped split, complex STFT inverse and three controls now repeat byte-identically with `420 context / 180 query`, context-only normalization and no sealed-role reads. This is data/representation readiness only. Next freeze and repeat exactly the preregistered data-only versus `0.0001` Helmholtz complex-pressure MLP ablation, then evaluate once against all controls and five unchanged endpoints. The independent validator and admission shadow remain isolated; production can consume only a bounded cooked record after a future consumer/ADR gate. |
