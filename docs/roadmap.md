...
| Physical sound source synthesis | SPEC-45 `Proposed`; V2 roadmap strategy `NEURAL_TRANSFER_FIELD / OFFLINE_TRAINING / DETERMINISTIC_COOKED_RUNTIME`; Q30/DCT, FEM/BEM and previous real-data work remain baselines, controls and negative knowledge; current status `R0_COMPLETE / R1_TRANSFER_CONTROLS_FROZEN / R2_TIME_DOMAIN_FAMILY_REJECTED / R2B_DENSE_COMPLEX_FIELD_DATA_NEXT / ADMISSION_SHADOW_SEALED / SELECTIVE_PASS_DISABLED / AUTHORED_CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V2](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). Direct and phase-aligned R2 PyTorch/MLflow fields reproduce but fail the unchanged conjunctive Rust-metric rule; a query-informed diagnostic also rejects the opened time-domain latent span. Do not tune that family on opened queries. Next project the full published 600-position fixed-impact semicylinder, freeze grouped spatial splits and a complex time-frequency representation, and measure data sufficiency/controls before another optimizer. The independent validator and admission shadow remain isolated; production can consume only a bounded cooked record after a future consumer/ADR gate. |
