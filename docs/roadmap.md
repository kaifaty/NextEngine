...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V45](plans/physical-sound-synthesis-roadmap-v45.md) is `ACTIVE / R0_SOURCE_CLAIM_LEDGER_COMPLETE / T0_RECIPE_V3_NEXT / 10_SOURCES / ZERO_NEW_POWER_CREDIT / 71_REAL_ACOUSTIC_PARENTS / DEFICIT_34 / PSEL_BLOCKED / REAL_TRAINING_BLOCKED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, runtime model or production gate promoted | [R0](development/physical-sound-v45-r0-source-claim-ledger-result-2026-09-03.md) repeat-exactly partitions Clatter, NISR, VibraVerse, TU Delft/cello, CMU/three-object and control/excluded sources across claim masks. All eight forbidden access counters, new physical-parent/project credit and protected prospects are zero. The V44 real baseline remains `1.312187716`; G0B1c1 remains `71/105`, deficit `34`, and one IETeasy project cannot close PSEL. T0 Recipe V3 is next while second-real-project growth continues in parallel. Real fitting still waits for PSEL/B1 and a frozen independent validator; only one-shot protected admission plus deterministic cooking may authorize an opt-in prop, and every failure retains the authored fallback. |
