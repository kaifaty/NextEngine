...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V38](plans/physical-sound-synthesis-roadmap-v38.md) is `ADOPTED / S0_IMPROVED_FRONTIER_6_STEEL_23_NON_METAL / H0_PERMANENTLY_CLOSED / DATA_AND_VALIDATOR_FIRST / OFFLINE_ML_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V38 S0](development/physical-sound-v38-s0-gap-directed-source-result-2026-09-03.md) admits one DOI-versioned IETeasy metadata project with four non-Metal physical sample groups and zero exact-Steel credit. The unchanged whole-project planner improves the best second-role deficit from `6/27` to `6/23` while preserving five reserved projects, but remains infeasible; no payload or role opens. Another bounded metadata batch and target-free harness work are next. After role-power closure, independent validator qualification and a bounded disclosed-development generator bake-off precede one-shot method holdout, joint admission, deterministic cooking and demo fallback. |
