...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration/causal audit and Ceramic Cup observation execution/diagnostic `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / MULTIOUTPUT_SYNTHETIC_CONTROL_SUPPORTED / CERAMIC_MULTIOUTPUT_COUNTERFACTUAL_REJECTED / INCOMPATIBLE_IMPACTS_REJECTED / FIXED_WINDOW_ASSUMPTION_UNSUPPORTED / ADAPTIVE_DECAY_SYNTHETIC_CONTROL_NEXT / REAL_REUSE_BLOCKED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [Ceramic counterfactual result](development/physical-sound-ceramic-cup-multioutput-counterfactual-result-ps2-2026-08-28.md). Repeated report `9947c427…96cbf` rejects spatial aggregation (`0.1875` decay versus reference `0.25`). Primary sources confirm synchronized microphones but reveal an adaptive bandpass/RMS-envelope fit rather than V2's fixed interval. Freeze a synthetic source-faithful adaptive-decay control before real reuse; no cutoff, fetch, admission, mechanics or Planter access. Clip fallback and all exact-domain, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
