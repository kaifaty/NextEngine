...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `ACTIVE_ISOLATED_RESEARCH / NPR0_PROFILE_BRIDGE / INTEGRATION_BLOCKED`; SPEC-38/ADR-076 `Proposed`; water W1 `LINUX_W1_PASS / CONTINUUM-WATER-REF-P1=PASS` at `fe223f9`; CPU/direct-GPU W2 miss; Nonlocal performance reclosure `NONLOCAL_50K_FIXED_WORK_RECLOSURE_CANDIDATE / NO_W2_CREDIT`; later ProductChecks `NOT_RUN` | W0F/G/H/I roots remain frozen and W3 remains blocked. [The performance decision](development/nonlocal-continuum-performance-decision-2026-08-20.md) retains exact fused owner-term traversal plus compact `u16` CSR and passes the standalone 50k `4/6 ms` gate. The new [productionization roadmap](plans/nonlocal-continuum-production/README.md) starts NPR0 because the fast fixture is not the product basin: spacing differs `10×`, cadence `4.1667×`, support is `3dx` versus `2dx`, lattice axes differ and the benchmark has no sealed boundary. NPR0 isolates scale, cadence and support under new identities before coefficient/boundary selection; NPR1 correctness and NPR2 authority precede any private integration. This does not close W2/W3, grant GPU authority or change runtime/public contracts. Promotion still requires physical reclosure, an explicit authority decision, one composition DAG/PhysX integration, scheduled epochs, admission/fault semantics, exact persistence, the successor combined budget and deferred cross-target evidence. Independent terrain closure remains Package 10T. |
