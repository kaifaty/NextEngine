...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration/causal audit and Ceramic Cup observation execution/diagnostic `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_MULTIOUTPUT_COUNTERFACTUAL_REJECTED / FIXED_WINDOW_ASSUMPTION_UNSUPPORTED / ADAPTIVE_DECAY_SYNTHETIC_CONTROL_FROZEN / TWO_SYNTHETIC_RUNS_AUTHORIZED / REAL_REUSE_BLOCKED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [adaptive control preflight](development/physical-sound-adaptive-decay-control-preflight-ps2-2026-08-28.md). Manifest `926921e2…cedf` and repeated preflight `f51e513a…7b49` bind delayed/noisy known truth, source-derived adaptive RMS fit and a fixed-window comparator. Execute twice; any failure rejects. Real reuse, cutoffs, admission, mechanics and Planter remain blocked. Clip fallback and all exact-domain, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
