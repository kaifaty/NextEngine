...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1 and PS-2 plan/pilot/`E1` import bundle `IMPLEMENTED`; current status `PS-2_INTERNET_CORPUS_PIPELINE_NEXT / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [internet corpus policy](development/physical-sound-internet-corpus-policy-ps2-2026-08-27.md): user/local physical capture is retired and force hardware is not a blocker. Build an official-source registry, bounded external fetch/cache, format adapters and claim-scoped `E1` synchronized / `E2` transfer / `E3` identified-real / `E4` synthetic capability matrix. REALIMPACT remains useful `E2` evidence without invented missing axes. Re-plan internet coverage for at least 35 reject parents and 16 in-domain groups before PS-3/AV-P0D; insufficient published evidence remains fallback-only. R&D stays PresentationOnly and non-blocking for R7; production P1 gates are unchanged. |
