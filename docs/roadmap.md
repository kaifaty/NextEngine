...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration/causal audit and Ceramic Cup observation execution `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_CUP_OBSERVATION_REJECTED / REPEATED_DECAY_GATE_FAILURE / OFFLINE_CAUSAL_DIAGNOSTIC_NEXT / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [Ceramic Cup result](development/physical-sound-realimpact-ceramic-cup-observation-result-ps2-2026-08-28.md). Report `56591bb8…3fd9` repeats: four unchanged V2 gates pass, decay is `0.25 < 0.50`, and `13/16` modes are below `500 Hz`. Pitcher failed the same gate at `0.4375` with `11/16` sub-`500 Hz` modes. This blocks another object and mechanics; run one frozen offline listener-axis diagnostic on existing Ceramic rows, keep clip fallback and Planter sealed. All exact-domain, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
