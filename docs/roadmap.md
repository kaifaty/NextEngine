...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1, PS-2 plan/pilot/`E1` import bundle and internet source registry/cache/capability audit `IMPLEMENTED`; current status `PS-2_SOURCE_REGISTRY_IMPLEMENTED / PS-2_REAL_ADAPTER_NEXT / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md), [internet corpus policy](development/physical-sound-internet-corpus-policy-ps2-2026-08-27.md) and [source/cache evidence](development/physical-sound-internet-source-pipeline-ps2-2026-08-27.md): user/local physical capture is retired and force hardware is not a blocker. Bounded HTTPS fetch, external content-addressed cache and artifact-backed `E1`–`E4` accounting now fail closed. ObjectFolder synthetic lineage is `E4`-ready; the 36.37 GB ObjectFolder-Real archive remains discovery-only because no publisher SHA-256 is available and was not downloaded. Implement one bounded hash-closed real-source `E2`/`E3` adapter, then re-plan coverage for at least 35 reject parents and 16 in-domain groups before PS-3/AV-P0D; insufficient published evidence remains fallback-only. R&D stays PresentationOnly and non-blocking for R7; production P1 gates are unchanged. |
