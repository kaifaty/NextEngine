...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V46](plans/physical-sound-synthesis-roadmap-v46.md) is `ACTIVE / EXECUTION_ROADMAP / FOUNDATION_D1_COMPLETE / CLAIM_MASKED_CORPUS_INDEX / B0_BASELINE_NEXT / RESEARCH_ONLY / OFFLINE_ML / AUTHORED_FALLBACK`; no public schema, runtime model or production gate promoted | V46 keeps V45's repeat-exact source ledger, `189`-coordinate Recipe V3 and 36-group Clatter comparator, plus the V44 real baseline `1.312187716` and `71/105` supported real parents. [D0](development/physical-sound-v46-d0-synthetic-source-preflight-result-2026-09-03.md) rejects NISR and VibraVerse before payload. [D1](development/physical-sound-v46-d1-corpus-compiler-result-2026-09-03.md) now repeat-exactly compiles 154 real rows, 80 raw parents, 10 projects and 36 Clatter groups into a claim-masked index, proves role/project/parent isolation and leaves all 308 referenced content objects unopened. B0 next compares only compatible analytic/global/retrieval/ridge/Clatter controls. Independent real-evidence/validator growth (E0/PSEL/B1/V0) continues; only one disclosed LabWinner may open one-shot A0 admission, and only A0 Pass may open deterministic K0 cooking plus an opt-in fallback-safe P0 demo. |
