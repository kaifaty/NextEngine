...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration/causal audit and Ceramic Cup observation execution/diagnostic `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_CUP_SHARED_DECAY_MISMATCH_SUPPORTED / LISTENER_LOCAL_REJECTED / SIMPLE_LOW_FREQUENCY_CAUSE_REJECTED / MULTIOUTPUT_SYNTHETIC_CONTROL_NEXT / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [diagnostic result](development/physical-sound-ceramic-cup-observation-diagnostic-result-ps2-2026-08-28.md). Report `47b578ac…2603` finds shared failure (`23/27`) and rejects both listener-local and simple sub-`500 Hz` causes. Do not select a passing row, filter, fetch or run mechanics. Freeze a synthetic 15-output spatial-energy decay estimator with known modal truth and a node-contaminated single-output control; keep clip fallback and Planter sealed. All exact-domain, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
