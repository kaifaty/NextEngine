...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V40](plans/physical-sound-synthesis-roadmap-v40.md) is `ACTIVE / D0_COMPLETE / C0_NEXT / S0_CLEAN_GROWTH_OPEN_SOURCE_POWER_OOD / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | The repeat-exact [D0 roster](development/physical-sound-v40-d0-disclosed-roster-result-2026-09-03.md) permanently assigns all nine exposed families as `5 generator-train / 2 generator-development / 2 validator-calibration`, with whole-family commitments, no clean-family spend and zero protected access. C0 may now build only this disclosed external corpus while metadata-only S0 continues against I0's exact clean deficits `13/34` and `13/31`. Model training still waits for C0/B0 lineage and baselines; independent validator qualification, method holdout and joint shadow gate deterministic cooking and the opt-in Steel demo. Every failure retains the authored fallback. |
