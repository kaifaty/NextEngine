...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `ACTIVE_ISOLATED_RESEARCH / INTEGRATION_BLOCKED`; SPEC-38/ADR-076 `Proposed`; water `RESEARCH_ONLY`: W1 `LINUX_W1_PASS / CONTINUUM-WATER-REF-P1=PASS` merged at `fe223f9`; current CPU and direct-GPU W2 profiles miss performance; Nonlocal `NR1_RECLOSED_GATHER_DIRECTED / NR2_O2_RETAINED_TERM_SPECIALIZATION / O3_SEGMENTED_NUMERIC_MISMATCH / O4_SPECIFICATION_NEXT / NO_W2_CREDIT`; later ProductChecks `NOT_RUN` | W0F/G/H/I geometry, energy, pressure and reference roots remain frozen. The clean W1 corpus passes, but the eight-worker CPU mean is `269.643 ms` and the optimistic direct GPU proxy p95 is `37.398 ms` `f64` / `18.015 ms` mixed before omitted stages, so W3 integration remains blocked. The original [Nonlocal NR1 evidence](development/nonlocal-continuum-nr1-baseline-evidence-2026-08-19.md) remains a stiff-surface atomic mismatch. [NR1-RC1](development/nonlocal-continuum-nr1-rc1-evidence-2026-08-19.md) separately reclosed owner-only directed gather; [NR2-O1](development/nonlocal-continuum-nr2-o1-evidence-2026-08-20.md) retains an exact pointer-swap handoff; and [NR2-O2](development/nonlocal-continuum-nr2-o2-evidence-2026-08-20.md) retains exact viscosity specialization with unchanged memory, adjacent p95 `3.595 ms` water-48k / `6.608 ms` viscous-16k and same-process profiler attribution. [NR2-O3](development/nonlocal-continuum-nr2-o3-evidence-2026-08-20.md) rejects unique-pair segmented endpoint pre-addition at stiff-surface i2 gather correspondence despite exact repeatability/reverse/work/capacity controls; no O3 timing or speedup is credited, and O4 locality specification remains on retained gather. The `8 ms` cutoff remains research-only and cannot inherit DFSPH roots, change GPU authority or reduce the `4/6 ms` production gate. Independent terrain closure remains Package 10T. Promotion still requires a new explicit solver/profile decision, exact cross-target and exchange identity, one composition DAG/PhysX integration, scheduled epochs, worst-case admission, primary-session fault domain and successor combined budget. |
