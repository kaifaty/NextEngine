...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1 consensus, PS-2 corpus-plan and real-inventory pilot `IMPLEMENTED`; current status `PS-2_PILOT_COMPLETE / PS-2_FULL_ACQUISITION_NEXT / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md): exact axes, split keys, OOD policy and numeric grouped risk/coverage are pre-registered. One 48 kHz REALIMPACT GlassGoblet transfer now passes hash/finite/leakage ingestion but correctly falls back because the downloadable archive lacks force-profile, composition, repeat and support-fixture revisions; it gives no release credit. The frozen full plan still needs 40 real objects, 35 reject parents and 16 coverage groups before PS-3 or AV-P0D. R&D remains PresentationOnly and non-blocking for R7. Production impact still waits for an explicit post-v1 slot, concrete consumer, Accepted promoting ADR, complete engine-owned contact projection, cooked acoustic content/fallback and exact PCM/root checks; rolling/scraping remain later. |
