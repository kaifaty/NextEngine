...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1, PS-2 plan/pilot/`E1` import, bounded internet source/cache/archive/Ogg/RAR5 handling, eight E3 project sources, explicit target/reject-parent roles, five typed REALIMPACT E2 rows, exact-domain/source gates, V1/V2 transfer calibration, listener acquisition, narrow vertical holdout and fixed/bbox/frequency-conditioned RBF rejection chain `IMPLEMENTED`; current status `PS-2_FREQUENCY_CONDITIONED_BANDWIDTH_CALIBRATION_REJECTED / RBF_BANDWIDTH_FAMILY_RETIRED / TWO_HOLDOUT_OBJECTS_PRESERVED / MODAL_RADIATION_REPRESENTATION_RESEARCH_NEXT / THREE_DIMENSIONAL_SPATIAL_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / FALLBACK_OUT_OF_DOMAIN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md), [internet corpus policy](development/physical-sound-internet-corpus-policy-ps2-2026-08-27.md) and [frequency-calibration evidence](development/physical-sound-realimpact-frequency-spatial-calibration-ps2-2026-08-28.md). Local capture is retired. Transfer V1 remains invalid; injective V2 passes narrow modal/damping and the original vertical RBF remains a narrow positive pilot. Fixed RBF then fails Blue/Glass; bbox-conditioned and per-mode `kL` bandwidth variants both lose fresh calibration to the unchanged control. Frequency calibration repeats at `42b6605d…983`, with median ratio `1.0146` failing `0.95` and max ratio `1.0266` failing `1.0`. Retire RBF-bandwidth tuning; keep ceramic holdout unopened and test one bounded complex per-mode radiation representation on already-open blocks before fresh validation. All eight exact-domain blockers, 3D spatial credit, corpus admission, `Pass`, PS-3, AV-P0D and production P1 remain disabled. |
