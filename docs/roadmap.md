...
| Physical sound source synthesis | SPEC-45 `Proposed`; frozen Q30 modal/DCT boundary, internet corpus/evidence adapters, synthetic FEM→Bempp controls and repeated real-method rejections `IMPLEMENTED`; strategy `NEURAL_FIRST / OFFLINE_MODEL / DETERMINISTIC_COOKED_RUNTIME`; current status `PS-2N0_CONTRACT_IMPLEMENTED / N0.2_Q30_STRUCTURED_EXPORT_PASS / SYNTHETIC_EXACT_AB_PASS / DCT_PER_ROW_FALLBACK / REAL_PROJECTION_NEXT / ADMISSION_SHADOW_SEALED / SELECTIVE_PASS_DISABLED / AUTHORED_CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md), [PS-2N0 evidence](development/physical-sound-neural-data-plane-ps2n0-2026-08-30.md), [N0.2 baseline evidence](development/physical-sound-classical-baseline-export-ps2n0-2026-08-30.md), [neural strategy](development/physical-sound-neural-acoustic-field-strategy-2026-08-30.md) and [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). Next construct a permitted real development projection and complete its exact baseline surface; the DCT branch remains fallback until a hash-closed per-row fit and exact cooker exist. Then evaluate the object-specific few-shot field before shared transfer. Validator and admission shadow remain independent; production consumes only a bounded cooked record after a future consumer/ADR gate. |
