...
| Physical sound source synthesis | SPEC-45 `Proposed`; V7 strategy `TASK_SPECIFIC_NEURAL_RATE_DISTORTION / CONTACT_TO_LATENT_FIELD / OFFLINE_BAKED_CLIP_ATLAS`; current status `R2_LISTENER_FIELD_REJECTED / R3A_V1_REPRESENTATION_REJECTED / R3A_V2_INCONCLUSIVE_RESAMPLING_CONTROL / R3A_V3B_NATIVE_NDAC_REJECTED / R3A_V4_FIT_REPRESENTATION_REJECTED / R3A_V5_NEURAL_PREFLIGHT_NEXT / ADMISSION_SHADOW_SEALED / PASS_DISABLED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V7](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). R3A V4 repeats every fit artifact byte-identically: all three analytical capacities meet byte budgets but fail spectrum on all `12/12` fit contacts, so development and sealed rows remain unread. V5 now freezes an internet-only training corpus and at most three task-specific neural latent capacities with direct spectral/modal losses. A pass may authorize an external contact-to-latent field and offline baked clip atlas, never runtime neural inference. Production still requires a future concrete consumer and ADR-046 gate. |
