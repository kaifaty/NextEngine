...
| Physical sound source synthesis | SPEC-45 `Proposed`; V2 roadmap strategy `NEURAL_TRANSFER_FIELD / OFFLINE_TRAINING / DETERMINISTIC_COOKED_RUNTIME`; Q30/DCT, FEM/BEM and previous real-data work remain baselines, controls and negative knowledge; current status `R0_REAL_DATA_BOUNDARY_IN_PROGRESS / NO_TRAINED_MODEL / ADMISSION_SHADOW_SEALED / SELECTIVE_PASS_DISABLED / AUTHORED_CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V2](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). First separate recorded-waveform and force-deconvolved-transfer semantics, materialize one verified REALIMPACT listener slice and freeze a compatible transfer-domain baseline. Then train a fixed-impact listener field before attempting full impact/listener few-shot or shared-object transfer. The independent validator and admission shadow remain isolated; production can consume only a bounded cooked record after a future consumer/ADR gate. |
