...
- R7c has accepted ADR-091/092/093 and implemented Linux Performance
  ...
  ten-run baselines plus isolated fixed gates on the single commit `0e8db75`;
  only R2 remains, waiting for a physical display.
...
3. **R7c — Linux performance authority
   ...
   core and advances the workload to `r5-physics-16.v3`. The R3, R4 and R5
   portions hold hard `PASS` ten-run baselines plus isolated fixed gates on
   the single commit `0e8db75`; remaining work is only R2 once a real display
   exists. Historical THOTH budgets/evidence are
   not inherited.
