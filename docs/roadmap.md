...
| Physical sound source synthesis | SPEC-45 `Proposed`; P0/P0.5, AV-P0A/B/C, PS-1, PS-2 internet corpus/evidence, converged elastic FEM→Bempp/full-angular synthetic controls, real-transfer preregistration and geometry-only V1 preflight `IMPLEMENTED`; current status `PS-2_GEOMETRY_PREFLIGHT_V1_REJECTED / BYTE_IDENTICAL_REPEATS / RESERVED_AUDIO_BYTES_ZERO / EXACT_COORDINATE_WELD_V2_PREREGISTRATION_NEXT / PITCHER_AND_PLANTER_AUDIO_SEALED / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / FALLBACK_OUT_OF_DOMAIN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [physical-sound subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [geometry-preflight evidence](development/physical-sound-realimpact-geometry-preflight-ps2-2026-08-28.md). V1 reports repeat at `2fb9fd0f…e25d` with zero reserved audio bytes: both published OBJ files are disconnected triangle soup and fail the closed-manifold gate. A post-rejection mesh-only discriminator shows exact bitwise coordinate welding recovers connected closed surfaces (`8070/7958` vertices, zero boundary/nonmanifold edges) without tolerance or repair. Next preregister only that V2 assembly change and repeat the unchanged `8192/2048` preflight before Pitcher audio. All eight exact-domain blockers, measured real 3D transfer, corpus admission, `Pass`, PS-3, AV-P0D and production P1 remain disabled. |
