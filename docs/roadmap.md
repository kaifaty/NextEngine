...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V43](plans/physical-sound-synthesis-roadmap-v43.md) is `ACTIVE / C1A_D2_C0R_COMPLETE / B0R_R0R_NEXT / C1_READY / V0_BLOCKED_BY_POWER / M0_BLOCKED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | Repeat-exact C1A proves two aliases; [D2/C0R](development/physical-sound-v43-d2-c0r-identity-repair-result-2026-09-03.md) moves all 20 derived AV-MSF records into train, merges both aliases, quarantines five object-80 material rows and preserves all 278 content objects byte-for-byte. B0R/R0R is next, C1 descriptor work is ready in parallel, and V0 still needs independent internet calibration parents/projects. Only later qualified validator, holdout and joint shadow evidence may authorize deterministic cooking and the opt-in Steel demo; every failure retains the authored fallback. |
