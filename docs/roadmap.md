...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V39](plans/physical-sound-synthesis-roadmap-v39.md) is `ACTIVE / F0_F1_COMPLETE / D0_NEXT / S0_FRONTIER_6_23_ACTIVE / TWO_LANE_ML_FOUNDRY / PROTECTED_ADMISSION_BLOCKED / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [F1](development/physical-sound-v39-f1-source-frontier-result-2026-09-03.md) now replays V30 `6/27` plus the IETeasy `0/4` increment into exact frontier `6 exact-Steel / 23 non-Metal`, with no material/repetition inflation and zero signal/model/network access. D0 permanent disclosed roster is next while bounded S0 discovery continues. Disclosed projects may never return to the protected pool. Strict whole-project role power, independent validator qualification, method holdout and joint shadow still gate deterministic cooking and the opt-in Steel demo; every failure retains the authored fallback. |
