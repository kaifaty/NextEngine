...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V36](plans/physical-sound-synthesis-roadmap-v36.md) is `ACTIVE / R0_A0_F0_C0_X0_E0_COMPLETE / D0_NEXT / EXECUTION_SEALED_PRE_ACCESS / FRESH_V36_TARGETS_UNOPENED / V35_QUALITY_UNOBSERVED / REAL_RELEASE_BLOCKED / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | [V36 E0](development/physical-sound-v36-e0-full-surrogate-seal-result-2026-09-03.md) runs the committed full-count/full-step D0+H0 owner twice exactly, closes all 24 files/stdout, every scientific terminal and the `300 s / 1 GiB / 64 MiB` envelope, then binds both roots into a checked-in pre-access seal with zero official values. D0 is next, but its provider must recompute every seal binding before constructing the one-shot fresh capability. Synthetic H0 and the independent real source/validator lane must both pass before Steel training; [E2](development/physical-sound-v30-e2-q1a-source-growth-result-2026-09-02.md) still leaves the weaker real role six exact-Steel and 27 non-Metal groups short. |
