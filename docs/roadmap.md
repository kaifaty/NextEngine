...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V11 closes V9 real reconstruction and targets `FORCE_NORMALIZATION / EXPLICIT_MODAL_TRANSFER / CONSTRAINED_OFFLINE_ML / BAKED_CLIP_ATLAS`; current status `A1R_FORCE_ONSET_PASS / V9_REAL_FIT_REJECTED / B1R2_REPEAT_REJECTED / LOCAL_MODAL_SUPPORT_NEXT / REAL_DATA_CLOSED / VALIDATOR_NOT_RELEASED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V11](plans/physical-sound-synthesis-roadmap-v11.md) and the [B1R2 exact result](development/physical-sound-r3a-v11-b1r2-noise-aware-gtls-result-2026-08-31.md). B1 V1 proved force-normalized reconstruction but rejected one combined mask; B1R exposed a noise-relative precheck defect. B1R2 then fixes weak-force OOD and gives `0.030031` mean held GTLS NRMSE with highly accurate `5/7` admitted poles, but broad-band SNR coverage is `0.661254` and the low-coherence corruption is incorrectly subtracted as calibrated noise. Holdout and real data remain closed. Next is a fresh local-modal-support policy that keeps stationary calibration noise separate from unmeasured interference. ML remains offline and runtime still receives only clips; production requires a concrete consumer and ADR-046 gate. |
