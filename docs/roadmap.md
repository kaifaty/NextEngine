...
R83 therefore adds only iteration-local max-normalized nonlinear restoration
...
7. **R5 humanoid movement TRAIN-4 clip-global prototype (`ACTIVE_R&D / CURRENT WIP`):**
   ...
   `cmu05`/`cmu16` and rejects `cmu139`; R74–R82 select iteration-local minimax
   nonlinear restoration after hard exact-row SOC is infeasible. До unchanged all-three
