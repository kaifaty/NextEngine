...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `ACTIVE_ISOLATED_RESEARCH / INTEGRATION_BLOCKED`; SPEC-38/ADR-076 `Proposed`; water W1 `LINUX_W1_PASS / CONTINUUM-WATER-REF-P1=PASS` at `fe223f9`; CPU/direct-GPU W2 miss; Nonlocal NR4 `NONLOCAL_48K_RECLOSURE_CANDIDATE / NO_W2_CREDIT`; performance reclosure `NP0_NEXT`; later ProductChecks `NOT_RUN` | W0F/G/H/I roots remain frozen and W3 remains blocked. [NR4](development/nonlocal-continuum-nr4-decision-2026-08-20.md) closes the fixed-work branch: retained directed gather, pointer swap, term specialization and stable sample storage pass their oracle, reach `3.27688x` HN-3 geometric-mean speedup and water-48k `4.019520 ms` p95. Source atomics and O3 remain numeric failures; O4 remains exact negative performance evidence. The [next performance roadmap](plans/nonlocal-continuum-performance/README.md) starts with a fresh exact-50k coherent/permuted/advected corpus, then pair/memory/topology and separately credited convergence work. It does not close W2, reduce the `4/6 ms` target, grant GPU authority or change runtime/public contracts. Promotion still requires an explicit consumer-backed decision, cross-target and exchange identity, one composition DAG/PhysX integration, scheduled epochs, admission/fault semantics and the successor combined budget. Independent terrain closure remains Package 10T. |
