...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V16 status `L0A_PROTOCOL_FROZEN / L0B_TRUTH_EXECUTION_NEXT / SOURCE_GROWTH_PARALLEL / ADMISSION_BLOCKED / METAL_FIRST / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | [L0a](development/physical-sound-v16-l0a-known-truth-neural-oracle-protocol-2026-09-01.md) freezes an 18-object synthetic structured-modal tournament, equal-budget geometry ablation, classical controls, mutation/OOD suite and two-run exact decision before implementation, with zero real/protected access. [S0c](development/physical-sound-v15-s0c-source-sufficiency-role-freeze-result-2026-09-01.md) still blocks real admission under the unchanged `4/1/1/1/1` shape. L0b now tests model capability while exposure-role recovery/source adapters proceed independently; only a passing truth/real tournament plus a frozen protected Metal split may open validator release, one-shot shadow, deterministic cooker and demo. Raw-force transfer, runtime inference and production promotion remain separate blocked claims. |
