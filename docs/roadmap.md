...
| Physical sound source synthesis | SPEC-45 `Proposed`; PS-2 internet evidence, synthetic FEM→Bempp/full-angular controls, exact-weld geometry, repeated Pitcher calibration/causal audit and Ceramic Cup observation execution/diagnostic `IMPLEMENTED`; current status `PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_CUP_SHARED_DECAY_MISMATCH_SUPPORTED / LISTENER_LOCAL_REJECTED / SIMPLE_LOW_FREQUENCY_CAUSE_REJECTED / MULTIOUTPUT_SYNTHETIC_CONTROL_FROZEN / TWO_SYNTHETIC_RUNS_AUTHORIZED / REAL_REUSE_BLOCKED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / SELECTIVE_PASS_DISABLED / P1_BLOCKED`; no public schema, content role or production gate promoted | Follow the [subsystem roadmap](plans/physical-sound-synthesis-roadmap.md) and [multi-output control preflight](development/physical-sound-multioutput-decay-control-preflight-ps2-2026-08-28.md). Manifest `cd8ee856…2078` and repeated preflight `88b017a1…1153` bind a 15-output spatial-modal-power candidate, known frequency/decay truth and a node-contaminated single-output comparator. Execute it twice; any failure rejects the revision. Real rows, fetching, mechanics and Planter remain blocked; clip fallback stays active. All exact-domain, admission, `Pass`, PS-3, AV-P0D and P1 gates remain disabled. |
