...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V16 status `L0B_REPEAT_EXACT_REJECT / PRIMARY_MODEL_FAMILY_CLOSED / L2_RESEARCH_NEXT / SOURCE_GROWTH_PARALLEL / ADMISSION_BLOCKED / METAL_FIRST / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | [L0b](development/physical-sound-v16-l0b-known-truth-neural-oracle-result-2026-09-01.md) is byte-exact across two zero-real-access runs. Geometry-aware gains beat classical controls and the equal-budget ablation, but global poles/damping, waveform, continuity and coverage OOD fail, so the V16 primary cannot open real training. A bounded successor-family research cycle must separate analytic global factorization, topology-aware contact inference and geodesic coverage before a new unopened truth protocol. [S0c](development/physical-sound-v15-s0c-source-sufficiency-role-freeze-result-2026-09-01.md) independently keeps real admission blocked under the unchanged `4/1/1/1/1` shape. Runtime inference and production promotion remain separately blocked. |
