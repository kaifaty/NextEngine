...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `PLANNED / NOT_ACTIVE`; SPEC-38/ADR-076 `Proposed`; water `RESEARCH_ONLY`: original W0B `INVALIDATED_BY_RC1`, W0C/W0D closed, W0E `LOCAL_PROFILE_DISCRIMINATOR_SURVIVED / NOT_SELECTED`, W1 `IMPLEMENTED / BLOCKED_ON_W0F`; all ProductChecks `NOT_RUN` | W0E separates density support, projected-PCG pressure and analytical velocity-level contact. Independent implementations match and the candidate passes 1200 local hydro steps with zero penetration, but this is not corpus credit. W0F must close internal aperture geometry, the product boundary capacity and new profile/corpus/scenario roots before W1 resumes. Do not start W2 or integration. Independent terrain closure remains governed by Package 10T. Main-roadmap activation still requires `CONTINUUM-WATER-REF-P1 = PASS`; promotion also requires exact exchange identity, one composition DAG/PhysX integration, scheduled epochs, worst-case admission, primary-session fault domain and successor combined budget. |
