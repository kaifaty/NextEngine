...
| Physical sound source synthesis | SPEC-45 `Proposed`; V2 roadmap strategy `NEURAL_TRANSFER_FIELD / OFFLINE_TRAINING / DETERMINISTIC_COOKED_RUNTIME`; Q30/DCT, FEM/BEM and previous real-data work remain baselines, controls and negative knowledge; current status `R0_COMPLETE / R1_TRANSFER_CONTROLS_FROZEN / R2_V1_REJECTED / PHASE_ALIGNED_SUCCESSOR_NEXT / ADMISSION_SHADOW_SEALED / SELECTIVE_PASS_DISABLED / AUTHORED_CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V2](plans/physical-sound-synthesis-roadmap.md) and its [implementation plan](plans/2026-08-30-physical-sound-neural-acoustic-field-implementation-plan.md). The repeated real projection and R1 controls are frozen. R2 V1 PyTorch/MLflow training and Rust-metric evaluation reproduce but reject both time-domain listener-latent candidates; do not tune that profile on opened queries. One separately preregistered propagation-delay-aligned successor is next before any full impact/listener few-shot or shared-object transfer. The independent validator and admission shadow remain isolated; production can consume only a bounded cooked record after a future consumer/ADR gate. |
