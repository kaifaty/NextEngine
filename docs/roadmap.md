...
| Nonlocal R63Y--R63ZI arithmetic research note | `REPORT_ONLY / TWOFOLD_AFFINE_IMAGE_CANDIDATE / BLOCK_OPERATOR_FROZEN / EXPORTED_FACTOR_CONSUMPTION_CANDIDATE / REVIEWED_RECURRENCE_LADDER_REJECTED / REVIEWED_OPERATOR_INPUT_FACTORIAL / REVIEWED_SINGLE_OPERATOR_SCHEDULE / REVIEWED_DENOMINATOR_STEP / REVIEWED_RAW_SIGN_ALIAS / REVIEWED_AFFINE_IMAGE_CENTER_RESIDUAL_SUFFICIENT / REVIEWED_COMMON_SOLUTION_OUTSIDE_VERIFIER_AFFINE_MODEL / CERTIFICATE_REPAIR_STOPPED / ORIGINAL_OPERATOR_TWOFOLD_RECURRENCE_AUTHOR_REJECTED`; no owner promotion | R63Y selects fixed twofold binary64 verification; R63Z/R63ZA close common-block products and exported-factor consumption. Reviewed R63ZB--R63ZF isolate the ladder loss through operator/input, schedule, denominator/step and unchanged raw signs. Reviewed R63ZG/R63ZH show that the common affine-image center faithfully rejects because the common K2 solution lies outside the verifier model; certificate repair stops. R63ZI preserves the tangent-Gram equation, contains all `10,404` coefficients and `306` executed product rows and closes two byte-identical Release runs, but all three K2 states remain `12+/24-/66?`; the width-two dense producer is author-rejected pending review. A product-only hybrid must distinguish stored-matrix precision from the remaining K2 recurrence before any width increase. Dynamic builder, corpus, runtime stop, timing, GPU, Rust integration and production remain blocked. |
