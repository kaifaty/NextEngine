...
| Local continuum materials: water, deformable terrain, mud and off-road coupling | `PLANNED / NOT_ACTIVE`; SPEC-38/ADR-076 `Proposed`; original water W0B `INVALIDATED_BY_RC1`, W0C `READY / NOT_STARTED`, W1 `IMPLEMENTED / BLOCKED_ON_W0C`; all ProductChecks `NOT_RUN` | Clean-tree free-fall passes, but `CW-HYDRO-001` stops before step 1. An independent brute-force audit exactly reproduces inputs, boundary volumes, residual curve and representative rows, so W0C must select and re-root a justified numeric profile; do not start W2 or integration. Main-roadmap activation still requires `CONTINUUM-WATER-REF-P1 = PASS`; promotion also requires exact exchange identity, one composition DAG/PhysX integration, scheduled epochs, worst-case admission, primary-session fault domain and successor combined budget. |
