...
| Physical sound source synthesis | SPEC-45 `Proposed`; Roadmap V12 targets `ACQUISITION_COVERAGE_CERTIFICATE / EXPLICIT_MODAL_TRANSFER / CONSTRAINED_OFFLINE_ML / INDEPENDENT_VALIDATOR / BAKED_CLIP_ATLAS`; current status `A1R_FORCE_ONSET_PASS / V9_REAL_FIT_REJECTED / B1R3_REPEAT_REJECTED / C1_REPEAT_PASS / C2_SOURCE_PASS / C3_ROLE_FREEZE_NEXT / REAL_PCM_CLOSED / VALIDATOR_NOT_RELEASED / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | Follow [Roadmap V12](plans/physical-sound-synthesis-roadmap-v12.md), the [C1 repeat-exact result](development/physical-sound-r3a-v12-c1-acquisition-coverage-oracle-result-2026-08-31.md) and [C2 source inventory](development/physical-sound-r3a-v12-c2-internet-source-zero-decode-inventory-2026-08-31.md). C1 recovers `7/7` known poles at `0.004961` holdout mean NRMSE with calibrated OOD. C2 proves a stable ObjectFolder paired force+mic source and coordinate/geometry lineage without a new waveform decode, but narrows the claim to one canonical setup and normalized instrument counts because numeric listener/support/SI calibration axes are absent. C3 must choose a fresh target and freeze six roles plus exact read budgets before PCM may open. ML, validator release and runtime stay closed; production requires a concrete consumer and ADR-046 gate. |
