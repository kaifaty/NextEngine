...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V18](plans/physical-sound-synthesis-roadmap-v18.md) status `P0B_PROTOCOL_FROZEN / B0_IMPLEMENTATION_NEXT / SOURCE_GROWTH_PARALLEL / ADMISSION_BLOCKED / METAL_FIRST / CLIP_FALLBACK / P1_BLOCKED`; no public schema, content role, runtime model or production gate promoted | [V17 G0](development/physical-sound-v17-g0-scale-separated-global-oracle-result-2026-09-01.md) passes absolute quality, raw-dimensional scale transfer, all 72 mutations and exact repeat, but its neural head misses the required complexity margin over degree-two ridge. [V18 P0b](development/physical-sound-v18-p0b-hybrid-truth-protocol-2026-09-01.md) now freezes the unchanged deterministic scaffold on `72` fresh B rows, preserves unopened O/F/I roles and makes B0 the next implementation boundary; fresh quality values remain unopened. [S0c](development/physical-sound-v15-s0c-source-sufficiency-role-freeze-result-2026-09-01.md) independently keeps real admission blocked under unchanged `4/1/1/1/1`; runtime inference and production promotion remain separately blocked. |
