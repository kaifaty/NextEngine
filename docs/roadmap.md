...
| Physical sound source synthesis | SPEC-45 `Proposed`; [Roadmap V42](plans/physical-sound-synthesis-roadmap-v42.md) is `ACTIVE / R0_COMPLETE / C1_NEXT / V0_READY / CORPUS_SIGNAL_INSUFFICIENT / PROTECTED_ADMISSION_SOURCE_POWER_OOD / OFFLINE_ONLY / AUTHORED_FALLBACK`; no public schema, content role, runtime model or production gate promoted | The repeat-exact [R0 audit](development/physical-sound-v42-r0-domain-information-audit-result-2026-09-03.md) finds strong project/source identity (`0.763333333` balanced accuracy, permutation `p=0.000488162`) but no cross-project coarse-material benefit: median relative improvement is `-0.122864043`, bootstrap 95% `[-0.168145282, -0.050277002]`, and simple oracle project-centering also fails. C1 is next: source-backed runtime descriptors and independent internet parents grow toward a planning floor of `125` supported evaluation parents; B1 must beat B0's global floor before any neural candidate. V0 may independently freeze validator mechanics; protected admission remains short by `13/34` and `13/31`. Only qualified validator, generator holdout and joint shadow may authorize deterministic cooking and the opt-in Steel demo; every failure retains the authored fallback. |
