...
`ProjectLockV3`/`ContentManifestV1` hashes. Async fetch, decode or
...
Текущий bounded R5 consumer развивает R2 content prerequisite семью принятыми
...
R5f publishes the existing sampled/bind local pose through an exact render-joint
mapping in `PresentationSnapshotV3`; the reference player and NPC share one
`NeutralBaseSkinningProfileV1`, and B0 reconstructs its vertex stream. This is
a downstream read-only consumer and adds no animation cursor, pose, save or
Replay owner.

R5g extends only that downstream surface. The complete character record now
declares `Sampled | HeldPresentationPose | BindPoseFallback` source and
`FullCorrectives | ReducedCorrectives | BaseSkinningOnly | Culled` deformation
work. Full evaluates the exact authored set, Reduced only its Essential class,
and the renderer repeats the newest complete V3 snapshot at 30/60/144 Hz
without advancing or selecting animation state. The production-shaped matrix
changes presentation/frame hashes where expected while preserving exact state,
ledger, physics and physical-animation roots.

R5h closes the previously declared `ANIM-ROOT-MOTION-P1` matrix over that same
bounded forward route. The release-mode `animation-root-motion` command
executes 1,000 isolated ten-cycle production generations:
4,000 ordinary accepted, 2,000 canonical faulted, 1,000 exact receipt retry,
1,000 checkpoint/restore and 2,000 animation-LOD-isolation cycles. Every one of
the 10,000 command/proposal pairs round-trips canonical bytes and replays through
`RuntimeReplayDriver`; 9,000 fresh proposals carry exact project-bound
procedural motor/safety evidence. The fixed collision fixture produces full,
partially clipped and blocked Physics outcomes, while all 3,000 rejected/retry
