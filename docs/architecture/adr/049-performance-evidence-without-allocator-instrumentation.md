...
| Superseded by | The `nextengine-performance-v4` methodology identity is superseded by [ADR-060](060-relaxed-thoth-performance-preflight.md); Performance V4 wire schemas and the allocator-removal decision remain Accepted. |

> Current reports retain the Performance V4 wire shape but carry
> `nextengine-performance-v5` under ADR-060. The v4 methodology string below is
