...
| Superseded by | [ADR-063](063-run-level-performance-evidence-and-fixed-gate-batches.md) supersedes the methodology-v7 identity and single-candidate-run relative gate. [ADR-090](090-linux-only-v1-and-indefinitely-deferred-windows.md) removes the Windows hard authority from current v1/R7 scope. The workload and exact-root requirements remain reusable; Windows budgets/driver fingerprint stay historical and cannot define Linux R7c. |
