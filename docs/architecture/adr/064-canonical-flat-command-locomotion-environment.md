...
| Superseded by | [ADR-065](065-curriculum-flat-command-locomotion-profile.md) narrowly supersedes only the assumption that these are the only two current Stage 0 profiles; [ADR-067](067-stage0-profile-identity-and-curriculum-hash-closure.md) narrowly closes exact V1 BodySchema identity and per-profile hash semantics. |
