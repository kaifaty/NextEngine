...
bounded `project run/package`; ADR-085 adds read-only `project inspect/diff`
and their separate reports for the independent `creator-smoke` consumer. All
wider creator automation must still arrive with a real consumer and cannot gain
a privileged gameplay mutation path.
