...
| Заменён | Runtime in-flight protocol в строках `begin`/callback/`finish` и запрет const destructor-free TLS selector узко заменены [ADR-040](040-fixed-tls-sharded-global-allocator-measurement.md) после measured enabled overhead `+15.95%`. Crate/dependency, unsafe/System delegation, exact counting, report/scope, bounds и fail-closed semantics этого ADR остаются Accepted. |

## Контекст

> Актуальный admission/close protocol определяется ADR-040. Описанный ниже
> checked-CAS global in-flight protocol сохраняется как исходное решение и
> evidence context, но не является текущим implementation contract.
