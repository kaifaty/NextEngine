# SPEC-20: World calendar, authored routines and bounded population lifecycle
...
| Replaces | SPEC-20 4.2; aligns retained R4 consumers with the current V7/V8 project and Replay V10 closure |

## Status and admission

This document is the current contract for the completed R4a through R4d
product increments: calendar/routine, population/navigation, the reciprocal
cognition view and one owner-validated systemic activity/bulk-time consumer.
...
3. `fast`, `play`, `persistence-replay`, `content-package` and report-only
   performance paths cover routine plus population/navigation success and
   malformed-input cases;
4. ADR-052, ADR-072 and ADR-074 are `Accepted` and explicitly supersede their affected
   clauses;
...
R4c consumes the accepted population location/route for one subject under
ADR-073. R4d adds a separate activity owner, exact tier-work dispatch and a
bounded ordinary-tick bulk-time consumer under ADR-074. This admission closes
the functional R4 increment, but not B-12, B-13, paired Windows/Linux evidence
or v1 shipping.
...
R4b adds two domain-relevant content roots and one separate World Services
owner. `WorldPopulationCatalogV1` contains exactly 100 sorted stable subjects:
16 active-cadence records with period 3, 32 near-cadence records with period
15 and 52 background-cadence records with period 60. Phase is the ADR-016
SHA-256-derived integer and is stored only after exact validation. The closed
durable tiers are `Dormant`, `Abstract`, `Simulated` and `Active`; physical LOD
remains a separate Physics/Physical Embodiment concern.

`WorldNavigationCatalogV1` contains four revision-bound region tiles and one
node per current world chunk. Sorted undirected positive-cost edges form a
connected graph. `NavigationQueryV1` binds exact graph revision, start, goal
and `AbstractTransfer`; deterministic Dijkstra produces the ordered-node
`NavigationRoutePlanV1`, integer total cost and plan hash.

One background courier is the bounded production mutation consumer. Beginning
at its authored start tick it commits exactly seven revisions:

```text
Dormant -> Abstract -> [route-bound relay-station to frontier transfer]
        -> Simulated -> Active -> Simulated -> Abstract -> Dormant
```

The transfer can commit only while `Abstract`. An `Active` transfer returns
`PHYSICAL_TRAVERSAL_REQUIRED`, changes no logical placement and never writes a
physical pose. The same `PersistentId` survives all tiers and both regions.
Every other population record remains a real due/query participant but retains
revision zero in this bounded scenario.

Current-only project types are
`ProjectAuthoringManifestV7 -> NeutralProjectSourceV7 -> CookedProjectV7 ->
ActivatedProjectV8`. R4b's two catalogs changed reference-alpha roots `28 ->
30` and entries `114 -> 116`; R4c cognition changed them to `31` and `117`,
and R4d activity changed them to `32` and `118`. Later R5a/R5c animation
content raised the closure to `35` roots and `121` entries; R5d adds the exact
body-schema root for the current `36`/`122` closure. The four-region/64-chunk
partition shape is unchanged. Replay is current-only V10 and the reference
application/save closure has ten full-tuple owner segments after adding the
physical-animation owner; R5d projections remain reconstructible and add no
save owner.
...
planning failure. Physical corridor traversal remains future scope; R4d
tier-wide scheduling is defined below and does not fabricate physical or RPG
outcomes.

## R4d activity, tier cognition and bounded bulk time

`WorldActivityCatalogV1` binds the one authored worker, commitment, workplace,
duration and systemic-work profile. `WorldActivitySnapshotV1` is a separate
World Services segment with the exact sequence `Unassigned -> Assigned ->
Working -> Completed`. Assignment requires an accepted RPG commitment;
starting requires exact workplace presence; completion requires elapsed work.
Every transition checks the expected activity revision and publishes through
the ordinary command/event/final-preflight path. Activity never writes RPG
resources, inventory, currency or physical pose.

Every due population record maps by tier to one closed cognition work kind:
Active `FullEvaluation`, Simulated `ReducedEvaluation`, Abstract
`AbstractMaintenanceOnly`, Dormant `DormantWakeCheckOnly`. Results are sorted
by stable subject ID, bounded to the 100-record catalog and record zero
fabricated outcomes. Abstract and Dormant entries cannot claim traversal,
trade, combat or quest effects.

`ReferenceGameDriverV2::advance_bulk_time` accepts `1..=4096` ticks, runs each
through the normal prepare/validate/commit pipeline and stops after the first
authoritative or presentation-observable boundary or at budget exhaustion.
Zero and oversized requests fail before mutation. At every returned boundary,
stepped and bulk execution exact-match complete state, command/event counts,
owner revisions, checkpoint root and application root.
...
- World Services owns the calendar/routine catalog reference, durable routine
  membership, committed routine activity and its revision.
- World Services also owns population membership, logical region/node, tier,
  cadence and record revision through `WorldPopulationSnapshotV1`.
- World Services separately owns the systemic activity lifecycle and revision
  through `WorldActivitySnapshotV1`; it consumes evidence from RPG/location
  projections but does not copy or mutate them.
- Asset & Persistence owns the immutable population/navigation catalogs and
  exact graph, placement and residency-policy hashes.
...
- Streaming continues to own its existing reconstructible residency/cache
  state. Routine/population segments do not copy a loaded chunk, runtime entity
  or physical pose.
...
## Retained R4a content contracts

The retained R4a layer uses typed content, not generic property bags or
reference-game constants. Its V3/V4 project names below describe that
increment's predecessor boundary; the current R4b successors are listed above.
...
The R4a routine V1 structurally contains exactly one subject and one transition, so it can
produce at most one due command in a tick. A second catalog, routine,
transition or unknown field returns `WORLD_ROUTINE_CONTENT_INVALID`. R4b adds
its population as a separate catalog/owner rather than widening this routine
wire shape.
...
are `O(1)`, and authoritative memory is `O(1)`. R4b retains a bounded sorted
100-record scan. Its report-only measurement does not justify a generic heap,
timing wheel or second scheduler authority; that requires another accepted
consumer.
...
ordering follows SPEC-21. These four entries define the retained R4a prefix.
R4b `core_r4b` adds exactly one fifth entry:

| Map key `(payload_schema_id, version)` | `command_kind_id` | Priority | Required capability | Validator phases |
|---|---|---:|---|---|
| (`nextengine.command.world-population`, `1`) | `nextengine.command-kind.world-population` | 260 | `nextengine.capability.world-population-commit` | Outcome only |

Its validator profile is
`SHA256("nextengine.command-validator.world-population.v1\0")`. The exact five
entries, not a numeric private registry version or partially upgraded table,
are the current canonical registry envelope/hash and golden vector.
...
The live authority
registry and the retained R4a Replay V6 `AuthorityGrant` contain exactly the unscoped
...
The current R4b schedule adds
`nextengine.system.world-population-boundary` at the same existing stage 6,
owned by `nextengine.world-services`, with its own one-shard plan and exact
population/navigation/snapshot read set. The routine descriptor orders itself
`before` population and the population descriptor orders itself `after`
routine; the relation is canonical rather than registration order. Population
writes only the private `nextengine.world-population-proposal` staging key.
Reducers remain empty, barriers stay Ingress-2/Outcome-9 ordinal zero and the
twelve-stage vector is unchanged.
...
core checkpoint (including the physics catalog), streaming/routine/population canonical
...
ordinary freeze/snapshot boundary and validates the same helper output.
Projects omit only the segments for absent optional World Services owners. The
current `world_checkpoint_with_world_services_v1_state_root` helper sorts
streaming plus optional routine/population alongside the three core owners.
Reference R4a returned five segments; current reference R4b always returns
six. Root or descriptor construction is validation work and cannot occur
after the first live write.

The ordinary `ReferenceGameDriverV2` live-advance path uses the same joint
Runtime/routine/population/optional-streaming generation validation and final stale-base
...
world-routine bytes. This five-segment application root was used by the R4a
application, save/restart checks and Replay V6; it does not silently redefine
the V4 checkpoint field.

R4a introduced the then-current-only `ReplayManifestV6`. Its initial closure for
...
### R4b population and R4d activity segments retained in Replay V10

R4b adds the separate segment:

```text
owner_id   = "nextengine.world-services"
schema_id  = "nextengine.world-population-snapshot"
segment_id = "world-population"
version    = 1
```

R4d adds the separate segment:

```text
owner_id   = "nextengine.world-services"
schema_id  = "nextengine.world-activity-snapshot"
segment_id = "world-activity"
version    = 1
```

The retained world-services root frames Runtime, RPG, Physics, streaming,
optional routine, population and activity using the same full-tuple sort.
Agent, Memory and the later R5 physical-animation segment remain separate
owners, so current reference alpha requires ten.
`SaveManifestV2` and the three-owner `WorldCheckpointV4.state_root` retain
their wire meanings.

Current-only `ReplayManifestV10` retains the ordered tick/query/streaming facts,
requires population, activity, Agent and Memory segments in its initial
closure and allows only the routine slot to be absent. Generic Outcome
command/event vectors carry population, activity and cognition mutations; no
owner-specific replay side channel or hard-coded courier/cognition/activity
tick exists. Every reference compare point exact-matches the ten sorted
descriptors, application root, command ledger and existing batch/query/trace
hashes. V9 and earlier pre-v1 formats return
`UNSUPPORTED_REPLAY_MANIFEST_VERSION` before nested decoding; there is no
V8-to-V9 migration reader.
...
stream commands, events, owner deltas, body/archive occurrences and receipt
chain that produce the snapshot. Reference alpha admits exactly the courier's
seven-command prefix and revision-zero state for the remaining 99 records.
Missing, extra, forged or reordered evidence returns
`WORLD_POPULATION_LEDGER_CLOSURE_INVALID` before Runtime, streaming, routine or
...
| Invalid population count/cadence/tier, navigation graph/catalog, cognition subject binding or activity/systemic-work closure | reject complete V7/V8 project before World Services/Agent activation; retain the prior project/world |
...
| Stale/invalid activity transition or non-byte-exact stage-9 candidate | reject the complete activity command; abort the joint candidate before any activity/RPG/ledger publication |
...
| Runtime/routine/population/activity/streaming/Agent/Memory joint-generation validation fails | publish none of the staged generations and retain every prior live owner |
...
| `ai-host`, learned model or optional nav adapter absent | no effect; the engine-owned graph and deterministic authored path remain complete |
...
`WORLD_ROUTINE_LEDGER_CLOSURE_INVALID`; a mismatched current Replay V10 oracle remains
...
These checks map retained R4a/R4b behavior and current R4c/R4d consumers with
their fail-closed variants.
...
| `WORLD-ROUTINE-REPLAY-P1` through `persistence-replay` | Save around the routine boundary, restart and replay the same assigned inputs; inject snapshot/stream receipt mismatches | Exact routine commands/events/delta, interaction availability and current ten-owner V10 application root match uninterrupted execution; mismatched routine/ledger closure fails before publication |
...
| `WORLD-POPULATION-REPLAY-P1` through `persistence-replay` | Save/load and Replay V10 the courier; inject a calendar-valid snapshot without its receipt | Population bytes, commands/events/deltas, ten descriptors, application/ledger roots and continuation are exact; missing ledger evidence returns `WORLD_POPULATION_LEDGER_CLOSURE_INVALID` before publication |
| `WORLD-ACTIVITY-P1` through focused tests and `play` | Drive assignment, workplace start and elapsed-work completion; inject stale/no-job/no-route/no-money/inventory failures | Exact `Unassigned -> Assigned -> Working -> Completed` revisions commit only with valid evidence; every rejected branch retains activity/RPG/event/ledger roots |
| `WORLD-BULK-P1` through focused tests | Compare stepped execution with requests `1..=4096`; inject zero/oversized requests | Each returned observable boundary matches state/counts/revisions/checkpoint/application roots; invalid requests mutate nothing |
| `WORLD-CONTENT-P1` through `content-package` | Cook/activate authoring V7 and inject malformed routine, 100-record, graph, cognition, activity, body-schema, hash, node/chunk, bootstrap and old-profile variants | Valid V7/V8 content produces `36` roots, `122` entries, four tiles/regions, 64 nodes/chunks and canonical seven-kind/four-system closure; every invalid candidate fails before partial publication |
| `fast` | Focused codecs, cadence phases, Dijkstra tie-break, stage-6/stage-9 invariant faults, owner-delta/receipt roots, owner ledger closures, joint commit, exact registry/schedule/profile roots and retired-version tests | Exact bytes/diagnostics remain stable; malformed candidates create no partial batch/archive/receipt/owner publication, barriers stay Ingress-2/Outcome-9 and direct mutation/opaque profile/owner-only segment lookup remain absent |
| conditional `performance --scenario smoke --mode report` | Compare the ordinary fixed-stage path with routine, population and systemic activity enabled | 900 movement plus the current routine/population/systemic work produce 917 command bodies and preserve exact roots; debug ceilings are watchdogs only and verdict remains report-only |
| conditional `performance --scenario r4-100npc --mode report` | 1,000 warm-up plus 10,000 measured production joint ticks over the exact 16/32/52 population and four tier-work kinds | Due/query/cognition counts and roots are exact, maximum queue depth is 16 and deferral/drop/starvation/fabricated outcomes are zero. Unsupported-host timing is `NOT_RUN`; local navigation/integrated rows exceed report targets and do not close B-12 |

`r4-100npc` is available only as an honest report-only workload. The recorded
unsupported-host run is `NOT_RUN`, not a hard performance pass.

## Future expansion

R4c deterministic cognition and the bounded R4d systemic
work/currency/trade/food vertical now consume the current R4b substrate.
Learned policy documents SPEC-33/34 and ADR-050/053/054 are optional R8 work
and do not block R4 or v1. Broader bulk-time optimization must preserve the
current first-observable-boundary equivalence and cannot restore the removed
ADR-021 migration or generic scheduler by default.
...
Tier-wide cadence retains this rule. Events enqueue work
