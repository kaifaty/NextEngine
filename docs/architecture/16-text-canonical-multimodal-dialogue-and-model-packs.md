...
The narrower [SPEC-47](47-streaming-tts-and-spatial-speech-presentation.md)
and [ADR-099](adr/099-bounded-streaming-tts-through-ai-host-and-audio-scene.md)
...
For the first local TTS implementation, SPEC-47 specializes this concept as
...
`TtsVoiceBindingV1` closure from SPEC-47. Model-neutral character voice intent
...
| `MODEL-TTS-P1` | `next ai benchmark dialogue --role tts --corpus russian-voice-v1 --profile $PROFILE` | First PCM p95 ≤500 ms, real-time factor ≤0.5, continuation is bounded, and audio schema limits hold; use the next TTS route, authored voice or subtitles. SPEC-47 separately requires protocol, streaming, spatial, replay and co-resident resource checks, so this model gate cannot prove engine integration by itself. |
...
Lifecycle остаётся `Deferred Proposed`; эти contracts не выбирают shipping model и не меняют Accepted runtime. A future SPEC-47/ADR-099 promotion accepts only its bounded TTS consumer unless a separate decision explicitly promotes more of this document.
