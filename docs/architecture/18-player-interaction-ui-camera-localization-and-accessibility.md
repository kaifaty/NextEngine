...
| Заменяет | SPEC-18 2.5; advances the current replay envelope after the R4b owner addition |
...
`ReplayManifestV7` stores the ordered targeting intents, reconstructed queries,
...
intent, query, result, command, event или root. Retired Replay V6 and earlier are rejected
