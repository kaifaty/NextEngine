# SPEC-47: Streaming TTS and spatial speech presentation
...
SPEC-47 проектирует один ограниченный TTS vertical:
...
| `TTS-RESOURCE-P1` | active renderer + resident model + audio/capture on the declared native Linux profile, including OOM/device/host fault | joint admission prevents oversubscription, queue remains bounded, no callback stall and declared latency/VRAM tails are reported | reject route before load or downgrade explicitly |
...
5. the renderer-co-resident native Linux resource profile is measured, with
...
ADR-099 status, index/routing/traceability and exact contracts together. A
