...
| **Creator Runtime Scenario V1** | Current-only exact-project-bound diagnostic manifest with a bounded ordered tick-action prefix, exact final-proof assertions and failure-preserving prefix minimization through the production headless session; it is not project, save/replay or gameplay authority. |
...
| **WorldDynamicsStep** | Proposed successor name for stage 8 when a non-physical world owner participates; it preserves the current twelve-stage order and does not rename or expand the current physical-only profile. |
| **WorldDynamicsCompositionProfile** | Proposed closed runtime-owned identity of active owners/edges, exact access sets, DAG/order, cadence, reducers, capacities, roots/receipts, checkpoint segments, fault domain and performance row; it permits exactly one merged PhysX integration. |
| **CanonicalFloatExecutionProfile** | Promotion closure for authoritative private floating-point work: toolchain/target features, FMA, rounding/subnormals, math primitives, reduction/factorization/tie order and convergence branches. Final fixed-point quantization alone is insufficient. |
| **Checkpoint epoch** | Fixed positive simulation-tick interval at whose every boundary a PhysX-coupled exact-continuation profile rebuilds and validates a fresh scene, independent of save requests. |
| **WorldDynamicsFaultDomainProfile** | Proposed exact containment identity with `Running -> Faulted -> DiagnosticSaved -> Closed \| ExplicitRestore`; the first primary gameplay world maps a fatal dynamics fault to the application session. |
| **PendingFracture** | Proposed bounded structural fact committed at stage 8; a stage-9 Outcome command consumes it through `PhysicsTopologyTransaction`, and the derived rigid body activates next substep. |
| **StructuralContactLoadBatch** | Proposed consumer-specific exact quantized impulse/moment batch with explicit reference point, stable one-use load identity and equal-and-opposite reaction receipt; generic contact events do not substitute for it. |
