...
| Открывать и закрывать production application exactly once | `fast`, `play`, `persistence-replay` | `game`, `headless` и runtime-bearing `tools` проходят один session coordinator; legal edges публикуются атомарно, exact retry возвращает прежний event/receipt, collision и publication fault сохраняют prior generation, required-save exhaustion остаётся `Finalizing`, recovery создаёт ровно одну linked live session | SPEC-01, SPEC-17, SPEC-29, ADR-028 |
