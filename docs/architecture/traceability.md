...
| Requirement | Нормативное требование | Primary owner | Contributors / required approvers | RFC / ADR | Gate | Evidence |
|---|---|---|---|---|---|---|
| REQ-009 | RuntimeEntityId не сериализуется; PersistentId/AssetId nominal/stable; runtime-spawn PersistentId выводится из causal command | Runtime Team | Persistence | SPEC-02, ADR-012 | RUNTIME-04/07, CONTRACT-01 | schema/API scan, spawn/retry/collision vectors |
| REQ-010 | Gameplay меняется только validated WorldCommand; extensions выдают только bounded proposals/commands | Runtime Team | RPG Framework | SPEC-02, SPEC-07, ADR-005/014 | RUNTIME-02, AI-03, PLUGIN-P1 | audit/fuzz/command trace |
| REQ-011 | Fixed schedule, arrival-independent command order и async commit points воспроизводимы | Runtime Team | — | SPEC-02, ADR-012 | RUNTIME-01/03/06 | state hashes, permutation/stage trace |
...
| REQ-013 | Immutable content-addressed bundles, canonical hashes и deterministic cooking | Asset Team | Runtime Team | SPEC-03, ADR-012 | ASSET-01/02, CANON-01, VS-01 | cross-target golden hashes, cache/validator report |
| REQ-014 | Versioned saves, migrations, command ledger и fail-closed corruption | Persistence Team | Runtime Team | SPEC-03, ADR-012 | SAVE-01/02, RUNTIME-07, VS-02 | power/migration matrices, ledger closure |
| REQ-015 | Canonical replay command stream и first-divergence diagnostics | Runtime Team | Persistence, Developer Experience | SPEC-02/03/09, ADR-012 | RUNTIME-06, CANON-01, REPLAY-01, VS-11 | replay diff, ordered ledger, state roots, RunManifest |
...
| REQ-019 | Physics владеет active avatar pose; renderer читает snapshot | Physical Embodiment | — | SPEC-05, ADR-013 | PHYS-P1/P3, VS-05 | state/replay/pose reports |
| REQ-020 | PhysX Proposed, replaceable через self-contained PhysicsBackend contract | Physical Embodiment | — | SPEC-05, ADR-013 | PHYS-P1…P7 | backend gate suite |
...
| REQ-023 | Physical gates всегда имеют baseline/success/failure/comparison media, привязанные к local physical contract | Physical Embodiment | Verification & Evidence | SPEC-05, SPEC-09, ADR-013 | PHYS-P8, MANIFEST-01 | GIF/MP4 + SHA-256 manifest |
...
| REQ-031 | Luau capability sandbox и deterministic instruction/allocation budgets | RPG Team | Security & Governance | SPEC-07, ADR-014 | SCRIPT-P1…P5, VS-06 | sandbox, CPU/load permutation, watchdog/replay reports |
| REQ-032 | Wasm/WIT plugin caps/fuel/memory/version negotiation и package trust | RPG Framework | Security & Governance | SPEC-07/11, ADR-014 | PLUGIN-P1…P5, MOD-P2, VS-06 | audit/fuzz/compat/signature matrix |
...
| REQ-046 | Data-first → Luau → Wasm tiers; native Rust не является community mod ABI | Gameplay Extensibility | RPG Framework, Security & Governance | SPEC-07/13, ADR-008/014 | MECH-01, MOD-02/P2 | package/API/SBOM/trust scan |
...
| REQ-048 | MCP только optional local projection; CLI/JSON остаётся normative complete fallback | Developer Experience | Security & Governance | SPEC-09/13, ADR-008 | MCP-P1, TOOL-03, SEC-05, VS-13 | schema parity, audit, packet capture |
...
| REQ-051 | Planner-visible package abilities self-describing и доступны NPC без custom AI integration | Agent Intelligence | Gameplay Extensibility | SPEC-06/13 | AI-06, MECH-07, VS-13 | affordance registry, planner traces/replays |
...
| REQ-061 | Save/replay фиксируют exact archetype/model/route hashes, transition и versioned bounded PolicyState | Persistence Team | Physical Embodiment | SPEC-03/14, ADR-009/012 | POLICY-02, SAVE-01/02, VS-11/14 | save/replay manifests, state/route oracle |
...
| REQ-068 | Engine-owned ImpactResolver генерирует required gates; AgentChangeSet не может уменьшить plan, unknown impact включает maximal suite и human review | Verification & Evidence | Security & Governance | SPEC-13/15, ADR-015 | IMPACT-01, REVIEW-02, VS-15 | dependency/ownership graph, mutation-corpus impact manifests |
...
| REQ-070 | Clean CPU-only Linux agent выполняет simulation/tests/diagnostics без GPU, display server, window calls или display environment | Runtime Team | Verification & Evidence | SPEC-01/15, ADR-015 | HEADLESS-01, VS-15 | container manifest, display/socket/API trace, RunManifest |
| REQ-071 | Portable CaptureJobManifest воспроизводит exact replay на displayless Vulkan worker без window/surface/swapchain и не меняет gameplay outcome | Rendering Team | Verification & Evidence | SPEC-01/04/15, ADR-015 | CAPTURE-01, RENDER-04, VS-15 | job/result manifests, API trace, null/capture gameplay hashes |
| REQ-072 | Raw frame/audio roots canonical; synchronized GIF/MP4 — review derivatives; FFmpeg Proposed и проходит MEDIA-P1 либо declared fallback | Verification & Evidence | Security & Governance | SPEC-08/09/15, ADR-015, EVIDENCE-001 | AUDIO-02, MEDIA-P1, REVIEW-02, VS-15 | PNG/WAV roots, encoder/SBOM/decoder report, media hashes |
| REQ-073 | EvidenceBundleManifest — content-addressed atomic closure impact/results/replays/metrics/media, внешняя к Git и проверяемая offline | Verification & Evidence | Security & Governance | SPEC-03/09/15, ADR-015 | EVIDENCE-01, MANIFEST-01, REVIEW-02, VS-15 | bundle closure/hash/quota/redaction/attestation reports |
...
| Failure requirement | Primary owner | Contributors / required approvers | Нормативный путь | Gate | Evidence |
|---|---|---|---|---|---|
...
| FAIL-003 Plugin/script overrun or denied capability | RPG Framework | Security & Governance | ADR-014: terminate callback/instance, discard proposals, continue/clean pre-world fail; wall watchdog cannot change outcome | SCRIPT-P2/P5, PLUGIN-P1/P2, VS-06 | CPU/load/watchdog permutations, budget/audit/replay |
...
| FAIL-007 Malformed/protected importer data | Importer | Security & Governance | ADR-015: sandbox abort/quarantine/no publish; isolate import-smoke and clean prohibited roots before neutral run | IMPORT-P3/P5, PRIVACY-02, VS-09 | fuzz/scan/quarantine/cleanup record |
...
| FAIL-010 Unsafe/stale agent change | Developer Experience | Security & Governance | changeset reject before mutation; project/base hashes unchanged | AGENT-02, SEC-05, VS-13 | changeset audit/fault report |
...
| FAIL-019 Required GPU, encoder or authorized reviewer unavailable | Release Engineering | Verification & Evidence | publish portable pending job and set `AwaitingCapability`; never emit `PASS` or synthetic approval | CAPTURE-01, MEDIA-P1, REVIEW-01/02, VS-15 | capability report, pending job/bundle, admission denial |
...
| FAIL-021 Missing, stale or tampered baseline, evidence bundle or review decision | Security & Governance | Verification & Evidence | ADR-015: fail verification/invalidate approval; rebuild exact bundle and obtain new human decision after automatic PASS | EVIDENCE-01, REVIEW-01/02, SEC-07, VS-12/15 | hash/closure/staleness/tamper/trust corpus, invalidation log |
| FAIL-022 Malicious/oversized/redaction-failed media or dossier, HTML injection or reviewer-key exposure | Security & Governance | Verification & Evidence | ADR-015: reject/quarantine evidence, revoke exposed credential, sanitize/rebuild and repeat review; no viewing/admission before verification | EVIDENCE-01, REVIEW-02, SEC-07, VS-12/15 | quota/redaction/HTML/media fuzz report, quarantine/revocation audit |
