...
| Детерминированно проходить current packaged two-chunk alpha | `play`, `content-package`, `persistence-replay`; `performance --scenario smoke --mode report` | exact `WorldPartitionManifestV1`/content closure активируется целиком и фиксируется `PinnedContentGeneration`; relay↔frontier использует bounded packaged fetch/decode/validate и paired `IngressCommit → WorldStreamingCommit → PhysicalStep` publication за два существующих ticks. Save после `Requested` повторно fetch-ит package после restart; worker/completion order и I/O timing не меняют final root, а corrupt/stale/oversized input сохраняет declared Requested root и прежнюю active generation | SPEC-03, SPEC-25, ADR-026, ADR-048, ADR-051 |
...
| Не ухудшать затронутый hot path | conditional `performance` | `PerformanceRunV4` сохраняет raw samples, nearest-rank p50/p95/p99, methodology, full fingerprint, canonical logical charges, peak working set, process I/O, device allocation ceilings, Vulkan timestamps, profiler integrity и exact authoritative roots. Smoke выполняет 1,000 real packaged streaming transitions и заполняет существующий `required_staging_bytes`; 30-second предел остаётся report-only. Все пять реализованных workload сохраняются: smoke, long-session, interactive-frame, production-worker и `r2-alpha-render`; diagnostics имеют `REPORT_ONLY`, wrong host/workload/preflight — `NOT_RUN`. Только compatible clean ten-run `release` baseline на полном `ref-win-thoth-v1` даёт hard timing verdict. Absolute overrun или >=5% regression с 95% interval — `FAIL`; report-only scenarios и Accepted boundaries не закрывают B-12 | SPEC-03, SPEC-04, SPEC-05, SPEC-06, SPEC-08, SPEC-09, SPEC-12, SPEC-25, SPEC-26, SPEC-27, SPEC-30, ADR-016, ADR-036, ADR-038, ADR-045, ADR-049, ADR-051 |
