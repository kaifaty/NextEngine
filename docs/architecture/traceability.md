...
| Играть в малый RPG loop полностью offline | `play` | boot, movement, interaction, item и bounded NPC/quest outcome работают без network/`ai-host` | SPEC-00, SPEC-01, SPEC-06, SPEC-19, SPEC-20 |
| Сохранять authoritative world и воспроизводить bug | `persistence-replay` | load и replay дают ожидаемые command/event/state roots; corrupt input не активируется частично | SPEC-02, SPEC-03, SPEC-21, SPEC-22 |
| Загружать neutral content и public mechanic packages | `content-package` | validate/cook/resolve/load и first-party mechanic проходят через public paths | SPEC-03, SPEC-07, SPEC-13, SPEC-17, SPEC-24 |
...
| Развивать живые quest opportunities без сетевой зависимости | `play`, `persistence-replay` | NPC intent/world need становится Quest только после admission; direct/solicited/contextual/public disclosure, activation, autonomous outcome и exact recorded candidate/template fallback проходят тот же RPG command path и replay | Proposed SPEC-31 |
