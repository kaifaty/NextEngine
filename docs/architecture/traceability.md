...
| Не ухудшать затронутый hot path | conditional `performance` | versioned run сохраняет raw samples, nearest-rank p50/p95/p99, methodology, full fingerprint и exact authoritative roots. Только compatible `release` baseline на полном `ref-win-thoth-v1` даёт hard timing verdict; Linux timing `REPORT_ONLY`, wrong host/workload/preflight — `NOT_RUN`. Absolute overrun или >=5% regression с 95% interval — `FAIL`; smoke не закрывает B-12 | SPEC-04, SPEC-05, SPEC-06, SPEC-08, SPEC-09, SPEC-12, SPEC-16, SPEC-23, SPEC-26, SPEC-27, SPEC-30, ADR-016, ADR-036 |
