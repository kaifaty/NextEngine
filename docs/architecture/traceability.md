...
| Детерминированно проходить current two-chunk alpha | `play`, `content-package`, `persistence-replay` | exact `WorldPartitionManifestV1`/content closure активируется целиком; relay↔frontier transition и возврат сохраняют durable RPG state, а corrupt/missing chunk closure отклоняется до world publication | SPEC-03, SPEC-25, ADR-026, ADR-048 |
