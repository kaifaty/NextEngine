...
| Rendering, Vulkan, shaders, presentation extraction, render content | [SPEC-04](04-rendering-and-platform.md), [SPEC-30](30-presentation-extraction-and-render-content.md) | ADR-003, ADR-028 (+ ADR-035), ADR-090 for Linux-only release/host execution; ADR-101 for declared dynamic surface rings and developer frame capture; ADR-102 for the particle surface pass | play (+ platform при host/packaging); `RENDER-DYNSURF-P1` when a dynamic surface consumer lands; `RENDER-PARTICLE-SURFACE-P1` for the particle pass |
...
| Physics world, collision, constraints, queries, canonical snapshots and PhysX SDK/FFI/backend | [SPEC-26](26-physics-world-collision-constraints-queries-and-canonical-snapshots.md), [SPEC-35](35-deterministic-humanoid-training-substrate.md), [SPEC-05](05-physics-animation-and-motor-control.md) | ADR-027, ADR-032, ADR-058, ADR-059, ADR-062, ADR-063, ADR-064, ADR-065, ADR-067, ADR-069 for biomechanics V2; ADR-070 for its tracker consumer; ADR-071 for material/ABI lineage; [ADR-100 (standing)](adr/100-bounded-standing-reward-profile.md) for bounded standing V2; [ADR-101 (standing)](adr/101-biomechanics-command-only-standing-environment.md) for command-only biomechanics standing; [ADR-100 (water)](adr/100-authoritative-water-volume-and-presentation-only-gpu-water.md) for the water-volume table and [ADR-103 (water)](adr/103-authoritative-water-flow-network.md) for the flow network in the checkpoint | `physical-character` when the bounded trip/carry/melee profile changes; the focused `free_body` tests plus `physics-backend-parity`, `physics-collision`, `play` and `persistence-replay` when the dynamic box integrator or the body mass rule changes (WR1); `water-volume` when the water table, command or query changes; `water-flow` when the network, its step or command changes; `water-buoyancy` when the batch law, the step-input impulse field or the crate scene changes; fast, play, persistence-replay, platform; conditional performance |
...
| Continuum materials: water (Accepted ADR-100/103/104/105, SPEC-38 3.0) and future sand, mud, soil, snow, deformable terrain and off-road coupling (Proposed SPEC-39) | [SPEC-38](38-continuum-material-physics.md), [SPEC-39](39-deformable-terrain-and-other-material-lanes.md), [SPEC-26](26-physics-world-collision-constraints-queries-and-canonical-snapshots.md), [SPEC-21](21-deterministic-runtime-primitives-command-ledger-and-causal-identity.md), [SPEC-03](03-assets-world-streaming-and-persistence.md), [SPEC-25](25-world-partition-streaming-admission-and-persistent-spatial-objects.md), [SPEC-30](30-presentation-extraction-and-render-content.md); add [SPEC-04](04-rendering-and-platform.md) for presentation water | ADR-104 (water authority ladder), ADR-100, ADR-103, ADR-105 (buoyancy batch), ADR-106 (Accepted 1.0: PhysX particle fluids as the optional presentation lane; read before touching the PhysX bridge, `apps/game --physx-water` or the particle pass for water), ADR-101, ADR-102, ADR-076 (terrain and the one-pass step), ADR-081, ADR-058, ADR-027, ADR-046, ADR-026, ADR-071 | `water-volume` (CONTINUUM-WATER-VOLUME-P1) plus play and persistence-replay for the authoritative table; `water-flow` (CONTINUUM-WATER-FLOW-P1) for the ADR-103 network; `water-present` (CONTINUUM-WATER-PRESENT-P1, plan `continuum-water/09`) for the presentation stage in the game root; `water-buoyancy` (CONTINUUM-WATER-BUOYANCY-P1, ADR-105, plan `continuum-water/08`) for the exact impulse batch; `water-lattice` (CONTINUUM-WATER-LATTICE-P1, SPEC-38 practices 1-3, plans 19-21); `physx pbd-probe` (PHYSX-WATER-PRESENT-R1, research report, ADR-106 plan 23); the four water product checks of ADR-104 are in place; REF/MIRROR/`CONTINUUM-PARTICLE-COUPLING-R1`/`CONTINUUM-PARTICLE-PERSISTENCE-R1` are research reports and TERRAIN a separate lane; content-package, platform and conditional performance as before; runtime resume surface `docs/development/task-state/water-volume-authority.md`, developer bridge evidence `docs/development/task-state/nonlocal-gpu-full-step-performance.md` |
