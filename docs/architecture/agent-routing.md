...
| First-party model-training environments, trajectories, datasets, runs, export, consolidation and accelerated mirrors | [SPEC-34](34-model-training-environments-trajectories-and-consolidation-lifecycle.md), [SPEC-15](15-headless-testing-agent-validation-and-human-evidence.md), plus the lane SPEC | ADR-053, ADR-030, ADR-046, plus the lane ADR; ADR-067 for current humanoid profile identity | SPEC-35 standing/flat-command/curriculum reset-step-recorder and mirror use current MOTOR-LOCOMOTION-ENV/MODEL-DATAPLANE/MIRROR gates; all other lanes remain Proposed until a consumer |
...
learned-Motor profiles beyond the SPEC-35/ADR-069 biomechanics substrate under
