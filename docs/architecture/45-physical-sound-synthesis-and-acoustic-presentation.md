...
| Replaces | SPEC-45 0.49; records the repeated Pitcher geometry-spatial rejection while retaining all material, quality, admission and production blocks |
| Latest evidence | [PS-2 REALIMPACT Pitcher calibration](../development/physical-sound-realimpact-pitcher-calibration-ps2-2026-08-28.md) |
...
The repaired Pitcher calibration now completes twice and rejects the candidate
byte-identically at report `8bd5323c…1aea`. Thirteen modes satisfy numeric and
coverage admission, but frequency mapping misses at median/p90
`0.5664/1.6260` octave. Every held 3D stratum fails: medians are
`19.90–23.44 dB`, p90 values are `45.24–57.39 dB`, and the candidate is
`2.7039×` worse than the frozen RBF median with `+27.58 dB` p90 regression.
The exact Rust projection also repeats. This is an immutable rejection of the
scalar cotangent-biharmonic/Bempp/full-angular proxy for Pitcher, not a Bempp
convergence failure and not a general rejection of physical synthesis. Planter
must remain sealed, the authored clip fallback remains active, and subsequent
work must use a separately frozen unopened development object to discriminate
thickness/interior shell mechanics, support/excitation coupling and metadata/
observation mismatch. Opened Pitcher gates or cooker degrees MUST NOT be tuned.
