...
| Replaces | SPEC-45 0.47; records the one Pitcher prefix/decode and serializer-only offline repair while retaining all measured-transfer, material, quality, admission and production blocks |
| Latest evidence | [PS-2 REALIMPACT Pitcher serializer repair](../development/physical-sound-realimpact-pitcher-serializer-repair-ps2-2026-08-28.md) |
...
manifest `603c1185…28e3` changes only comparison-result conversion to built-in
JSON booleans, reconstructs every other field from original execution manifest
`8e791327…ba45`, and prohibits `acquire` and `decode`. Its two local preflights
repeat at `b24e7c90…5036`. Offline analysis repeat is next; Planter remains
sealed and no physical-transfer credit is available.
