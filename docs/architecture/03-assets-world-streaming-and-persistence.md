...
| Заменяет | SPEC-03 2.10; records `PhysicsWorldCheckpointV1` schema version 3 (ADR-103 water flow network as field 5) inside the unchanged `WorldCheckpointV4` composition |
...
The physics checkpoint is schema version 3 since R8d: it carries the
Proposed ADR-100 water-volume table as field 4 and the Proposed ADR-103
water flow network as field 5 (SPEC-26 2.6), so earlier checkpoint bytes
are current-only alpha artifacts.
