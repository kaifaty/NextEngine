...
| Заменяет | SPEC-03 2.11; records `PhysicsWorldCheckpointV1` schema version 4 (ADR-105 buoyancy batch profile as field 6) inside the unchanged `WorldCheckpointV4` composition; 2.11 recorded schema version 3 (ADR-103 water flow network as field 5) |
...
importer/cooker backend. Only an explicit validated atomic promotion turns one
captured exact candidate into ordinary project source.

Deterministic cooking starts from those promoted source bytes and their exact
hashes. Prompt, provider, model, seed, receipt or remote task identity cannot
replace source hashes and is never consulted by cook, package, activation,
save or replay. Provider unavailability therefore has no effect on the current
offline authored-source path. This section constrains a future consumer; it
adds no current schema, command or generation dependency.
...
The physics checkpoint is schema version 4 since the ADR-105 increment: it
carries the Proposed ADR-100 water-volume table as field 4, the Proposed
ADR-103 water flow network as field 5 and the optional Proposed ADR-105
buoyancy batch profile as field 6 (SPEC-26 2.8), so earlier checkpoint
bytes are current-only alpha artifacts.
...
- `content-package`: deterministic cook and activation of the complete
  four-region/64-chunk, 125-entry/39-root closure plus malformed/cyclic/missing content
...
- `performance --scenario r4-100npc --mode report`: 1,000 warm-up plus 10,000
  measured production ticks preserve exact due/tier-cognition/application/
  ledger roots with no drop/starvation/fabricated outcome; unsupported-host
  timing remains `NOT_RUN` and cannot close B-12.
