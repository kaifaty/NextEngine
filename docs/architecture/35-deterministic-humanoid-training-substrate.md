...
| Заменяет | SPEC-35 3.19; adds corrected-body canonical V9 environment and pipeline smoke |
| Дополнительные зависимости V3.20 | [ADR-126](adr/126-corrected-body-walking-environment.md) |
...
| Дополнительные зависимости V3.18 | [ADR-124](adr/124-bandwidth-settling-and-standing-diagnostics.md) |
| Дополнительные зависимости V3.19 | [ADR-125](adr/125-support-effort-calibration-diagnostic.md) |
...
[ADR-119](adr/119-articulated-foot-standing-diagnostics.md) adds exact-V8-bound
contact classifier V2, articulated terminal construction and standing reference
V3 for bounded native diagnostics. Keep the6 Ns anatomical-foot vector ceiling
across raw rear/toe contacts and all old per-pair limits; track anatomical support
continuity without replacing per-pair evidence. Distinct contact/reference roots
bind body and subject; old profiles remain exact. Neutral MTP targets plus the
retained upright law are an experiment, not dynamic robustness or learning
admission. Loaded heel rise/re-contact and disturbances still need evidence.

[ADR-120](adr/120-stiffness-proportional-damping-diagnostic.md) adds opt-in V9
with all25 damping coefficients derived by one exact stiffness-proportional
rule. Geometry/mass/stiffness/safety and old identities stay unchanged. Explicit
V9 diagnostic constructors reuse the contact/reference/terminal algorithms,
with new contact and standing-V4 identities and full compiled-input validation.
Local sampled-model stability is not native balance or training admission.

[ADR-121](adr/121-screened-damping-native-diagnostic.md) adds opt-in V10 with
eight fixed screened damping coefficients and explicit exact-body diagnostic
consumers. All anatomy/K/safety and old identities remain. New standing-V5 and
...
[ADR-126](adr/126-corrected-body-walking-environment.md) admits explicit native
walking V9: exact V11/CompiledV4 body,25 PD residuals around walking referenceV1,
94 observations and anatomical rearfoot+MTP contacts/heights. V8 reward formulas
use25-channel normalizations and the explicitly defined anatomical-foot proxies.
Old profiles remain immutable. Only the frozen16384-sample pipeline smoke is
admitted; final five-seed observations are report-only, not calibrated-body or
walking-quality admission. Diagnostic standing/support laws are not inherited.

[ADR-125](adr/125-support-effort-calibration-diagnostic.md) additionally admits
an exact-V11 diagnostic support-effort input added to rounded PD before the
complete unchanged safety intersection. Separate compiled-bound safety roots
and composite diagnostic IDs distinguish this law; old zero-input consumers
retain their identities. The f64 support generator is not a runtime fallback,
measured contact force, learned action or calibrated/training body selection.

[ADR-124](adr/124-bandwidth-settling-and-standing-diagnostics.md) adds two-second
return observation and explicit exact-V11 standing/contact/terminal consumers,
preserving prior inputs, full safety and all body gains. It is diagnostic-only.
...
remain; all ground contacts reject the unloaded fixture. No standing consumer,
gravity-support, disturbance or training admission follows from this tool.
