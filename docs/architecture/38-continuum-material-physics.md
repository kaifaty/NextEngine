...
3. **Presentation is spawned at edges (implemented 2026-09-03, plan `continuum-water/21`).** Jets, falls, splashes and foam
   ...
   authority split). Rule (3.3): the stage's frame carries one
   `WaterEdgePresentationV1` per edge with non-zero flux in edge id
   order (`Jet` for pipes and gates, `Fall` for an open sill whose sink
   lies more than `20 mm` below it, `Sill` otherwise, `Mouth` for
   sources, sinks and pumps) with the crest, plan direction, levels and
   flux; falls shed droplets through the jet's stateless lane inside its
   `4,096` bound; no surface exists without an authored binding.
4. **Rotational flow is presentation (authored vortex implemented 2026-09-03, plan `continuum-water/22`; the shallow-water grid stays planned).** Whirlpools, eddies and flow
   ...
   exact edge flux and the cell geometry, never the visual field. Rule
   (3.4): the stage adds, to the bound ring of a volume, one vortex per
   sink on it and per pump drawing from it with non-zero flux: radius
   `0.4 m`, dip `min(12 mm, |flux| / 2)` micrometres per cubic
   millimetre of tick flux, a `3 mm` two-arm spiral with one radial turn
   per `150 mm` and one revolution per second, all inside the ring cap.
