...
| `LNX-004` | `WAITING_ON_WINDOWS_PACKAGE` | После завершения minimal render content на Windows | Проверить neutral content cook/load, Vulkan B0 upload/draw, fallback material и relevant performance/package paths. Можно объединить с `LNX-003` одним полным gate на более новом checkpoint. | Linux side первого R2 render-content increment. |
