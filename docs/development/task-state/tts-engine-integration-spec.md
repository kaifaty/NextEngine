...
- **Next action:** Implement T0/T1 from SPEC-36: engine-owned protocol contracts and one supervised mock/real host request with authored fallback, without yet adding propagation breadth.
...
- **Normative result:** Proposed SPEC-36 and ADR-075 define the bounded vertical; SPEC-08, SPEC-16, SPEC-30, ADR-017, index, routing, glossary and traceability are synchronized without changing current shipped status.
- **Checks:** `git diff --cached --check` passed; every relative link in all changed documents resolves; SPEC/ADR IDs plus index/routing/traceability rows are unique. Cargo, `host-check`, `play`, `persistence-replay`, `content-package`, `platform` and `performance` are `NotRun(NoExecutableChange)` under the documentation-only cheap path.
