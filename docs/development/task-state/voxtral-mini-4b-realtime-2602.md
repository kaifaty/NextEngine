...
| Scope | Determine whether useful quantizations run on the workstation RTX 3080 and assess available quality evidence |
| Definition of done | Identify supported artifacts, verify one recommended quant locally, and distinguish measured quality from inference |
...
- **Current conclusion:** Use GGUF Q4_K_M through transcribe.cpp on the 10 GB RTX 3080; it was locally verified at about 4.08 GB peak VRAM and 3x realtime.
- **Why:** Local CUDA runs passed, including an exact Russian sample; the published English WER is 2.08%, equal to the BF16 reference within noise.
...
- **Workspace state:** Two documentation files added; downloaded model and build remain only under `/tmp/codex-voxtral-research`.
- **Checks:** Local CUDA Q4_K_M runs, `git diff --check`, and direct link validation passed.
