...
- **Next action:** Run the optional pinned DPDFNet ASR-only profile on paired normal/whispered user phrases through the five-WAV diagnostic buffer, recording VAD admission, raw/enhanced RMS/peak, ASR text and per-chunk p50/p95. Keep raw VAD/affect and all prior profiles as rollback; add gain only if this A/B proves denoising alone is insufficient. LLM/TTS and FunctionGemma remain out of scope.
...
| Pinned DPDFNet streaming prototype, 2026-08-20 | `dpdfnet==0.6.0`, `Ceva-IP/DPDFNet` commit `dd6818d00f50c836fed43a6243ebe49116de5964`, `dpdfnet2.onnx` SHA-256 `4f0ee28935b4a32abecc717d745416976565834d839601acf43031094b4dc94c`; offline 80-ms CPU probe is p50/p95 `11.45/13.32 ms`, then real resident paced replay is `13/16 ms`, zero overloads and exact `89856/89856` samples. Native `flush()` omitted a 6-ms fractional tail; the adapter drains/trims it. Raw p90 fell `−57.64→−76.47 dBFS`; no ASR-quality claim yet. | Optional `dpdfnet-streaming/1` profile routes enhanced PCM only to ASR on a separate CPU worker; raw VAD/affect/diagnostics stay unchanged. It is a local prototype, not a promoted native/Rust or product default. |
