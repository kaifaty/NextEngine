...
- **Next action:** User manually tests the separately pinned `nikatonika/aniemore-audio-finetuned` profile now resident at the diagnostic endpoint; then run its exact RESD-70 v2 direct/full-path screen before any default claim. Retain the prior WavLM and base profiles as rollback. LLM/TTS and FunctionGemma remain out of scope.
