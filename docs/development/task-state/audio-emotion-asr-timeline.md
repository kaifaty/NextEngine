...
- **Next action:** Freeze a small reference-transcribed Russian microphone set, then benchmark pinned `gigastt` as `buffered_emulation` against Voxtral and offline GigaAM. Use that measured baseline to decide whether the bounded 1 s/2 s native GigaAM chunk/cache feasibility spike is justified. Do not train before architecture parity and zero-training negative controls.
...
2. Benchmark pinned `gigastt` as bounded buffered emulation. If native GigaAM adaptation is then authorized, prove full-context parity and measure zero-training fixed 1 s/2 s behavior as negative controls before fine-tuning; otherwise retain the two-pass boundary. Keep Nemotron out of the active profile.
