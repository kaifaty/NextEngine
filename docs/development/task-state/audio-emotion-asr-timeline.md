...
| Field | Value |
| --- | --- |
| Status | `PHASE_1_IMPLEMENTED`; pinned Russian WavLM affect profile integrated for local evaluation |
| Updated | `2026-08-20` |
...
- **Current conclusion:** Phase 1 now implements the resident optional `SpeechTimelineService` with model-neutral Voxtral plus emotion2vec/WavLM adapters, bounded scheduling, three independently revisioned tracks, authenticated loopback WebSocket, 80 ms CLI/browser transport, drift-free browser resampling, same-origin Vue diagnostics and benchmark runner.
...
- **Next action:** User-test the explicit DPDFNet `pre_and_post_denoise` speech-gain profile on paired normal/whispered phrases: refresh the dashboard, click quiet calibration once, then compare VAD admission, text and raw/processed WAV. Keep raw affect and the `post_denoise`/raw branches as rollback; do not claim emotional-quality improvement before that A/B. LLM/TTS and FunctionGemma remain out of scope.
...
| Pinned DPDFNet streaming + gain prototype, 2026-08-20 | `dpdfnet==0.6.0`, `Ceva-IP/DPDFNet` commit `dd6818d00f50c836fed43a6243ebe49116de5964`, `dpdfnet2.onnx` SHA-256 `4f0ee28935b4a32abecc717d745416976565834d839601acf43031094b4dc94c`; offline 80-ms CPU probe is p50/p95 `11.45/13.32 ms`, resident post-only replay is `13/17 ms`, zero overloads and exact `89856/89856` samples. On external user whisper `record-a8d248d80e8a53dac16f610e17581c8c.wav`, post-only retained `21/388` frames above −75 dBFS (RMS −53.3); exact `pre_and_post_denoise` replay retained `91/388` (RMS −29.3), 124160/124160 samples, 60-ms algorithmic delay. Native `flush()` omitted a 6-ms fractional tail; the adapter drains/trims it. | Optional `dpdfnet-streaming/1` routes derived PCM only to ASR on a separate CPU worker; `gain_placement` makes the trial replayable, and raw affect/source clock stay unchanged. The old post-only mode is retained as rollback; quiet calibration only changes VAD. This is a local prototype, not a promoted native/Rust or product default. |
...
- **Consequence:** The local Python prototype exposes the same model-neutral resident seam for evidence only; the future native Rust DSP must expose it directly. ASR may use enhanced PCM and bounded speech-aware gain, while affect remains raw until an A/B gate. `DPDFNet` through local Rust-capable `sherpa-onnx` remains the first native candidate, with DeepFilterNet/RNNoise as alternatives; see [research](../audio-preprocessing-models-research-2026-08-19.md).
...
1. Measure the local optional DPDFNet `asr_only` profile on normal/whispered microphone pairs; retain one integer acoustic clock, raw-affect branch, exact lineage and immediate raw bypass.
