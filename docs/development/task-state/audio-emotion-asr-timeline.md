...
- **Next action:** Do not run `emotion2vec_plus_large` through the full path: it lost the frozen direct gate. Select the next Russian-capable candidate, pin it, and apply the same direct gate before any resident-service swap. Keep a separate paced latency benchmark and live microphone checklist. LLM/TTS and FunctionGemma remain out of scope for this increment.
...
| H4: 1 s/250 ms emotion windows give useful Live transitions | A frozen Russian RESD-70 v2 screen now shows `70/70` VAD admissions and observations, so the energy gate does not lose this acted material. | Base scores `29/60` direct versus `23/60` final timeline; the ten-point gap and eight final abstentions implicate temporal window/final aggregation, while natural microphone noise, probability calibration and live transitions remain unmeasured. The official large candidate lost the direct gate (`26/60`). | Pin a different Russian-capable candidate and require a direct v2 win before full-path comparison or a smoothing-policy experiment; evaluate a neural VAD only if a separate calibrated live gate fails. |
