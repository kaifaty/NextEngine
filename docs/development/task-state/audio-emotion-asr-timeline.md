...
- **Next action:** Freeze a small reference-transcribed Russian microphone set, then decide whether to run the bounded 1 s/2 s GigaAM chunk/cache feasibility spike described in `docs/development/gigaam-native-streaming-feasibility-2026-08-20.md`. Do not train before architecture parity and zero-training chunk controls.
...
| GigaAM native-streaming feasibility, 2026-08-20 | The paper pre-trains with dynamic chunk attention for later streaming fine-tuning, but pinned `e2e_rnnt` and upstream commit `7447938` use full-context attention/symmetric convolution and expose no cache-aware inference or streaming training flags; upstream issue 18 remains open. | A true derivative is plausible but requires our own chunk/causal encoder, cache-aware runtime and fixed-latency fine-tuning. Prefer two-pass streaming partials plus authoritative GigaAM final now; research: [GigaAM streaming feasibility](../gigaam-native-streaming-feasibility-2026-08-20.md). |
...
2. If GigaAM adaptation is authorized, prove full-context parity and zero-training fixed 1 s/2 s chunk behavior before any fine-tuning; otherwise retain the two-pass boundary. Keep Nemotron out of the active profile.
