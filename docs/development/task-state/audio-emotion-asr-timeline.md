...
- **Next action:** User-test the optional pinned DPDFNet+speech-gain ASR profile on paired normal/whispered phrases: click quiet calibration once, then compare VAD admission, text and the explicitly paired raw/ASR-enhanced WAV players. Keep raw affect and all prior profiles as rollback; do not claim emotional-quality improvement before that A/B. LLM/TTS and FunctionGemma remain out of scope.
...
| Pinned DPDFNet streaming + gain prototype, 2026-08-20 | `dpdfnet==0.6.0`, `Ceva-IP/DPDFNet` commit `dd6818d00f50c836fed43a6243ebe49116de5964`, `dpdfnet2.onnx` SHA-256 `4f0ee28935b4a32abecc717d745416976565834d839601acf43031094b4dc94c`; offline 80-ms CPU probe is p50/p95 `11.45/13.32 ms`, resident `DPDFNet+gain` replay is `13/17 ms`, zero overloads and exact `89856/89856` samples. Native `flush()` omitted a 6-ms fractional tail; the adapter drains/trims it. The bounded 32-dB, limiter-backed gain moves one quiet diagnostic RMS `−54.67→−39.18 dBFS`; paired raw/enhanced WAV persisted. No ASR/emotion-quality claim yet. | Optional `dpdfnet-streaming/1` routes denoised, gain-normalized PCM only to ASR on a separate CPU worker; raw affect and source clock stay unchanged. Quiet calibration makes the energy VAD relative to the noise floor. This is a local prototype, not a promoted native/Rust or product default. |
