...
- **Next action:** Replay the frozen external RESD-70 test manifest through the resident VAD/window/smoothing path and record admission, abstention and final-label metrics. Then compare the same manifest with the pinned `emotion2vec_plus_large`; do not tune thresholds or labels to the held-out set. Keep a separate live microphone checklist. LLM/TTS and FunctionGemma remain out of scope for this increment.
...
| External RESD-70 held-out screen, 2026-08-18 | 10 actor-stratified test clips × 7 labels were normalised externally to 16 kHz mono; pinned base scored `28/60` exact mapped top-1 (`46.7%`), with happy and neutral `3/10` each. Enthusiasm has no exact checkpoint label and yielded `other` on `6/10`. Model-only mean/max inference `27.6/341 ms`; report `/home/kaifaty/.cache/nextengine/emotion-calibration/resd-70-v1/emotion2vec-plus-base-report.v1.json`, SHA-256 `587b3e704e2d01604c0a365d5bc7924aae926dfe45a961bb610f97331ce0326e`. | Treat it as a fixed, small acted-speech screen, not a model-selection gate or probability-calibration result. Do not tune VAD/label semantics to it; replay the frozen clips through the resident path before comparing base to large. |
...
| H4: 1 s/250 ms emotion windows give useful Live transitions | Resident `energy-vad/1` gates no-speech, the browser can make a bounded quiet-room calibration that affects only VAD, and final output aggregates confirmed non-overlapping primary-expression segments. A frozen external RESD-70 screen now supplies a reproducible Russian acted-speech baseline. | The base model only reached `28/60` exact mapped top-1 on that whole-utterance screen; it does not exercise VAD/window/smoothing, natural microphone noise, probability calibration or live transitions. | Replay RESD-70 through the resident path with per-clip admission/abstention/final-label metrics, then compare the pinned base and large; evaluate a neural VAD only if calibrated energy VAD misses the declared gate. |
