...
| Pinned DPDFNet streaming prototype, 2026-08-20 | `dpdfnet==0.6.0`, `Ceva-IP/DPDFNet` commit `dd6818d00f50c836fed43a6243ebe49116de5964`, `dpdfnet2.onnx` SHA-256 `4f0ee28935b4a32abecc717d745416976565834d839601acf43031094b4dc94c`; 80-ms CPU probe on the latest diagnostic WAV is p50/p95 `11.45/13.32 ms`. Native `flush()` omitted a 6-ms fractional tail; the local adapter now drains and trims to an exact 16-kHz output clock (`179712/179712` bytes). Raw p90 fell `−57.64→−76.47 dBFS`; no ASR-quality claim yet. | Optional `dpdfnet-streaming/1` profile routes enhanced PCM only to ASR on a separate CPU worker; raw VAD/affect/diagnostics stay unchanged. It is a local prototype, not a promoted native/Rust or product default. |
