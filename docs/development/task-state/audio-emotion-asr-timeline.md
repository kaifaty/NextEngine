...
- **Next action:** First direct-screen the user-requested pinned `nikatonika/aniemore-audio-finetuned` WavLM against the exact six-class RESD-70 v2 map; only a win permits a profile-adapter A/B. Otherwise run the labelled user-microphone checklist against the current WavLM profile; retain base as rollback. LLM/TTS and FunctionGemma remain out of scope.
