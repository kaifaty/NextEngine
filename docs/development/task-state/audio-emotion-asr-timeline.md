...
| Field | Value |
| --- | --- |
| Status | `PHASE_1_IMPLEMENTED`; focused vocal-expression quality increment in progress |
| Updated | `2026-08-18` |
...
- **Next action:** Integrate the pinned Russian WavLM candidate behind the existing affect facade, preserving its native `enthusiasm` label; then deliberately stop the base service only long enough for a one-process Voxtral+WavLM residency/latency screen and restart the base rollback profile. Keep a separate paced benchmark and labelled live-microphone checklist. LLM/TTS and FunctionGemma remain out of scope for this increment.
...
| Pinned `Aniemore/wavlm-emotion-russian-resd` A/B, 2026-08-19 | Standard-Transformers revision `7a4ca18b34adff59b56b451acc7ff44fc43a12dc` (safetensors SHA-256 `dabf15d84b451195346b8050102a7243b3f92276064195f6e34b65aaa06a12ab`) scored `45/60` (`75.0%`) direct against v2 versus base `29/60` (`48.3%`); its production VAD/cadence/smoothing/WebSocket path with an explicit ASR stub scored `44/60` (`73.3%`), admitted/observed all 70 clips and had two final abstentions. Reports `/home/kaifaty/.cache/nextengine/emotion-calibration/resd-70-v2/aniemore-wavlm-russian-resd-direct-report.v1.json` SHA-256 `ed66c8e250695daf13805ef61888ed976f1aa35562ce1ab96fa93db1834c3489` and `/home/kaifaty/.cache/nextengine/emotion-calibration/resd-70-v2/aniemore-wavlm-russian-resd-affect-timeline-report.v1.json` SHA-256 `8fdfc51f47474c1888b575f2d5c62b9e238a21bb60db071bb065c0106c8e572f`. | WavLM is the integration candidate, not the new default. Preserve its native `enthusiasm` label; first implement a real adapter and pass one-process Voxtral+WavLM resource/latency plus labelled microphone gates. The base service remains the rollback. |
...
| H4: 1 s/250 ms emotion windows give useful Live transitions | A frozen Russian RESD-70 v2 screen now shows `70/70` VAD admissions and observations. Pinned Russian WavLM improved from `45/60` direct to `44/60` through the VAD/cadence/smoothing path, with only two abstentions. | The WavLM timeline trial stubs ASR to coexist with the user's live base service; natural microphone noise, joint GPU scheduling and live transitions remain unmeasured. | Implement the candidate adapter, then run isolated one-process Voxtral+WavLM paced residency/latency and labelled microphone gates before a default swap; evaluate neural VAD only if a separate calibrated live gate fails. |
...
- Deploy the ephemeral WavLM evaluation adapter or treat its ASR-stub timings as joint live latency.
