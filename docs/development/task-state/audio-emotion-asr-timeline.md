...
- **Next action:** Run direct and full-path evaluation of the pinned `emotion2vec_plus_large` against frozen external RESD-70 v2. Preserve its 16 kHz inputs, label map, VAD and smoothing policy; report direct/model and timeline/temporal metrics separately. Keep a separate paced latency benchmark and live microphone checklist. LLM/TTS and FunctionGemma remain out of scope for this increment.
