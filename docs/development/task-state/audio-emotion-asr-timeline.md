...
- **Next action:** Restart the resident service, calibrate the actual microphone in quiet, then record a small labelled silence/noise/expression checklist and establish base metrics before any model swap. LLM/TTS and FunctionGemma remain out of scope for this increment.
...
| H4: 1 s/250 ms emotion windows give useful Live transitions | Resident `energy-vad/1` gates no-speech, and the browser can make a bounded quiet-room calibration that affects only VAD. Final output aggregates confirmed non-overlapping primary-expression segments, not a transient terminal `unknown`; metrics expose coverage, abstention and churn. | The energy baseline remains unproven against representative Russian microphone noise; categorical scores are not calibrated probabilities and no labelled live corpus exists yet. | Establish a labelled silence/noise/expression baseline, then compare pinned `emotion2vec_plus_base` and `emotion2vec_plus_large`; evaluate a neural VAD only if calibrated energy VAD misses the declared gate. |
