...
- **Current conclusion:** The game-facing route must emit stateful streaming transcript revisions while the player speaks. GigaAM-v3 remains the strongest local Russian quality reference but is final-only. Its dynamically chunk-pretrained SSL ancestry makes a native streaming derivative plausible, but a maintainer confirms that the released checkpoints were ASR-trained full-context. The official runtime exposes no streaming caches. Community `gigastt` is now integrated as a bounded CPU Rust/ONNX rolling-window baseline and explicitly remains `buffered_emulation`, not native streaming.
...
| `codex/speech-timeline-service`, Phase 1 fake suite | 96 Python service tests + 14 preserved Voxtral probe tests pass | Auth/version/size/state/fault/dashboard boundaries, model/audio routing, GigaSTT sidecar contract, second-session residency, cadence, no-word-span fusion, bounded terminal metrics and client/benchmark paths are executable without weights. |
...
| GigaAM full upstream audit, 2026-08-20 | Official issue 70 confirms released checkpoints were trained full-context and inference-only local attention may noticeably regress; all official branches/history expose no cache-aware stream. Community `gigastt` 2.18.0 implements bounded 2.5 s rolling-window re-decode at 0.8 s stride / 1.5 s left context, but its own paired 100-clip tests report streaming WER `+10.60` to `+14.49` pp over batch. | A true derivative still requires our own chunk/causal encoder, cache-aware runtime and fixed-latency fine-tuning. Before that work, evaluate `gigastt` honestly as a no-training `buffered_emulation` partial-ASR baseline while offline GigaAM remains authoritative; research: [GigaAM streaming feasibility](../gigaam-native-streaming-feasibility-2026-08-20.md). |
| Pinned local GigaSTT route, one `7.216 s` paced raw take, 2026-08-20 | `gigastt 2.18.0` loaded as a resident CPU sidecar (`~283400 KiB` RSS, no CUDA); first transcript `1766.664 ms`, finish-to-final `105.998 ms`, busy RTF `0.014690`. Final `знает год у него хвостик есть один` arrived much earlier but diverged from final-only GigaAM's `Знает он, что у него хвостик есть один всего.`; Voxtral was empty on this take. | The model-neutral route and latency shape work, but one unreferenced take cannot establish WER. Keep `buffered_emulation` tentative-only and GigaAM final as the quality reference until paired labelled evaluation; implementation/evidence: [GigaAM streaming feasibility](../gigaam-native-streaming-feasibility-2026-08-20.md). |
...
### D-015 — Select ASR per turn and report capabilities honestly

- **Observation/evidence:** Voxtral preserved useful streaming partials but failed the whisper portion of the available mixed recording. Pinned GigaAM-v3 recognized the whispered and normal repetitions on that take and fits resident beside the existing models, but its available RNNT interface finalizes a complete utterance rather than producing trustworthy partial revisions.
- **Decision:** Advertise stable ASR route IDs and select one in `session.start`; lock that route for the turn. Keep Voxtral Realtime as the default streaming route. Expose GigaAM-v3 as `final-only`, `utterance` timing, Russian-only and bounded to 25 seconds. Both adapters use the same transcript/timeline events and exact model lineage.
- **Rejected:** Reloading a model when the selector changes, switching a live turn between models, inventing GigaAM partial text/timestamps, changing the product default from one successful recording, or leaking vendor response types into the facade.
...
### D-018 — Admit GigaSTT only as a bounded buffered-emulation baseline

- **Observation/evidence:** Pinned `gigastt 2.18.0` supplies 800 ms partials by re-decoding a rolling window capped at 2.5 s with 1.5 s retained context. On the same paced 7.216 s raw take it emitted text around 1.77 s and finalized about 106 ms after stop, versus text only after finish for GigaAM, but its final wording was materially weaker than GigaAM's result.
- **Decision:** Expose `gigastt-v3-rnnt-buffered` behind the existing ASR selector with `streaming_mode=buffered_emulation`. Keep one CPU sidecar resident, use manual endpointing under the outer turn boundary, and mark every partial tentative because the public GigaSTT WebSocket does not expose its internal committed/live split.
- **Rejected:** Calling rolling-window recomputation native streaming, publishing a fabricated stable prefix, enabling a second hidden VAD/endpoint authority, moving the CPU baseline onto scarce RTX 3080 VRAM, or replacing final GigaAM based on one microphone take.
