...
| H4: 1 s/250 ms emotion windows give useful Live transitions | Resident `energy-vad/1` gate now emits `speech_activity`, requires 600 ms/35% voiced coverage, and keeps raw emotion windows on the source clock. 5-s zero-PCM run produced zero affect jobs/observations; 3-s voiced tone produced 10 bounded affect jobs. | The energy baseline is not noise-robust enough to establish Russian emotion quality; upstream categorical scores remain uncalibrated and overlapping windows remain correlated. | Run live microphone plus silence/noise/voiced corpus with VAD false-positive/negative, affect coverage and boundary/churn metrics; evaluate Silero/WebRTC adapter only if energy-VAD misses the quality gate. |
