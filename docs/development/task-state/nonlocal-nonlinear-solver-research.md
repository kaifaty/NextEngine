...
- **R63ZN frozen next step:** revision 2 corrects the pre-code arithmetic
  ...
  recurrence.
- **R63ZN author package:** the exact revision-3 package now covers all eight
  candidate routes with a fixed 1,368-byte receipt, 67 candidate work fields
  and an independent 676-byte/48-field checker audit. Two Release runs and two
  clean Release builds are byte-identical. The complete corpus passes
  `116/116` twice with `116/116` distinct audit identities; Dev has the same
  control records, ASan/UBSan passes the same corpus, and every package
  allocation probe is zero. Accepted receipt `e0015763...f8da`, audit
  `d0a98ac6...2cc3`, report `745eafba...4a02`.
- **R63ZN claim boundary:** this is hash-closed author evidence only. Formal
  read-only review is pending, so no reviewed prefix, complete recurrence,
  certificate, portable representation, runtime or production authority
  exists. The next action is to freeze exact snapshot/diff/source/binary/input
  identities and send only that neutral package to the independent reviewer.
