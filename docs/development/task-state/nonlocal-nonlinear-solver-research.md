...
- **R63ZG frozen:** the profile's contraction multiplier is only about
  `1.00914x`. Cross tangent/common solution representations with tangent/common
  unamplified-image, center-only, radius-only and native global budgets; native
  cells must reproduce R63ZF exactly.
- **Next:** implement only the frozen R63ZG private detail DTO and separate-TU
  diagnostic. Do not change an operator, certificate, precision, iteration
  depth or tolerance.
...
3. Preserve reviewed R20R63ZB result semantic `acfe6baf...a709ebf`, stdout
   ...
   `CERTIFICATE_ENCLOSURE_AMPLIFICATION_CANDIDATE` interpretation. Research
   and freeze only the R63Y certificate-detail decomposition before another
   executable. R63ZG is now frozen: implement its private detail DTO and 18
   synthetic sign cells with immutable native reproduction. Do not apply a
