...
- **R63ZN frozen next step:** revision 2 corrects the pre-code arithmetic
  ...
  recurrence.
- **R63ZN preflight:** the external offset-only diagnostic reproduces
  `102/102` `x0` components, exact/no-underflow `r0`, exact `z0` and a
  certified-positive `rho0` lower bound. Two stdout captures are exact at
  `dccaf8c1...fc5f`. This proves reachability only; parser, work, receipt,
  controls and independent checker remain unimplemented.
- **R63ZN early rejection matrix:** the first 1,120-byte author receipt was
  superseded after independent reconstruction found repeated unowned input
  hashes, an omitted parent-set root and incomplete work categories. The
  repaired candidate/checker emit byte-identical two-Release 1,216-byte
  accepted receipts `4229e550...296c` and 548-byte audits
  `16875a04...fb2`; both allocation probes are zero. Public
  missing/short/trailing/same-size controls for all three inputs pass `12/12`
  with distinct audits and verified early routes. Late arithmetic rejection
  receipts, complete work/event controls and fresh review remain pending, so
  there is still no admitted R63ZN result.
...
1. Preserve the integrated R63ZM revision-5 source hashes, fixed artifact and
   ...
   ceiling. Preserve the R63ZN preflight only as reachability evidence and the
   tracked candidate receipt only as author evidence. Implement the separate
   revision-2 checker, rejection receipts and controls without modifying
   R63ZM. Derive `x0` before consuming
