...
- **Next:** implement only the frozen R63ZF separate-TU diagnostic. Do not
  change an operator, factor, precision, iteration depth or tolerance. Build
  observations and dev output have no solver-performance authority.
...
3. Preserve reviewed R20R63ZB result semantic `acfe6baf...a709ebf`, stdout
   ...
   `DENOMINATOR_STEP_SUFFICIENT` interpretation. Implement only frozen R63ZF:
   the binary128/twofold final quadratic, induced-step and raw-sign/certificate
   boundary audit. Do not apply a correction, fit a threshold, run timing or
