...
- **R63ZK contract frozen:** preserve R63ZJ output only as untrusted numerical
  input. The separate verifier independently derives all state transitions and
  certificates from the fixture, compares complete traces and owns distinct
  sealed candidate/replay work before any causal classifier. Implementation
  stops if it would need the author transaction scheduler as an oracle.
...
3. Preserve reviewed R20R63ZB result semantic `acfe6baf...a709ebf`, stdout
   ...
   `INCONCLUSIVE` and receives no third repair or review. Implement only the
   frozen R63ZK verifier-first trace-correspondence contract: independently
   derive all recurrence transitions and certificates, compare full traces,
   and seal candidate/replay work separately. Stop before portable
