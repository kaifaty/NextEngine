...
- **R63ZN author candidate:** the tracked standalone candidate independently
  parses all cache bytes, verifies all three exact parent files, reproduces
  the prefix and emits byte-identical Dev/two-Release 1,120-byte receipts at
  `91810c72...c245`. Separate checker, rejection receipts, full controls and
  work-ledger audit remain pending, so there is no admitted R63ZN result.
