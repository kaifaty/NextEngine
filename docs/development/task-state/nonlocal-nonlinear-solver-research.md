...
- **R63ZE frozen:** retain the tangent trajectory through `p1`, compute both
  final tangent/common products and denominators, then cross denominator choice
  with terminal residual-product choice. Exact solution/residual aliases and
  equal work distinguish scalar step-length sufficiency from residual-vector
  dependence.
- **Next:** implement only the frozen R63ZE separate-TU discriminator. Do not
...
3. Preserve reviewed R20R63ZB result semantic `acfe6baf...a709ebf`, stdout
   ...
   single re-review returned `GO`. Implement only frozen R63ZE: the equal-work
   final-product denominator/residual `2 x 2` factorial. Do not apply a
   correction, fit a threshold, run timing or
