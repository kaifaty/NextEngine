...
- **Current decision:** R20R62 selects strict binary64 `Dot2Err` intervals as
  the smallest runtime-plausible certificate arithmetic. Exact dyadics remain
  an offline oracle; mixed-precision refinement is not a verifier.
- **Next:** implement frozen R20R63 on the immutable R60 dimension-102 tuple:
  exact containment and two-sided contraction of the projected binary64
  inverse defects only. Do not evaluate a center or solver trajectory.
