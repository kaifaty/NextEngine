...
- **R63ZP next action:** build only a non-serialized exact-dyadic apparatus
  probe, publish all `102` containment results or the first escape and stop
  before candidate/checker receipts if containment fails.
...
1. Preserve the integrated R63ZM revision-5 source hashes, fixed artifact and
   ...
   product from rounded state differences. Implement only the frozen R63ZP
   exact-dyadic apparatus probe for direct `H*p0` value-plus-error containment
   at the exact R63ZM boundary. Stop before receipt/checker code if any
   component escapes; no recurrence, future state, representation selection or
   endpoint authority is permitted.
