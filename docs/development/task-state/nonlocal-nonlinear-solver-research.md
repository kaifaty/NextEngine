...
- **Next:** research and freeze R63ZF: exact/twofold audit of
  `p1^T(H_common-H_tangent)p1`, the induced `alpha1` difference and the R63Y
  certificate boundary. Do not change an operator, factor, precision,
  iteration depth or tolerance. Build observations and dev output have no
  solver-performance authority.
...
3. Preserve reviewed R20R63ZB result semantic `acfe6baf...a709ebf`, stdout
   ...
   `DENOMINATOR_STEP_SUFFICIENT` interpretation. Research/freeze only R63ZF:
   exact/twofold final quadratic-denominator, induced-step and certificate
   discrepancy. Do not apply a correction, fit a threshold, run timing or
