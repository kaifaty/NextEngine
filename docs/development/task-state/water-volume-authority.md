...
### D-012 — A player preference for the PhysX water lane (proposed, awaiting the user)
...
- **Proposal (agent, 2026-09-04):** keep the request on the run option
  (`--physx-water`, plans 25-30) until the lane is either retained as a
  shipped presentation option or dropped; add the preference as its own
  plan when that decision is made. Not built.
