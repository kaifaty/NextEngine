...
| Field | Value |
| --- | --- |
| Status | `ROADMAP_V40_ACTIVE / D0_COMPLETE / C0_NEXT / S0_OPEN_SOURCE_POWER_OOD / FALLBACK_REQUIRED` |
| Updated | `2026-09-03` |
...
- **Planning authority:** [Roadmap V40](../../plans/physical-sound-synthesis-roadmap-v40.md)
  has completed I0/D0; C0 disclosed corpus is next while S0 clean-source discovery proceeds independently.
...
- **Next action:** implement V40 C0: bind the D0 roster root, acquire only disclosed payloads into the external content-addressed store and emit canonical `48 kHz` segments, observed-axis masks, deterministic features and immutable role projections; S0 remains independent.
...
| V34–V40 R0–PR | `V37_D0_METRIC_REJECT_CLOSED / V39_MECHANICS_PRESERVED / V40_D0_COMPLETE / C0_NEXT / S0_SOURCE_POWER_OOD` | QSO-v0 remains closed. D0 freezes `5/2/2` disclosed roles without spending clean sources; C0 may build only that corpus, and a disclosed development winner never earns admission credit. |

## Material transition: V40 D0 permanent disclosed roster

- **Evidence:** D0 binds I0/F0 and the historical project split, assigns all nine disclosed families exactly once and repeats a four-artifact external tree with every forbidden access counter zero.
- **Conclusion/decision:** roles are frozen `5 train / 2 development / 2 validator`; descendants inherit the family role, clean-family spend and protected access are zero, and later quality cannot change the split.
- **Smallest next action:** build C0's external hash-closed disclosed corpus against D0 roster root; keep S0 and every protected role sealed.
...
| D-078 | D0 repeat-exactly freezes all nine disclosed families as `5 train / 2 development / 2 validator`, preserves zero clean-family spend and protected access, and grants only C0 disclosed-corpus sequencing. | C0 proves a frozen family alias or capability is invalid before model/feature values; repair requires a new pre-access roster revision, never an in-place role move. |
## Open hypotheses
...
| H4: automatic validator reaches useful bounded risk | V0a synthetic mechanics pass `9/7`; D0 reserves two disclosed projects only for calibration | I0 clean power is `9/7` and `SourcePowerOOD`; no real validator/shadow result | V40 C0/B0/V0 plus S0/S1, then one-shot V1 |
...
- **Workspace/quality/next:** V40 D0 is repeat-exact and zero-signal with immutable `5/2/2` disclosed roles. C0 external corpus is next, S0 clean-source growth remains open and `SourcePowerOOD`, and authored fallback is mandatory.
