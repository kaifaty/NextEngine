...
| Field | Value |
| --- | --- |
| Status | `ROADMAP_V37_ACTIVE / R0_N0_NR_RESEARCH_COMPLETE / QSO_V0_SELECTED / A0_F0_C0_REPEAT_EXACT_PASS / X0_NEXT / V36_CLOSED / H0_UNOPENED / SOURCE_FRONTIER_6_STEEL_27_NON_METAL / FALLBACK_REQUIRED` |
| Updated | `2026-09-03` |
...
- **Planning authority:** [Roadmap V37](../../plans/physical-sound-synthesis-roadmap-v37.md)
  selects QSO-v0; corrected [A0](../physical-sound-v37-a0-query-surface-contract-result-2026-09-03.md) and [F0](../physical-sound-v37-f0-fresh-query-surface-operator-result-2026-09-03.md) freeze its typed/scientific boundary. [C0](../physical-sound-v37-c0-query-surface-structural-cost-result-2026-09-03.md) repeat-exactly proves full-shape structure and all nine cost paths inside resources with zero scientific target/truth/official access. X0 is next; independent data/validator gates are unchanged.
...
- **Next action:** implement X0 only: bind materialization, fit, prediction, controls, metrics, hard/resources and atomic terminal publication into one owner; mutation-test all D0/H0 terminal/access/failpoint paths on surrogate values. S0 remains independent.
...
| V34–V37 R0–PR | `V34_H0_REJECT_CLOSED / V35_D0_OWNER_FAULT_CLOSED / V36_D0_METRIC_REJECT_CLOSED / V37_A0_F0_C0_REPEAT_EXACT_PASS` | V36 contact loses every control; V37 A0/F0/C0 freeze the new operator, prove 15,120 structural rows and nine cost paths with zero scientific values. X0 is next; H0 remains unopened. |

## Material transition: QSO-v0 replaces scalar hybrid interaction

- **Evidence:** [D0](../physical-sound-v36-d0-fresh-development-result-2026-09-03.md) rejects scalar contact transfer; [F0](../physical-sound-v37-f0-fresh-query-surface-operator-result-2026-09-03.md) freezes fresh science and [C0](../physical-sound-v37-c0-query-surface-structural-cost-result-2026-09-03.md) repeats 9,720 fields, 15,120 queries, remesh/topology/ablation reachability and nine cost paths below `300 s / 1 GiB` with zero scientific values.
- **Conclusion:** DeepONet/GNO evidence and the V36 ablations support an explicit field-branch/query-trunk interaction; full GINO/Geo-FNO is unnecessary before a small graph-kernel operator is tested.
- **Decision:** [QSO-v0](../physical-sound-v37-query-surface-operator-research-2026-09-03.md) consumes a P1-owned canonical probe graph plus a separate surface query, predicts only bounded contact correction and has no scalar expert blend.
- **Smallest next action:** implement X0 complete owner and atomic terminal publisher on discarded surrogate roles; E0 remains unauthorized.
...
| D-069 | V36 rejects scalar hybrid transfer; QSO-v0 is the smallest successor. A0/F0 freeze its typed/scientific question; C0 repeat-exactly proves full-shape structure and cost after replacing the rejected all-fields autograd layout (`1,278,464 KiB`) with exact 1,024-row gradient accumulation over 512-row microbatches. Do not retry the rejected layout. X0 is authorized. | Fresh QSO D0/H0 rejects, or X0–E0 proves the complete-owner/determinism contract infeasible before official access. |
...
| H3: explicit field/query operator can recover contact transfer | V36 hybrid fails contact; A0/F0 freeze QSO and C0 proves full-shape cost/remesh/topology/ablation reachability | Complete-owner terminal behavior and held transfer are unproved | X0–E0 value-free conformance, then one fresh QSO D0/H0 tournament |
...
## Handoff
- **Workspace/quality/next:** Roadmap V37 A0/F0/C0 are in Git; C0 repeats four files exactly, proves full-shape structure and nine cost paths at `33.99/34.07 s`, `981,252/968,592 KiB`, with zero scientific target/truth/official access. Implement X0 complete-owner terminal conformance next; source frontier remains `6/27` short.
