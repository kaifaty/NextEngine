...
| Field | Value |
| --- | --- |
| Status | `PS2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_CUP_SHARED_DECAY_MISMATCH_SUPPORTED / MULTIOUTPUT_SYNTHETIC_CONTROL_SUPPORTED / CERAMIC_COUNTERFACTUAL_FREEZE_NEXT / REAL_ANALYSIS_BLOCKED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / PASS_DISABLED / P1_BLOCKED` |
| Updated | `2026-08-28` |
...
- **Current conclusion:** Diagnostic `47b578ac…2603` finds shared Ceramic Cup
  failure (`23/27`) and rejects row-local and simple sub-`500 Hz` causes. The
  surviving issue is the single-output early-decay statistic. Mechanics did not run.
...
- **Next action:** Hash-close a read-only counterfactual over the existing
  Ceramic Cup block, fixed impact row and all 15 microphones. Do not execute it
  before the preflight checkpoint is committed and transferred.
...
| [Multi-output decay control result](../physical-sound-multioutput-decay-control-result-ps2-2026-08-28.md), report `a099f50d…e8bea` | `MULTIOUTPUT_SYNTHETIC_CONTROL_SUPPORTED / CERAMIC_COUNTERFACTUAL_FREEZE_NEXT / REAL_ANALYSIS_BLOCKED` | Two reports are byte-identical; all gates pass, including spatial/node decay fractions `1.0/0.0` and known-decay error `0.35979 dB/s`. Freeze a separate Ceramic read-only experiment before reuse. |
...
2. Freeze the existing Ceramic block/fixed-row multi-output counterfactual; do
   not execute before commit, tune, fetch, run physics or access Planter audio.
