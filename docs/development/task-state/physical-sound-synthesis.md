...
- **Next action:** Commit the serializer-only repair checkpoint, then run the
  calibration twice sequentially from immutable block `182f2010…1e0f` without
  network access. Stop before Planter unless both reports repeat and pass.
...
| H6: A selective specialist ensemble can safely automate admitted impact domains | Synthetic controls, exact-weld geometry, one frozen Pitcher block and serializer-repair preflight pass | No published Pitcher calibration decision, Planter transfer or calibrated shadow risk exists | Run the repaired Pitcher analysis twice offline and stop before Planter on any failure |
...
2. Preserve PS-2, transfer-V1/V2 and all spatial rejection hashes. Never retune
   ...
   Pitcher calibration preregistration, runner parity, execution preflight and
   opened immutable block; rerun only the serializer-repaired offline analysis
   twice and keep Planter sealed until a repeated pass.
