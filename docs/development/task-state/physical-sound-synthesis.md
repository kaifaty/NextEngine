...
| Field | Value |
| --- | --- |
| Status | `PS2_PITCHER_COMBINED_PROTOCOL_REJECTED / ADAPTIVE_DECAY_SYNTHETIC_CONTROL_SUPPORTED / CERAMIC_ADAPTIVE_COUNTERFACTUAL_FROZEN / TWO_READ_ONLY_ANALYSES_AUTHORIZED / INDEPENDENT_VALIDATION_REQUIRED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / PASS_DISABLED / P1_BLOCKED` |
| Updated | `2026-08-28` |
...
- **Next action:** Commit and transfer the Ceramic adaptive preflight, then
  execute it twice. Any failure rejects; a pass remains development-only.
...
| [Ceramic Cup adaptive counterfactual preflight](../physical-sound-ceramic-cup-adaptive-counterfactual-preflight-ps2-2026-08-28.md), report `4358d4da…220f` | `CERAMIC_ADAPTIVE_COUNTERFACTUAL_FROZEN / TWO_READ_ONLY_ANALYSES_AUTHORIZED / INDEPENDENT_VALIDATION_REQUIRED` | Manifest `48001fb7…ff9a` binds the same rows, synthetic pass, rejected fixed comparator and seven checks. Preflight reads zero payload bytes. |
...
2. Commit, transfer and execute the Ceramic adaptive counterfactual twice; do
   not tune, fetch, run physics or access Planter audio.
