...
| Field | Value |
| --- | --- |
| Status | `ROADMAP_V42_ACTIVE / R0_COMPLETE / C1_NEXT / V0_READY / CORPUS_SIGNAL_INSUFFICIENT / S0_SOURCE_POWER_OOD / FALLBACK_REQUIRED` |
| Updated | `2026-09-03` |
...
- **Current conclusion:** [V42 R0](../physical-sound-v42-r0-domain-information-audit-result-2026-09-03.md) repeats exactly and returns `CorpusSignalInsufficient`: project identity is strongly visible, but coarse material and oracle project-centering both lose under strict cross-project evaluation. No candidate was trained.
- **Planning authority:** [Roadmap V42](../../plans/physical-sound-synthesis-roadmap-v42.md) makes C1 descriptor/source growth next, keeps V0 independently ready and blocks neural training until B1 proves deployable descriptor signal.
...
- **D0/D1 result:** the [permanent disclosed roster](../physical-sound-v40-d0-disclosed-roster-result-2026-09-03.md) preserves all nine opened families; [D1](../physical-sound-v41-d1-alias-component-roster-result-2026-09-03.md) repairs its Blue Bowl cross-role alias, preserves `5/2/2` and becomes C0 authority.
- **C0 result:** the [repeat-exact disclosed corpus](../physical-sound-v41-c0-disclosed-corpus-result-2026-09-03.md) contains 139 hash-verified recordings, 278 PCM/feature objects, 67 isolated parents, complete axis masks and immutable `44/70/25` record projections with zero network/protected/model access.
...
- **Next action:** implement V42 C1: freeze the source-backed runtime descriptor schema and metadata-first independent-parent growth profile toward R0's planning floor of `125`; V0 and S0 remain independent.
...
| V34–V42 R0–PR | `V37_D0_METRIC_REJECT_CLOSED / V42_R0_COMPLETE / C1_NEXT / V0_READY / CORPUS_SIGNAL_INSUFFICIENT / S0_SOURCE_POWER_OOD` | QSO-v0 remains closed. R0 proves project/domain signal is strong but coarse material and simple project-centering do not transfer. C1 must add runtime descriptors and independent parents before B1; Product/admission remain behind independent one-shot gates. |

## Material transition: V42 R0 audit to C1 descriptor/source growth

- **Evidence:** R0 repeats byte-for-byte; source project is predictable, while leave-project-out material conditioning degrades median error and oracle project-centering does not recover it.
- **Conclusion/decision:** the corpus contains domain confounding but neither coarse material nor one additive source offset supplies deployable object signal. Both routes are closed; larger-network work remains blocked.
- **Smallest next action:** freeze C1's source-backed descriptor contract and internet-only parent-power target, then require B1 to beat the global floor; keep V0 and protected S0 isolated.
...
| H4: automatic validator reaches useful bounded risk | V0a synthetic mechanics pass `9/7`; D0 reserves two disclosed projects only for calibration | I0 clean power is `9/7` and `SourcePowerOOD`; no real validator/shadow result | V42 V0 plus S0/S1, then one-shot V1 |
| H5: baked atlas meets product cost | Offline clips preserve current deterministic fallback boundary | Whole-mixer/voice cost is unmeasured | V42 K0/P0 after Steel A0 Pass |
...
- **Workspace/quality/next:** V42 R0 repeat-exactly returns `CorpusSignalInsufficient`: domain signal is real, but coarse material and simple source-centering do not generalize. C1 source-backed runtime descriptors and independent-parent growth are next; V0 remains ready, S0 remains `SourcePowerOOD`, neural training stays blocked and authored fallback is mandatory.
