...
| Field | Value |
| --- | --- |
| Status | `ROADMAP_V39_ACTIVE / F0_F1_REPEAT_EXACT / D0_NEXT / S0_FRONTIER_6_STEEL_23_NON_METAL_ACTIVE / TWO_LANE_ML_FOUNDRY / PROTECTED_ADMISSION_BLOCKED / FALLBACK_REQUIRED` |
| Updated | `2026-09-03` |
...
- **Planning authority:** [Roadmap V39](../../plans/physical-sound-synthesis-roadmap-v39.md)
  now closes [F1](../physical-sound-v39-f1-source-frontier-result-2026-09-03.md) as repeat-exact `ImprovedFrontier`; D0 permanent disclosed roster is next while metadata-only S0 continues and the strict admission lane remains blocked at `6/23`.
...
- **Next action:** implement V39 D0 as one permanent-disclosed roster/projection for generator train, generator development and validator calibration. Freeze parent-disjoint identities and axes before payload access; prove protected counters zero and irreversible protected-pool exclusion. Bounded S0 discovery continues in parallel.
...
| V34–V39 R0–PR | `V37_D0_METRIC_REJECT_CLOSED / V39_F0_MECHANICS_READY / F1_IMPROVED_FRONTIER_6_23 / D0_NEXT` | QSO-v0 remains closed and protected admission still needs `6/23`. F1 proves only source-frontier replay; V39 may train only on permanently disclosed projects and a development winner never earns admission credit. |

## Material transition: V39 F1 source-frontier closure

- **Evidence:** [V39 F1](../physical-sound-v39-f1-source-frontier-result-2026-09-03.md) repeats V30 `6/27` and IETeasy `0/4` into exact `6/23` twice, preserves five reserved projects, rejects inflation/drift and opens no signal.
- **Conclusion/decision:** portable source-frontier automation is complete, but the protected partition remains infeasible. D0 disclosed work may proceed independently; S1 remains blocked by S0.
- **Smallest next action:** freeze the permanent disclosed D0 roster/projection with parent-disjoint axes and irreversible protected-pool exclusion before any payload decode.
...
| D-075 | F1 mechanically reproduces IETeasy as one project with 15 physical samples, 150 non-independent recordings, four non-Metal groups and zero strict-Steel groups; frontier is `6/23`. RSAudio/DiffImpact remain zero-power. | A source-identity defect invalidates IETeasy or fresh versioned metadata makes either deferred lead auditable. |
## Open hypotheses
...
| H4: automatic validator reaches useful bounded risk | V0a synthetic mechanics pass `9/7` twice; V38 S0 improves source power to `6/23` | Partition remains infeasible; no real validator/shadow result | V39 V0 disclosed calibration, then one-shot V1 after S1 |
| H5: baked atlas meets product cost | Offline clips preserve current deterministic fallback boundary | Whole-mixer/voice cost is unmeasured | V39 K0/P0 after Steel A0 Pass |
...
- **Workspace/quality/next:** V39 F0/F1 are repeat-exact; source frontier remains infeasible at `6 exact-Steel / 23 non-Metal` with zero signal. D0 permanent disclosed roster is next, S0 remains active, and authored fallback remains mandatory.
