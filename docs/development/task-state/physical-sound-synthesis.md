...
| Field | Value |
| --- | --- |
| Status | `PS2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_METHOD_DEVELOPMENT_CLOSED / SALIENCE_SELECTOR_SYNTHETIC_CONTROL_SUPPORTED / SCALE_INVARIANCE_SUPPORTED / IRON_SKILLET_DISCOVERY_VERIFIED / ONE_IMPACT_OBSERVATION_PROTOCOL_FROZEN / EXECUTION_RUNNER_REQUIRED / OBSERVATION_NOT_OPENED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / PASS_DISABLED / P1_BLOCKED` |
| Updated | `2026-08-28` |
...
- **Current conclusion:** Iron Skillet discovery `b9f659b4…6f6c` and repeated
  audit `a3bd84f1…8dab` verify the archive with zero member payload. Protocol
  `573ff0d6…8c93` freezes four future ranges and 600 impact-zero rows.
...
- **Next action:** Implement and hash-close the Iron Skillet execution runner,
  then repeat zero-access preflight before any of the four frozen requests.
...
| [Salience-selector result](../physical-sound-salience-selector-control-result-ps2-2026-08-28.md) and [Iron Skillet discovery/protocol](../physical-sound-realimpact-iron-skillet-discovery-and-observation-protocol-ps2-2026-08-28.md), reports `fa940710…0b3e` / `a3bd84f1…8dab` / `19c1f57a…2680` | `SALIENCE_SELECTOR_SYNTHETIC_CONTROL_SUPPORTED / IRON_SKILLET_DISCOVERY_VERIFIED / ONE_IMPACT_OBSERVATION_PROTOCOL_FROZEN / OBSERVATION_NOT_OPENED` | Candidate recovers `16/16` versus top-16 `4/16`. Two exact discovery requests and offline audits verify object 17 with zero member payload; four future ranges, `(3000,230549)` shape, 600 rows and unchanged gates are frozen. Next execution runner, not payload or physics. |
...
2. Implement and hash-close the Iron Skillet execution runner, then rerun the
   zero-access protocol preflight; do not fetch payload, tune, run physics or access Planter.
