...
- **Next action:** Hash-close a read-only Ceramic adaptive counterfactual over
  the same rows and lineage. Do not execute before its checkpoint is committed.
...
| [Adaptive modal-decay control result](../physical-sound-adaptive-decay-control-result-ps2-2026-08-28.md), report `9fabc2bd…0297f` | `ADAPTIVE_DECAY_SYNTHETIC_CONTROL_SUPPORTED / CERAMIC_ADAPTIVE_COUNTERFACTUAL_FREEZE_NEXT / REAL_ANALYSIS_BLOCKED` | All 16 fits pass; median error `0.68081 dB/s`, `R² 0.999812`, improvement `58.27233 dB/s`. Freeze a separate real counterfactual before reuse. |
...
2. Freeze the Ceramic adaptive counterfactual; do not execute before commit,
