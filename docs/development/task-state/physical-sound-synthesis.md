...
| Field | Value |
| --- | --- |
| Status | `ROADMAP_V32 / P0_P1_T0_V0A_COMPLETE / V0_V1_CLEAN_FALSE_POSITIVE_REJECT / M0_PROTOCOL_NEXT / VALIDATOR_FIRST / SOURCE_FRONTIER_6_STEEL_27_NON_METAL / REAL_ADMISSION_BLOCKED / FALLBACK_REQUIRED` |
| Updated | `2026-09-02` |
...
- **Current conclusion:** [V28 R2](../physical-sound-v28-r2-official-feasibility-result-2026-09-02.md)
  is a repeat-exact `REPRESENTATION_REJECT`: six A/B artifacts match. Frequency,
  gain and spectrum beat ridge, but decay, remesh and all four physical
  counterfactuals fail; M0c is closed before holdout/real access.
...
- **Planning authority:** [Roadmap V32](../../plans/physical-sound-synthesis-roadmap-v32.md)
  turns the completed causal owner into an artifact-gated automatic chain:
  T0 truth, V0 validator mechanics, M0 correction freeze and one M1 tournament;
  source scouting stays bounded and real gates remain unchanged.
...
- **Next action:** freeze V32 M0's bounded correction family, controls, losses, hard causal gates and resource oracle before any model value opens.
...
| V32 F0–PR | `P0_P1_T0_V0A_COMPLETE / V0_V1_REJECT_PRESERVED / M0_PROTOCOL_NEXT / VALIDATOR_FIRST / SOURCE_FRONTIER_PRESERVED / REAL_RELEASE_BLOCKED` | V0a repeats `9 Pass / 7 Reject`, admits both exact modal alias classes and retains plate→beam rejection; T0 and all real `6/27` gates remain unchanged. |

## Material transition: V32 automatic release chain

- **Observation:** exact target modal signature separates lawful causal aliases
  from the frozen plate-to-beam retrieval mutation.
- **Evidence:** two 35-file V0a publications match; `9/9` clean pass, all seven
  mutations reject once, aliases pass and the negative copy control rejects.
- **Conclusion:** automatic synthetic mechanics no longer depend on case labels
  or per-sound human review, while real quality remains deliberately unclaimed.
- **Decision:** close V0a and sequence M0 protocol freeze before model values.
- **Smallest next action:** preregister the bounded M0 correction tournament.
...
| H4: automatic validator reaches useful bounded risk | V0a synthetic mechanics pass `9/7` twice without label or human-review authority | E3 rejects whole-project partition; no real validator/shadow result | S1/V1 real qualification after M0/M1 synthetic tournament |
| H5: baked atlas meets product cost | Offline clips preserve current deterministic fallback boundary | Whole-mixer/voice cost is unmeasured | V31 K0/D0 after Steel A0 Pass |
...
- **Workspace:** Q1a/E2/V32 and the P0/P1/T0 owners/results are in Git; generated artifacts remain external.
...
- **Next commit boundary:** freeze V32 M0 protocol before model values; fallback remains authority.
