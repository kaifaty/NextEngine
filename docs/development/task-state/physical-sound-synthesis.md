...
| Field | Value |
| --- | --- |
| Status | `ROADMAP_V42_ACTIVE / B0_COMPLETE / R0_NEXT / V0_READY / DESCRIPTOR_SIGNAL_REQUIRED / S0_SOURCE_POWER_OOD / FALLBACK_REQUIRED` |
| Updated | `2026-09-03` |
...
- **Planning authority:** [Roadmap V42](../../plans/physical-sound-synthesis-roadmap-v42.md) keeps the disclosed/one-shot split but inserts a domain/information audit and descriptor-signal gate before neural training; R0 is next and V0 is independently ready.
...
- **B0 result:** the [repeat-exact grouped baseline](../physical-sound-v41-b0-grouped-baseline-result-2026-09-03.md) freezes a global median floor `1.337593650`; ridge `1.443077073`, pointwise MLP `1.484006315` and every material/retrieval/local control lose. Exact Steel is absent from train, so only coarse metallic transfer is observed.
...
- **Next action:** implement V42 R0 on frozen B0/C0: quantify project-domain predictability, leave-project-out material signal and source-normalized target behavior without training a candidate; V0 and S0 remain independent.
...
| V34–V42 R0–PR | `V37_D0_METRIC_REJECT_CLOSED / V41_B0_COMPLETE / V42_R0_NEXT / V0_READY / DESCRIPTOR_SIGNAL_REQUIRED / S0_SOURCE_POWER_OOD` | QSO-v0 remains closed. B0 proves coarse material one-hot is weaker than the global prototype; V42 requires domain attribution, runtime descriptor enrichment and a B1 signal gate before neural training. Product/admission remain behind independent one-shot gates. |

## Material transition: V41 B0 baseline to V42 descriptor-first work

- **Evidence:** B0's six deterministic methods reproduce byte-for-byte on 21 supported development parents; global prototype beats material prototype, retrieval, nearest, ridge and small MLP.
- **Conclusion/decision:** current material-only conditioning has insufficient deployable signal. This is compatible with both missing object descriptors and capture/project domain shift; B0 alone does not distinguish them and cannot justify a larger network.
- **Smallest next action:** freeze R0's domain/information audit, then require a runtime-available descriptor baseline to beat the global floor before M0; keep validator-calibration and protected S0 isolated.
...
- **Workspace/quality/next:** V41 B0 repeat-exactly freezes the global prototype as the best current floor and proves material-only conditioning insufficient. Roadmap V42 makes R0 domain/information attribution next, keeps V0 ready and S0 `SourcePowerOOD`, and blocks neural training until runtime descriptors beat that floor; authored fallback is mandatory.
