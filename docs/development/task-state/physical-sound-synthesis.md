...
- **Current conclusion:** Execution preflight `1af90ed4…e4f68` freezes staged
  fit→selection→holdout→shadow execution and deterministic DSP fixtures.
...
- **Next action:** Acquire/decode only fit-role `86_MetalHoledSpoon`, then
  publish development/calibration selection before any holdout request.
...
| [Registry result](../physical-sound-modal-observation-registry-result-ps2-2026-08-28.md), [metal-batch protocol](../physical-sound-realimpact-metal-batch-protocol-ps2-2026-08-28.md) and [execution preflight](../physical-sound-realimpact-metal-batch-execution-preflight-ps2-2026-08-28.md), reports `34a7f209…daed0` / `bd755741…709e3` / `1af90ed4…e4f68` | `MODAL_OBSERVATION_REGISTRY_V0_BUILT / METAL_BATCH_EXECUTION_PREFLIGHT_FROZEN / FIT_ACQUISITION_NEXT` | Nine staged reports and deterministic excitation/fit/metric fixtures repeat before new payload. Open only Holed Spoon fit data next; holdout/shadow remain sealed. |
...
2. Acquire/decode only `86_MetalHoledSpoon`, then freeze development/calibration
   selection before any Spoon holdout or Spatula shadow access.
