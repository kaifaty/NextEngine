...
| Field | Value |
| --- | --- |
| Status | `PS2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_METHOD_DEVELOPMENT_CLOSED / IRON_SKILLET_METHOD_TRANSFER_REJECTED / SALIENCE_SELECTOR_REAL_TRANSFER_REJECTED / SYNTHETIC_CONTROL_RETAINED / CURRENT_TOP16_DIAGNOSTIC_STRONGER / BOUNDED_METHOD_RESEARCH_REQUIRED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / PASS_DISABLED / P1_BLOCKED` |
| Updated | `2026-08-28` |
...
- **Current conclusion:** Repeated Iron Skillet report `a9c4ae36…206d` rejects
  current real transfer: persistence `0.35294`, adaptive-valid `0.50`.
  Diagnostic top-16 is stronger at `0.9375/0.9375` valid/decaying.
...
- **Next action:** Freeze bounded existing-block diagnostics separating ±`10%`
  dominance from fixed `900 ms` persistence; research a multichannel method.
...
| [Salience-selector result](../physical-sound-salience-selector-control-result-ps2-2026-08-28.md) and [Iron Skillet result](../physical-sound-realimpact-iron-skillet-observation-result-ps2-2026-08-28.md), reports `fa940710…0b3e` / `a9c4ae36…206d` | `SYNTHETIC_CONTROL_RETAINED / IRON_SKILLET_METHOD_TRANSFER_REJECTED / CURRENT_TOP16_DIAGNOSTIC_STRONGER / BOUNDED_METHOD_RESEARCH_REQUIRED` | Candidate recovers `16/16` synthetic truth but real persistence/adaptive-valid are `0.35294/0.50`; top-16 diagnostic is `0.9375/0.9375`. Freeze diagnostics separating dominance/tail timing and research a multichannel estimator; no tuning, new object or physics. |
...
2. Freeze bounded Iron diagnostics separating dominance and tail timing, then
   research a multichannel transient modal estimator; no tuning, new object or Planter.
