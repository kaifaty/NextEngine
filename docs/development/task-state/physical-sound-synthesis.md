...
| Field | Value |
| --- | --- |
| Status | `PS2_PITCHER_COMBINED_PROTOCOL_REJECTED / CERAMIC_METHOD_DEVELOPMENT_CLOSED / SALIENCE_SELECTOR_SYNTHETIC_CONTROL_SUPPORTED / SCALE_INVARIANCE_SUPPORTED / IRON_SKILLET_DISCOVERY_VERIFIED / ONE_IMPACT_OBSERVATION_PROTOCOL_FROZEN / EXECUTION_PREFLIGHT_SUPPORTED / FOUR_REQUEST_ACQUISITION_AUTHORIZED / OBSERVATION_NOT_OPENED / MECHANICS_BLOCKED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / EIGHT_EXACT_CLAIMS_OPEN / PASS_DISABLED / P1_BLOCKED` |
| Updated | `2026-08-28` |
...
- **Current conclusion:** Execution manifest `2c98971a…d8ee`, runner
  `0c53553a…0988` and repeated preflight `b9927f17…672b` bind the full Iron
  Skillet path with zero observation payload bytes so far.
...
- **Next action:** After commit execute exactly four acquisition requests once,
  decode once and analyze twice offline; stop on any failure.
...
| [Salience-selector result](../physical-sound-salience-selector-control-result-ps2-2026-08-28.md), [Iron Skillet protocol](../physical-sound-realimpact-iron-skillet-discovery-and-observation-protocol-ps2-2026-08-28.md) and [execution preflight](../physical-sound-realimpact-iron-skillet-execution-preflight-ps2-2026-08-28.md), reports `fa940710…0b3e` / `19c1f57a…2680` / `b9927f17…672b` | `SALIENCE_SELECTOR_SYNTHETIC_CONTROL_SUPPORTED / IRON_SKILLET_DISCOVERY_VERIFIED / EXECUTION_PREFLIGHT_SUPPORTED / FOUR_REQUEST_ACQUISITION_AUTHORIZED` | Candidate recovers `16/16` synthetic truth. Complete real-object runner binds exact ranges, 600-row decode, variable-length adaptive analysis and immutable gates with zero payload so far. Next acquire once, decode once and analyze twice; no retry or physics. |
...
2. Execute the exact Iron Skillet four-request acquisition once, decode once and
   analyze twice offline; do not retry, tune, run physics or access Planter.
