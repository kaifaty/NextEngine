...
| Date | 2026-09-04; reordered the same day by D-013 (the player's interaction with water goes last: swimming will be a separately trained model on PD controllers) |
...
D-013 (user, 2026-09-04): the player's own interaction with the water
(carrying, slowing, swimming, the player's wake) is placed last. Swimming
will be a separately trained model driving the SPEC-37 motors through PD
controllers; the water's forces on the body are that model's environment
and are built when that project needs them, not before.
...
| --- | --- | --- | --- | --- |
| 1 | The camera under the surface | the player already walks on the basin floor and sees the walls through clear air; the underwater view is the first thing a player who steps in expects | M | — |
| 2 | Water heard | jets, falls and splashes are silent; sound sells contact more than pixels do | M | stage records (done), SPEC-08 / SPEC-45 |
| 3 | Wetness | bodies leave the water dry; the shoreline and the crate do not darken | M | — |
| 4 | Gates, pumps, sources and sinks as play | the flow network can only be watched; levels cannot ask the player to raise, drain or redirect water | M | flow network (done) |
| 5 | Floating bodies that tilt, drift and turn | boxes rise and fall on a vertical line; a crate that never lists or drifts with a current is a lift, not a float | M | buoyancy batch (done) |
| 6 | Waves that answer events | the wave spectrum is the same whether the water is still or a crate just fell in; the shallow-water presentation grid gives ripples from bodies and shores | M | 5 |
| 7 | Big water | the lattice bounds fit a basin and vessels; an open plane (a lake, the sea) with the exact level and no flow network, and larger bounds when a region needs them, with the quiescence clause of D-010 | L | D-010 revisit |
| 8 | The PhysX lane's finish | kernels smoothed over time, the neighbourhood analysis on the GPU, the fluid recreated after its failure, and the retention decision that D-012 waits for | M | ADR-106 evidence |
| 9 | Water and the ground | soaking, seepage and the ground-water level of SPEC-38's housekeeping; felt only in levels that use them | L | 7 |
| 10 | Far water and rendering order | LOD for distant water, shadows on and under the surface, ordering against other transparent objects | S-M | 7 |
| 11 | The player in the water | last by D-013: the water's forces on the body are the environment of the swimming model; built when that project needs them | L | the swimming-model project |
| 12 | The player's own wake and splashes | last by D-013: needs the player's immersion record from 11 | M | 11 |

## The items

### 1. The camera under the surface (M)

- **Now.** The water pass renders the surface from above only; the
  player already walks on the basin floor and sees the walls through
  clear air.
...
- **What.** A sound event set from the stage records (flow rate to
  ...
  immersion change; the player's entry only after item 11) into SPEC-08's
...
- **What.** A wetness term in the material suite (WL1) driven by the
  ...
  bounded decay for bodies (presentation-only state on the render side),
  plus drips from the boxes' immersion records for bodies leaving the
  water; the player joins after item 11.
- **Evidence.** Captures of the rim and the crate before, in and after
  the water; the material pass cost gate.

### 4. Gates, pumps, sources and sinks as play (M)
...
### 5. Floating bodies that tilt, drift and turn (M)
...
- **What.** The shallow-water presentation grid on the ring's plan
  ...
  (the player's record joins after item 12);
...
### 10. Far water and rendering order (S-M)
...
### 11. The player in the water (L)

- **Placed last by D-013.** Swimming will be a separately trained model
  on PD controllers over the SPEC-37 motors; this item then supplies
  that model's environment (the exact forces of the water on the body)
  and the wading and climbing rules, when that project asks for them.
...
### 12. The player's own wake and splashes (M)
