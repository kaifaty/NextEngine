...
| Status | `ADOPTED / P0_P1_T0_V0A_COMPLETE / V0_V1_REJECT_PRESERVED / M0_PROTOCOL_FROZEN / M0_IMPLEMENTATION_NEXT / VALIDATOR_FIRST / BOUNDED_SOURCE_SCOUTING / REAL_RELEASE_BLOCKED_BY_SOURCE_POWER / OFFLINE_ML_ONLY / NO_PER_SOUND_HUMAN_REVIEW / AUTHORED_FALLBACK` |
...
| M0 | Bounded correction protocol | [`PROTOCOL_FROZEN / IMPLEMENTATION_NEXT`](../development/physical-sound-v32-m0-physics-locked-residual-protocol-2026-09-02.md) | The `1,491`-parameter branch-isolated model cannot alter P1 frequency/nodes/impulse/remesh and may learn only bounded decay/global-gain/contact multipliers. Controls, splits, gates and resources are frozen before values. |
| M1 | Known-truth tournament | `BLOCKED_BY_M0_IMPLEMENTATION` | One candidate run improves the preregistered aggregate over every control while all causal, remesh, decay, energy, resource, determinism and mutation gates pass. A reject closes the family. |
...
5. **V32.5 — freeze M0 — complete / implementation next:** one physics-locked
   correction family, baselines, losses, hard gates and resources before values.
...
V32.1–V32.4 and the V32.5 protocol are complete; the immediate next commit builds M0 conformance without official values. A source lead may interrupt only between
