...
seeded two-exponential residual. New network/member payload remains zero. This
authorized only `86_MetalHoledSpoon`; Spoon holdout and both Spatulas remained
sealed until their immutable predecessor reports passed. See the [execution preflight](../development/physical-sound-realimpact-metal-batch-execution-preflight-ps2-2026-08-28.md).

PS-2 metal batch fit/calibration result: acquisition `454933ae…163d8` uses
exactly two responses and decode `af7af9fb…e068a` verifies Holed Spoon rows
`0..14` at shared `0°/0 mm`, vertex `38440`. Selection A/B repeats at
`83f858bf…43f2`. The truncated transient fails; the seeded two-exponential
residual passes both calibration objects with aggregate envelope/energy/
flatness ratios `0.387055/0.709616/0.552735`, maximum object/metric ratio
`0.943533` and improved-object fraction `1.0`. It is representation evidence,
