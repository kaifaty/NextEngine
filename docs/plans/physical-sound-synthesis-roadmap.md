...
| Поле | Значение |
| --- | --- |
| Статус | `ACTIVE_R&D / PS-2_ACQUISITION_CONTRACT_COMPLETE / PS-2_PHYSICAL_CAPTURE_NEXT / AUTOMATIC_PASS_DISABLED / PRODUCTION_P1_BLOCKED` |
...
| PS-2. Corpus and risk closure | `IN_PROGRESS / ACQUISITION_CONTRACT_COMPLETE` | L | Policy and complete synchronized microphone/force/metrology bundle are executable; one public pilot correctly falls back, and new physical capture remains open. |
...
| 4 | Controlled pilot и complete acquisition bundle contract — `COMPLETE`; физически записать первую полную real object family — `NEXT` | PS-2 evidence expansion |
