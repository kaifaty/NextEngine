...
| PS-2. Corpus and risk closure | `IN_PROGRESS / SEVEN_POINT_QUADRATURE_REJECTED / ABSOLUTE_GATES_PASS / REFINEMENT_AND_COMPARISON_FAIL / NO_FRESH_DATA_OPENED / TWO_HOLDOUT_OBJECTS_PRESERVED / INDEPENDENT_BEMPP_CONTROL_NEXT / THREE_DIMENSIONAL_SPATIAL_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN` | L | Seven-point regular quadrature leaves fine/coarse error at `2.6109` and makes fine median error `1.0447x` control, so the local quadrature hypothesis is rejected. Reports repeat at `7460750e…b48d` and V1 stays exact. Next freeze an independent Bempp-cl Galerkin/singular-quadrature sphere control before neural acceleration, real-data validation or PS-3. |
...
| 4 | Controlled pilot, `E1` bundle import, internet registry/cache, eight-project E3 normalization, explicit roles, five typed E2 rows, exact-domain/source gates, transfer calibration and spatial discriminators — `SEVEN_POINT_QUADRATURE_REJECTED / ABSOLUTE_GATES_PASS / REFINEMENT_AND_COMPARISON_FAIL / NO_FRESH_DATA_OPENED / INDEPENDENT_BEMPP_CONTROL_NEXT / THREE_DIMENSIONAL_SPATIAL_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN`; next run independent Bempp-cl Galerkin/singular control while preserving ceramic holdout | PS-2 domain-axis readiness |
