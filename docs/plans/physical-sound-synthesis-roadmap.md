...
| Поле | Значение |
| --- | --- |
| Статус | `ACTIVE_R&D / PS-2_PITCHER_COMBINED_PROTOCOL_REJECTED / OBSERVATION_ADMISSION_FAILED / CERAMIC_CUP_DISCOVERY_PREFLIGHT_SUPPORTED / TWO_NONPAYLOAD_REQUESTS_AUTHORIZED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN / AUTOMATIC_PASS_DISABLED / PRODUCTION_P1_BLOCKED` |
| Архитектурная граница | [SPEC-45](../architecture/45-physical-sound-synthesis-and-acoustic-presentation.md), `Proposed` |
| Актуальное evidence | [PS-2 REALIMPACT Ceramic Cup discovery preflight](../development/physical-sound-realimpact-ceramic-cup-discovery-preflight-ps2-2026-08-28.md) |
...
| Последний пакет | [Ceramic Cup observation discovery preflight](../development/physical-sound-realimpact-ceramic-cup-discovery-preflight-ps2-2026-08-28.md) |
...
| PS-2. Corpus and risk closure | `IN_PROGRESS / PITCHER_COMBINED_PROTOCOL_REJECTED / OBSERVATION_ADMISSION_FAILED / CERAMIC_CUP_DISCOVERY_PREFLIGHT_SUPPORTED / TWO_NONPAYLOAD_REQUESTS_AUTHORIZED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN` | L | Parent `8bd5323c…1aea` rejects the combined protocol; audit `68c79a37…5a57` exposes observation failure. Manifest `cf2b72ee…a6a9` and preflight `5fa54efe…17ee` now authorize only one ZIP-tail and one fixed local-header request for unopened `78_CeramicCup`. Audit that cache, then freeze observation access before physics; do not open Planter. |
...
Only this discovery is authorized next; metadata, geometry, audio and Planter
remain prohibited until cached audit and a separate manifest. See the
[discovery-preflight evidence](../development/physical-sound-realimpact-ceramic-cup-discovery-preflight-ps2-2026-08-28.md).
...
| 4 | Controlled pilot, `E1` bundle import, internet registry/cache, eight-project E3 normalization, explicit roles, five typed E2 rows, exact-domain/source gates, transfer calibration and spatial discriminators — `PITCHER_COMBINED_PROTOCOL_REJECTED / OBSERVATION_ADMISSION_FAILED / CERAMIC_CUP_DISCOVERY_PREFLIGHT_SUPPORTED / TWO_NONPAYLOAD_REQUESTS_AUTHORIZED / PLANTER_AUDIO_SEALED / AUTHORED_CLIP_FALLBACK / REAL_3D_FIELD_OPEN / EIGHT_EXACT_CLAIMS_OPEN`; next execute/cache-audit discovery, then freeze observation before physics | PS-2 domain-axis readiness |
