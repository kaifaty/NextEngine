...
| Последний пакет | [REALIMPACT metal batch execution preflight](../development/physical-sound-realimpact-metal-batch-execution-preflight-ps2-2026-08-28.md) |
...
PS-2 subband common-pole control result: runner `4acf8016…bf141`, manifest
`38025d25…f8703`, preflight `9fb1c488…64e4a` and byte-identical run
`3fcb5fc8…2284f` pass all eleven synthetic gates. Orders `[2,2,1,1]` recover
six common poles, including 34/31 Hz pairs below the `46.875 Hz` FFT bin;
maximum frequency/decay errors are `0.043014 Hz/0.259092/s`. A declared node
output misses one mode while 15 outputs recover both, and scales `0.125/1/8`
are exact. This is a preselected-subband core only. Next prove broad-band band
discovery, duplicate clustering, amplitudes, pruning and reconstruction on a
new synthetic seed; no real reuse, mechanics or Planter. See the [control
result](../development/physical-sound-subband-common-pole-control-result-ps2-2026-08-28.md).

PS-2 broad-band common-pole control result: runner `317fa3a2…ec8d`, manifest
`a8f59bde…040b`, preflight `eab32084…e8bc` and byte-identical holdout report
`dcd83252…0094` pass all fifteen frozen gates without truth band centres. The
candidate discovers 6 regions/18 bins, removes 12 duplicate estimates, retains
all 7 strong modes, prunes the weak nuisance and reaches maximum
`0.095876 Hz/0.190671/s` errors. Strong/full/post-transient NRMSE is
`0.033913/0.130840/0.049273`, and discovery is exact at scales `0.125/1/8`.
This closes only the synthetic discovery-to-resynthesis method. Next freeze a
read-only existing-Iron counterfactual; no new payload, object, mechanics,
Planter, quality admission or runtime credit. See the [control result](../development/physical-sound-broadband-common-pole-control-result-ps2-2026-08-28.md).

PS-2 existing-Iron broad-band counterfactual result: runner
`63d46bf4…2e76`, manifest `8a445a4f…ff18`, preflight `04e54415…d357` and
byte-identical analysis `7635f8ac…075d` reject the frozen real envelope before
pole estimation. Input discovery finds 31 regions/91 neighboring bins against
the preregistered 16/48 maxima; scale signatures remain exact at
`0.125/1/8`. Do not rank the opened Iron regions or raise the cap. Next freeze
a fresh-seed dense synthetic scaling/leakage discriminator that tests the full
post-estimation prune cost before any Iron V2. See the [counterfactual result](../development/physical-sound-realimpact-broadband-counterfactual-result-ps2-2026-08-28.md).

PS-2 dense broad-band scaling control result: runner `8e4cc18b…fbbe`, manifest
`5ca8b1da…e1ab0`, preflight `81b44dec…ac95` and byte-identical holdout report
`b9a5b226…978b` pass all 18 gates. Rank-7 partial SVD reproduces the complete
parent to about `1e-12`, then processes 32 regions/96 bins, removes 122
duplicates and recovers 64/64 modes with zero false or selected weak nuisance
regions. Maximum errors are `0.080465 Hz/0.610466/s`; post-transient NRMSE is
`0.049039`. Next freeze Iron V2 over all 31 opened regions with the supported
partial core and no ranking. See the [scaling result](../development/physical-sound-dense-broadband-scaling-control-result-ps2-2026-08-28.md).

PS-2 existing-Iron broad-band V2 result: runner `3f1c07b5…6aa5`, manifest
`876e125d…a4bf`, preflight `81f1b089…3fcf` and byte-identical analysis
`f2fb359f…6a73` pass all 17 gates. Every one of 31 regions/91 bins is analyzed;
59 estimates become 31 clusters and 30 retained modes. Even/odd partition
matches are `0.833333/1.0`, full damped/undamped NRMSE ratio is `0.803542` and
predictive ratios are `0.005848/0.002251`. Absolute NRMSE remains modest, so
this is one-object method transfer only. Next freeze an object-disjoint internet
real holdout with unchanged V2. See the [V2 result](../development/physical-sound-realimpact-broadband-v2-result-ps2-2026-08-28.md).

PS-2 broad-band independent holdout discovery result: runner
`c59aea1d…79cd`, manifest `931f6a72…f5e7`, preflight `3b4c5b6a…c8bd`,
two-range acquisition `84308e8e…d080` and byte-identical offline audit
`67c92621…8a42` verify official `43_IronMortar` without member-payload access.
The observation entry has exact inferred shape `3000×208375`, sufficient for
V2 windows. Next freeze one-impact metadata/audio ranges and unchanged V2 gates.
See the [discovery result](../development/physical-sound-realimpact-broadband-holdout-discovery-result-ps2-2026-08-28.md).

PS-2 Iron Mortar independent broad-band holdout result: protocol
`c6943d12…34fd`, execution preflight `67e83d24…a67e`, acquisition
`1ac0fd39…fb41`, decode `5029930e…f127` and byte-identical analysis
`64bc63aa…d0cc` pass the frozen method protocol. Discovery is 26 regions/77
bins; 13 modes survive, both microphone halves match `0.846154`, and full
damped/undamped NRMSE ratio is `0.766820`. Absolute future-window NRMSE remains
about `1.0`, so quality/admission stay disabled. Next build a report-only modal
observation registry with residual facts, then batch object-grouped validation.
See the [holdout result](../development/physical-sound-realimpact-broadband-holdout-result-ps2-2026-08-28.md).

PS-2 modal-observation registry V0 preregistration: builder
`a958c795…c064b`, manifest `0f473f4d…05c20` and byte-identical preflight
`fbffbe1e…441c0` freeze the exact 924,196-byte Skillet and 696,263-byte Mortar
reports. The V0 record carries full modal/spatial/fit/prune/residual facts and
explicitly disables quality/domain/runtime admission; authored clips remain
required. Next run two byte-identical builds. See the
[preregistration](../development/physical-sound-modal-observation-registry-preregistration-ps2-2026-08-28.md).

PS-2 modal-observation registry V0 result: registry `e0bec857…e09e98` and
report `34a7f209…daed0` repeat byte-identically. Two entries retain 43 modes,
three pruned modes, spatial replication and absolute residual metrics. Both
keep quality/domain/runtime admission disabled and authored fallback required.
Next freeze an object/family-grouped batch protocol before evaluating
transient/residual candidates. See the
[result](../development/physical-sound-modal-observation-registry-result-ps2-2026-08-28.md).

PS-2 object-grouped metal batch preregistration: runner
`ecbaa724…acbfc`, manifest `873d82e3…23ae4` and repeated preflight
`578e143c…aec31` freeze all eight explicit Iron/Metal roster objects into
`3/2/1/2` development/calibration/holdout/shadow roles. Both MetalSpatula
objects stay in shadow. Four new archives remain at zero member payload. Next
read only their ZIP tails/local headers and audit twice. See the
[preregistration](../development/physical-sound-realimpact-metal-batch-preregistration-ps2-2026-08-28.md).

PS-2 object-grouped metal batch discovery result: acquisition
`d0a051eb…b0468` uses exactly eight structural range requests; repeated audit
`5790f8cc…a1219` verifies four observation entries of at least
`3000×208584`, with zero new member payload. Next freeze exact member ranges,
decoder shapes, modal/transient/residual parameters and calibration-only gates.
See the
[result](../development/physical-sound-realimpact-metal-batch-discovery-result-ps2-2026-08-28.md).

PS-2 metal batch payload/candidate protocol: runner `c3d0b406…7e2ea`,
manifest `24231974…b724b` and repeated preflight `bd755741…709e3` freeze nine
metadata ranges, four 32 MiB prefixes, decoder conditions, modal/transient/
seeded-residual candidates, stochastic seeds, metrics and calibration→holdout→
shadow order. This authorized the separately frozen execution preflight below. See
the [protocol](../development/physical-sound-realimpact-metal-batch-protocol-ps2-2026-08-28.md).

PS-2 metal batch execution preflight: runner `8484f947…13cc`, manifest
`bffaa21c…a8664` and byte-identical preflights `1af90ed4…e4f68` freeze nine
role-ordered stages from fit acquisition through shadow. The offline fixture
exercises SHA-256-counter Box-Muller excitation, rectangular subbands,
non-negative one/two-exponential fits, all three gating metrics and the
calibration tie-break; it rejects the truncated transient and selects the
seeded two-exponential residual. New network/member payload remains zero. Next
acquire only `86_MetalHoledSpoon`; Spoon holdout and both Spatulas stay sealed
until their immutable predecessor reports pass. See the [execution preflight](../development/physical-sound-realimpact-metal-batch-execution-preflight-ps2-2026-08-28.md).
