...
| --- | --- |
| Дата | `2026-09-03` |
| Статус | `ACTIVE / EXECUTION_ROADMAP / FOUNDATION_COMPLETE / D0_SOURCE_PREFLIGHT_NEXT / RESEARCH_ONLY / OFFLINE_ML / AUTHORED_FALLBACK` |
...
| D0 | Source preflights | NISR и VibraVerse отдельно проходят revision, lineage, generation, integrity и bounded-sample gates; Delft/CMU/three-object candidates получают metadata-first disposition. Никакого bulk download. | `SourceUntrusted` по каждому источнику отдельно; отказ одного не даёт credit другому. |
...
1. **V46.1 — D0 — NEXT:** завершить отдельные metadata/generation preflights
   NISR и VibraVerse; при provenance failure остановить источник до payload.
2. **V46.2 — D1:** собрать единый external corpus compiler поверх уже
...
Первый следующий checkpoint — не «ещё один хороший WAV», а D0 result:

- exact revisions и metadata roots NISR/VibraVerse;
- доказанная либо отвергнутая generation lineage;
- нулевой payload access при provenance reject либо отдельный заранее
  определённый bounded sample при pass;
- явное `SyntheticTeacherTrusted`/`SyntheticTeacherUntrusted` для каждого
  источника;
- решение, какие exact modal labels получает D1 corpus compiler.

После него работа автоматически переходит к D1/B0. Неудача обоих synthetic
teachers не останавливает весь проект: B0 использует analytic + Clatter, а
контур E продолжает real-source и validator growth.
