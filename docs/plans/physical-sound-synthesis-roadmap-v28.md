...
| Status | `ADOPTED / H0_RESOURCE_CAUSE_CONFIRMED / R0_PREFIX_PROTOCOL_NEXT / Q0_SIGNAL_BLIND_CAN_PROCEED / M0B_SPENT / RUNTIME_ML_NOT_AUTHORIZED` |
...
| R0 | M0c protocol and resource oracle | `NEXT` | Freeze inherited hashes and the sole `144,000 → 4,096` training change; define exact equivalence fixtures, full-owner cost workload, wall/RSS gates, access order and terminal outcomes before model values. |
| R1 | Prefix-bounded M0c owner | `BLOCKED_BY_R0` | Complete owner passes A/B fixtures byte-exactly; random and adversarial prefix fixtures match full loss/components/gradients exactly; official-shape cost workload passes twice under the frozen envelope. |
...
1. Freeze R0 M0c protocol from H0; do not touch official values.
2. Implement R1 and pass equivalence/full-owner resource gates twice; commit
