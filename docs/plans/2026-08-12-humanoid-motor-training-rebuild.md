...
`8.8592`; exact audit rejected it and restored the baseline. R84 now replaces
only the stale baseline Jacobian with the already computed trial-point
Jacobian, retaining hard rows, trust and final exact-zero gate. Only `cmu139`
PASS permits an unchanged clean all-three/all-17 offline rerun; only that PASS
