...
pub const LEGACY_NATIVE_GATE_SCHEMA_VERSION: u32 = 1;
pub const NATIVE_GATE_SCHEMA_VERSION: u32 = 2;
...
pub fn validate_native_gate_linux_report(
    ...
    validate_check_records_v2(report.status, &report.checks)?;

    if let Some(target) = &report.release_target {
        validate_linux_release_target(target)?;
    }
    if let Some(roots) = &report.release_roots {
        validate_release_roots_v2(roots)?;
        ...
        validate_derived_release_roots_v2(roots)?;
    }
    ...
    match report.status {
        NativeGateRunStatusV1::Pass => {
            ...
            validate_linux_package(package, roots)?;
        }
...
pub fn validate_native_gate_target_report(
    ...
    if report.schema_version != LEGACY_NATIVE_GATE_SCHEMA_VERSION {
        return Err(report_invalid(format!(
...
fn validate_linux_release_target(
    target: &NativeGateClosureTargetSummaryV1,
) -> Result<(), NativeGateComparisonError> {
...
fn validate_derived_release_roots_v2(
    roots: &NativeGateReleaseRootsV2,
) -> Result<(), NativeGateComparisonError> {
