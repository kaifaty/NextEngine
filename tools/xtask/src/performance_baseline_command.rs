...
use xtask::performance::{
    ...
    PERFORMANCE_REPORT_FILE_NAME, PerformanceBaselineV6, PerformanceRunV6,
};
...
pub(crate) fn performance_baseline(
    ...
    let baseline = PerformanceBaselineV6::from_runs(&runs).map_err(|diagnostics| {
        format!(
    ...
    let bytes = serde_json::to_vec(&baseline)
        .map_err(|error| format!("failed to serialize PerformanceBaselineV6: {error}"))?;
