...
const RECORDING_BYTES: u64 = 1_920_058;
const RECORDING_MAXIMUM_BYTES: u64 = 2 * 1024 * 1024;
const RECORDING_FRAMES: u64 = 240_000;
    ...
    for expected in frozen.recordings {
        ...
        let audio = wav::validate_float_wav(
            ...
            wav::FloatWavExpectation {
                ...
                channel_count: 2,
                maximum_frames: RECORDING_FRAMES,
            },
        )?;
        if audio.sample_frames != RECORDING_FRAMES {
            return Err(format!(
                "YCB Impact recording {recording_id} must contain exactly {RECORDING_FRAMES} frames"
            ));
        }
...
fn frozen_object(object_id: &str) -> Result<&'static FrozenObject, String> {
    match object_id {
        ...
        "28" => Ok(&SKILLET_LID),
        _ => Err(format!(
            "YCB Impact adapter v1 does not freeze object {object_id}"
        )),
    }
}
...
fn validate_artifacts(source: &InternetSource, frozen: &FrozenObject) -> Result<(), String> {
    if source.artifacts.len() != frozen.recordings.len() + 1 {
        return Err(format!(
            "source {} must contain four recordings and one metadata workbook",
            source.id
        ));
    }
    require_artifact(
