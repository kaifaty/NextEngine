...
when the service supports it. Reuse existing authorization; ask immediately
before a material external mutation only when that action is not yet authorized. A broad command such as `fix` or `auto` must not
