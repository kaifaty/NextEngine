...
> NextEngine already configures this server in `.codex/config.toml` through
> `tools/qmd/mcp.mjs`. Current QMD exposes the search tool as `query`; older
> examples may call the same structured-search operation `structured_search`.
